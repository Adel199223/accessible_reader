...
const outputDir = process.env.RECALL_STAGE641_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE641_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE641_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE641_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE641_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE641_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
    path.join(
      outputDir,
      'stage641-home-lower-card-copy-seam-and-title-source-chip-descent-settle-reset-after-stage640-failure.png',
    ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeImplementation(page, outputDir, 'stage641')

  await writeFile(
    path.join(
      outputDir,
      'stage641-home-lower-card-copy-seam-and-title-source-chip-descent-settle-reset-after-stage640-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        implementationFocus: [
          ...
          'The next polish pass should reduce the remaining March 25, 2026 Recall homepage mismatch by settling one final subtler lower-card copy-seam and title/source/chip descent step beyond the Stage 639 finish state.',
...
async function captureHomeImplementation(page, directory, stagePrefix) {
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 641 card'),
    readMeasuredBox(preview, 'Stage 641 card preview'),
    readMeasuredBox(copy, 'Stage 641 card copy block'),
    readMeasuredBox(title, 'Stage 641 card title'),
    readMeasuredBox(sourceRow, 'Stage 641 card source row'),
    readMeasuredBox(chip, 'Stage 641 card chip'),
  ])
  ...
  const [titleStyles, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 641 Home canvas, day header, and grid.')
      }
  ...
  if (!(titleStyles.lineHeightPx > stage639TitleLineHeightPxBaseline && titleStyles.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 639 baseline, found ${titleStyles.lineHeightPx}.`)
  }
