...
const outputDir = process.env.RECALL_STAGE399_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE399_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE399_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE399_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE399_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE399_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage399-home-filtered-card-density-reset-after-stage398-failure.png'), {
  force: true,
})
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage399-home-wide-top.png')
  const homeControlSeamWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage399-home-control-seam-wide-top.png',
    320,
  )
  const homeOrganizerRailWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage399-home-organizer-rail-wide-top.png',
    980,
  )
  const homePrimaryFlowWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage399-home-primary-flow-wide-top.png',
    980,
  )
  const homeSelectedGroupBoardWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage399-home-selected-group-board-wide-top.png',
    960,
    'locator',
  )
  const homePinnedReopenShelfWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage399-home-pinned-reopen-shelf-wide-top.png',
    920,
    'locator',
  )
  ...
  const homeOrganizerRailCollapsedWideTop = await captureLocatorTopScreenshot(
    ...
    await getVisibleLocator(desktopPage, ['[aria-label="Home browse strip"]']),
    outputDir,
    'stage399-home-organizer-rail-collapsed-wide-top.png',
    980,
  )
  ...
  const homeAlternateGroupBoardWideTop = alternateOrganizerGroup
    ? await captureLocatorTopScreenshot(
        ...
        await getVisibleLocator(desktopPage, ['[aria-label="Saved library"]']),
        outputDir,
        'stage399-home-selected-group-board-alternate-wide-top.png',
        960,
        'locator',
      )
    : null
  ...
  await writeFile(
    path.join(outputDir, 'stage399-home-filtered-card-density-reset-after-stage398-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        ...
        validationFocus: [
          'wide desktop Home is judged first after the Stage 398 checkpoint',
          'evidence centers the organizer rail that stayed stable, the denser selected-group board, the tighter pinned reopen sidecar, and the reduced lower dead space',
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage399-home-filtered-card-density-reset-after-stage398-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
