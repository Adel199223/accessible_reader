...
const outputDir = process.env.RECALL_STAGE274_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE274_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE274_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE274_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE274_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE274_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage274-post-stage273-narrower-width-focused-study-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage274-focused-overview-narrow-top.png',
  )
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage274-focused-notes-drawer-open-empty-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const focusedGraphNarrowTop = await captureTopScreenshot(page, outputDir, 'stage274-focused-graph-narrow-top.png')
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureTopScreenshot(page, outputDir, 'stage274-focused-study-narrow-top.png')
  ...
  const focusedStudyAnswerShownNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage274-focused-study-answer-shown-narrow-top.png',
  )
  const focusedStudyAnswerShownNarrowFull = await captureFullPageScreenshot(
    page,
    outputDir,
    'stage274-focused-study-answer-shown-narrow-full.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureTopScreenshot(page, outputDir, 'stage274-reader-narrow-top.png')

  await writeFile(
    path.join(outputDir, 'stage274-post-stage273-narrower-width-focused-study-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'narrower-width focused Study answer-shown right-lane deflation beside live reading',
          'retention of the Stage 223 shell correction and Stage 225 Reader gains',
          'retention of the Stage 227 focused-source strip compression, Stage 229 split balance, Stage 249 focused Notes gains, Stage 251 focused overview gains, Stage 253 focused Study gains, Stage 263 focused Notes drawer-open empty-detail gains, Stage 267 focused Graph gains, Stage 269 focused Study gains, and Stage 271 Study answer-shown compression gains',
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage274-post-stage273-narrower-width-focused-study-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
