...
const outputDir = process.env.RECALL_STAGE315_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE315_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE315_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE315_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE315_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE315_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(
  path.join(
    outputDir,
    'stage315-bundled-narrower-width-focused-graph-same-source-continuation-tail-inline-compaction-failure.png',
  ),
  {
    force: true,
  },
...
try {
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage315-focused-overview-narrow-top.png',
  )
  ...
  const focusedGraphNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage315-focused-graph-narrow-top.png')
  const focusedGraphNarrowFull = await captureFullPageScreenshot(
    page,
    outputDir,
    'stage315-focused-graph-narrow-full.png',
  )
  ...
  const focusedGraphDeepestSameSourceTailNarrowTop = await captureCurrentViewportScreenshot(
    page,
    outputDir,
    'stage315-focused-graph-deepest-same-source-tail-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage315-reader-narrow-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage315-bundled-narrower-width-focused-graph-same-source-continuation-tail-inline-compaction-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
