...
const outputDir = process.env.RECALL_STAGE464_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE464_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE464_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE464_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE464_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE464_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage464-post-stage463-home-organizer-deck-and-results-sheet-audit-failure.png',
  ),
  {
    force: true,
  },
...
await rm(
  path.join(
    outputDir,
    'stage464-post-stage463-home-organizer-deck-and-results-sheet-audit-failure-reader.png',
  ),
  {
    force: true,
  },
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage464-home-wide-top.png')
  const homeOrganizerDeckWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage464-home-organizer-deck-wide-top.png',
    460,
    'locator',
  )
  const homeOrganizerListStartWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage464-home-organizer-list-start-wide-top.png',
    760,
    'locator',
  )
  const homeInlineReopenStripWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage464-home-inline-reopen-strip-wide-top.png',
    420,
    'locator',
  )
  const homeActiveBoardStateWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage464-home-active-board-state-wide-top.png',
    880,
    'locator',
  )
  ...
  const homeFilteredResultsWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage464-home-filtered-results-wide-top.png',
    880,
    'locator',
  )
  ...
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage464-graph-wide-top.png')
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopReaderPage,
    outputDir,
    'stage464-reader-original-wide-top.png',
  )

  await writeFile(
    path.join(
      outputDir,
      'stage464-post-stage463-home-organizer-deck-and-results-sheet-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 463 organizer-deck and results-sheet reset',
          'Home evidence centers a flatter organizer deck, an earlier working board start, and a lighter inline reopen strip inside the same sheet',
