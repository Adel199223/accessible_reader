...
const outputDir = process.env.RECALL_STAGE293_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE293_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE293_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE293_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE293_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE293_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(
  path.join(
    outputDir,
    'stage293-bundled-narrower-width-focused-graph-trailing-same-source-confidence-action-seam-deflation-failure.png',
  ),
  {
    force: true,
  },
...
try {
  ...
  const focusedOverviewNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage293-focused-overview-narrow-top.png',
  )
  ...
  const focusedGraphNarrowTop = await captureTopScreenshot(page, outputDir, 'stage293-focused-graph-narrow-top.png')
  const focusedGraphNarrowFull = await captureFullPageScreenshot(
    page,
    outputDir,
    'stage293-focused-graph-narrow-full.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureTopScreenshot(page, outputDir, 'stage293-reader-narrow-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage293-bundled-narrower-width-focused-graph-trailing-same-source-confidence-action-seam-deflation-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
