...
const outputDir = process.env.RECALL_STAGE653_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE653_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE653_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE653_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE653_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE653_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage653-home-poster-differentiation-and-parity-closeout-reset-after-stage652-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeImplementation(page, outputDir, 'stage653')

  await writeFile(
    path.join(
      outputDir,
      'stage653-home-poster-differentiation-and-parity-closeout-reset-after-stage652-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        implementationFocus: [
          ...
          'The next broader closeout pass should reduce the remaining March 25, 2026 Recall homepage mismatch by making Home posters read less blank and less samey at page scale without reopening shell, rail, toolbar, or width cadence.',
          'Shell, rail, toolbar, lower-card cadence, and board-start compaction should stay intact while paste, web, and file previews gain clearer source-aware hero text and differentiated accent treatment for screenshot-comparison review.',
        ],
...
async function captureHomeImplementation(page, directory, stagePrefix) {
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 653 card'),
    readMeasuredBox(preview, 'Stage 653 card preview'),
    readMeasuredBox(copy, 'Stage 653 card copy block'),
    readMeasuredBox(title, 'Stage 653 card title'),
    readMeasuredBox(sourceRow, 'Stage 653 card source row'),
    readMeasuredBox(chip, 'Stage 653 card chip'),
  ])

  const [titleStyles, sourceState, chipState, previewHeroState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 653 Home canvas, day header, and grid.')
      }
  ...
  if (!previewHeroState.text.trim().length) {
    throw new Error('Expected a visible Stage 653 preview hero text node in the representative Home card.')
  }
  ...
  if (distinctPreviewGlowCount < 2) {
    throw new Error(`Expected at least two distinct Stage 653 preview accent glows in the first day group, found ${distinctPreviewGlowCount}.`)
  }
...
async function readTextNodeState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible text content for the measured Stage 653 node.')
  }
  return {
...
async function readChipState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible chip text for the measured Stage 653 chip.')
  }
  return {
    alpha: parseColorAlpha(styles.color),
