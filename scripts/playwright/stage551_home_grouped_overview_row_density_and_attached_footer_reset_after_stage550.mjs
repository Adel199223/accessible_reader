...
const outputDir = process.env.RECALL_STAGE551_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE551_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE551_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE551_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE551_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE551_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
const maximumOverviewRowHeight = 59

await mkdir(outputDir, { recursive: true })
await rm(
  path.join(
    outputDir,
    'stage551-home-grouped-overview-row-density-and-attached-footer-reset-after-stage550-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeOverviewEvidence = await captureHomeOverviewEvidence(page, outputDir, 'stage551')

  await writeFile(
    path.join(
      outputDir,
      'stage551-home-grouped-overview-row-density-and-attached-footer-reset-after-stage550-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        runtimeBrowser,
        validationFocus: [
          'wide desktop Home keeps the grouped overview open while grouped-overview board rows retire the preview-detail line and the Captures footer reads like attached continuation',
          'the Stage 547 lean title seam, the Stage 549 lean card tops, and the Stage 537 primary-lane plus secondary-stack composition should stay intact while the card bodies get denser',
          'the row-density and attached-footer reset should improve the working-board read without widening into selected-group or generated-content Reader work',
        ],
      },
      null,
      2,
...
async function captureHomeOverviewEvidence(page, directory, stagePrefix) {
  ...
  if (
    ...
    throw new Error('Grouped overview still rendered the old helper sentence; the lean grouped-overview heading reset did not hold.')
  }
  ...
  if (/groups/i.test(overviewMetaText) || /\bboard\b/i.test(overviewMetaText)) {
    throw new Error(
      `Overview status summary stayed too verbose (${overviewMetaText}); the inline title note still exposed redundant groups or Board labels.`,
    )
  }

  const overviewCards = await getVisibleLocators(savedLibraryOverview, '.recall-home-library-card-stage551-reset')
  ...
  if (!primaryMetric || secondaryMetrics.length !== 2) {
    throw new Error('Could not measure the primary lane and secondary stack cards for the Stage 551 validation.')
  }

  for (const metric of cardMetrics) {
    if (metric.rowDetailCount == null || metric.rowHeight == null) {
      throw new Error(`Could not measure the grouped-overview row-density geometry for ${metric.title ?? 'one card'}.`)
    }
    if (metric.rowDetailCount > 0) {
      throw new Error(
        `Grouped-overview card ${metric.title} still rendered ${metric.rowDetailCount} preview-detail node(s); the row-density reset did not hold.`,
      )
    }
    if (metric.rowHeight > maximumOverviewRowHeight) {
      throw new Error(
        `Grouped-overview card ${metric.title} row height (${metric.rowHeight}px) stayed too tall; the denser source-list treatment did not hold.`,
      )
    }
    if (metric.footerButtonHeight != null && metric.footerButtonHeight > maximumOverviewFooterButtonHeight) {
      throw new Error(
        `Grouped-overview card ${metric.title} footer button height (${metric.footerButtonHeight}px) stayed too tall; the attached-footer reset did not hold.`,
      )
    }
    if (metric.title === 'Captures' && metric.footerButtonHeight == null) {
      throw new Error(
        'The Captures grouped-overview card no longer exposed a measurable Show all footer button for the Stage 551 validation.',
      )
    }
  }
  ...
  if (!overviewBox || !headerBox || !gridBox || !titleRowBox || !titleHeadingBox || !metaBox) {
    throw new Error('Could not measure the grouped-overview shell geometry for the Stage 551 validation.')
  }
  ...
  if (overviewHeaderHeight > maximumOverviewHeaderHeight) {
    throw new Error(
      `Overview header height (${overviewHeaderHeight}px) stayed too tall; the lean grouped-overview heading regressed while row density was being tightened.`,
    )
  }
  ...
  if (statusBlockHeight > maximumStatusBlockHeight) {
    throw new Error(
      `Status block height (${statusBlockHeight}px) stayed too tall; the overview status note regressed while row density was being tightened.`,
    )
  }
...
async function getCardMetric(card) {
  const [box, firstRowBox, rowDetailCount, footerButtonBox, rowCount] = await Promise.all([
    card.boundingBox(),
    card.locator('.recall-home-library-stage-row-stage551-reset').first().boundingBox().catch(() => null),
    ...
    card.locator('.recall-home-library-stage-row-stage551-reset').count().catch(() => null),
  ])
  if (!box) {
    return null
  }
