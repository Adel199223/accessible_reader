...
const outputDir = process.env.RECALL_STAGE510_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE510_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE510_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE510_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE510_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE510_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage510-post-stage509-home-organizer-child-row-metadata-and-branch-footer-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage510-post-stage509-home-organizer-child-row-metadata-and-branch-footer-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeWideTop = path.join(outputDir, 'stage510-home-wide-top.png')
  await desktopPage.screenshot({ path: homeWideTop })

  const homeOrganizerChildRowsWideTop = path.join(outputDir, 'stage510-home-organizer-child-rows-wide-top.png')
  await childrenContainer.screenshot({ path: homeOrganizerChildRowsWideTop })

  const homeOrganizerChildRowWideTop = path.join(outputDir, 'stage510-home-organizer-child-row-wide-top.png')
  await firstChildRow.screenshot({ path: homeOrganizerChildRowWideTop })

  const homeOrganizerBranchFooterWideTop = path.join(outputDir, 'stage510-home-organizer-branch-footer-wide-top.png')
  await footer.screenshot({ path: homeOrganizerBranchFooterWideTop })

  const homeSelectedGroupWideTop = path.join(outputDir, 'stage510-home-selected-group-wide-top.png')
  await desktopPage.screenshot({ path: homeSelectedGroupWideTop })

  await openGraph(desktopPage)
  const graphWideTop = path.join(outputDir, 'stage510-graph-wide-top.png')
  ...
  const readerOriginalWideTop = path.join(outputDir, 'stage510-reader-original-wide-top.png')
  await desktopReaderPage.screenshot({ path: readerOriginalWideTop })

  await writeFile(
    path.join(
      outputDir,
      'stage510-post-stage509-home-organizer-child-row-metadata-and-branch-footer-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 509 child-row metadata and branch-footer deflation pass',
          'active-branch child rows should now feel lighter and less metadata-heavy while remaining easy to scan',
          'the Show all or Show fewer footer should now read more like attached branch continuation while Graph and original-only Reader remain regression surfaces',
        ],
        baseUrl,
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 510 audit.')
  }
  await page.waitForTimeout(250)
  return { sourceTitle }
}
...
async function getSourceTitle(locator) {
  ...
  throw new Error('Could not determine the source title from the selected Home source button.')
}
