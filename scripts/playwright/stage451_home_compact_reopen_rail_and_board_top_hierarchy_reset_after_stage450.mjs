...
const outputDir = process.env.RECALL_STAGE451_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE451_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE451_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE451_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE451_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE451_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(outputDir, 'stage451-home-compact-reopen-rail-and-board-top-hierarchy-reset-after-stage450-failure.png'),
  {
    force: true,
  },
)
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage451-home-wide-top.png')
  const homeOrganizerRailWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage451-home-organizer-rail-wide-top.png',
    820,
    'locator',
  )
  const homeCompactReopenRailWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage451-home-compact-reopen-rail-wide-top.png',
    360,
    'locator',
  )
  const homePrimaryBoardShellWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage451-home-primary-board-shell-wide-top.png',
    980,
  )
  const homeActiveBoardStateWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage451-home-active-board-state-wide-top.png',
    860,
    'locator',
  )

  await writeFile(
    path.join(outputDir, 'stage451-home-compact-reopen-rail-and-board-top-hierarchy-reset-after-stage450-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        ...
        runtimeBrowser,
        validationFocus: [
          'wide desktop Home is judged first after the Stage 450 checkpoint',
          'evidence centers a compact reopen rail that stays visible without dominating the board top like a featured hero shelf',
          'the selected-group board starts earlier and reads more like one continuous organizer-led work surface without widening into Graph or generated-content Reader work',
        ],
      },
      null,
      2,
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(
          outputDir,
          'stage451-home-compact-reopen-rail-and-board-top-hierarchy-reset-after-stage450-failure.png',
        ),
        fullPage: true,
      })
      .catch(() => undefined)
