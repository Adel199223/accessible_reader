...
const outputDir = process.env.RECALL_STAGE621_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE621_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE621_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE621_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE621_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE621_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage621-home-lower-card-share-and-title-source-chip-follow-through-reset-after-stage620-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeImplementation(page, outputDir, 'stage621')

  await writeFile(
    path.join(
      outputDir,
      'stage621-home-lower-card-share-and-title-source-chip-follow-through-reset-after-stage620-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        implementationFocus: [
          ...
          'The next polish pass should reduce the remaining March 25, 2026 Recall homepage mismatch by reclaiming a little more height from the poster and returning it to the lower title/source/chip seam.',
          'Toolbar, rail, column cadence, and source-aware poster variants should stay intact while the lower card rhythm reads slightly fuller and steadier at page scale.',
        ],
...
async function captureHomeImplementation(page, directory, stagePrefix) {
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 621 card'),
    readMeasuredBox(preview, 'Stage 621 card preview'),
    readMeasuredBox(copy, 'Stage 621 card copy block'),
    readMeasuredBox(title, 'Stage 621 card title'),
    readMeasuredBox(sourceRow, 'Stage 621 card source row'),
    readMeasuredBox(chip, 'Stage 621 card chip'),
  ])
  ...
  const [titleStyles, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 621 Home canvas, day header, and grid.')
      }
  ...
  if (!(titleStyles.lineHeightPx > stage614TitleLineHeightPxBaseline && titleStyles.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 614 baseline, found ${titleStyles.lineHeightPx}.`)
  }
  ...
  if (!(representativeCardBox.height <= cardHeightPxCeiling)) {
    throw new Error(`Expected representative card height to stay compact after the Stage 621 pass, found ${representativeCardBox.height}.`)
  }
...
async function readTextNodeState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible text content for the measured Stage 621 node.')
  }
  return {
...
async function readChipState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible chip text for the measured Stage 621 chip.')
  }
  return {
    alpha: parseColorAlpha(styles.color),
