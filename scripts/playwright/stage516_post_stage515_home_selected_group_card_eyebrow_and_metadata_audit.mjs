...
const outputDir = process.env.RECALL_STAGE516_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE516_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE516_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE516_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE516_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE516_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage516-post-stage515-home-selected-group-card-eyebrow-and-metadata-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage516-post-stage515-home-selected-group-card-eyebrow-and-metadata-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const firstCard = savedLibrarySection.locator('.recall-home-library-stage-row-stage515-reset').first()
  const eyebrow = firstCard.locator('.recall-home-library-stage-row-overline-stage515-reset').first()
  const metadata = firstCard.locator('.recall-home-library-stage-row-meta-compact-stage515-reset').first()
  const title = firstCard.locator('strong').first()

  await savedLibrarySection.waitFor({ state: 'visible', timeout: 20000 })
  await firstCard.waitFor({ state: 'visible', timeout: 20000 })
  await eyebrow.waitFor({ state: 'visible', timeout: 20000 })
  await metadata.waitFor({ state: 'visible', timeout: 20000 })
  await title.waitFor({ state: 'visible', timeout: 20000 })
  await desktopPage.waitForTimeout(300)

  const eyebrowText = ((await eyebrow.textContent().catch(() => '')) ?? '').trim()
  const metadataText = ((await metadata.textContent().catch(() => '')) ?? '').trim()
  const sourceTitle = ((await title.textContent().catch(() => '')) ?? '').trim()

  const homeWideTop = path.join(outputDir, 'stage516-home-wide-top.png')
  await desktopPage.screenshot({ path: homeWideTop })

  const homeSelectedGroupCardWideTop = path.join(outputDir, 'stage516-home-selected-group-card-wide-top.png')
  await firstCard.screenshot({ path: homeSelectedGroupCardWideTop })

  const homeSelectedGroupCardEyebrowWideTop = path.join(
    outputDir,
    'stage516-home-selected-group-card-eyebrow-wide-top.png',
  )
  await eyebrow.screenshot({ path: homeSelectedGroupCardEyebrowWideTop })

  const homeSelectedGroupCardMetadataWideTop = path.join(
    outputDir,
    'stage516-home-selected-group-card-metadata-wide-top.png',
  )
  await metadata.screenshot({ path: homeSelectedGroupCardMetadataWideTop })

  const homeSelectedGroupWideTop = path.join(outputDir, 'stage516-home-selected-group-wide-top.png')
  await savedLibrarySection.screenshot({ path: homeSelectedGroupWideTop })

  await openGraph(desktopPage)
  const graphWideTop = path.join(outputDir, 'stage516-graph-wide-top.png')
  ...
  const readerOriginalWideTop = path.join(outputDir, 'stage516-reader-original-wide-top.png')
  await desktopReaderPage.screenshot({ path: readerOriginalWideTop })

  await writeFile(
    path.join(
      outputDir,
      'stage516-post-stage515-home-selected-group-card-eyebrow-and-metadata-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 515 selected-group card eyebrow and metadata density deflation pass',
          'the first visible selected-group card should now feel lighter while keeping source identity explicit',
          'the compact metadata line should hold while Graph and original-only Reader remain regression surfaces',
        ],
        baseUrl,
        benchmarkMatrix: 'docs/ux/recall_benchmark_matrix.md',
        captures: {
          graphWideTop,
          homeSelectedGroupCardEyebrowWideTop,
          homeSelectedGroupCardMetadataWideTop,
          homeSelectedGroupCardWideTop,
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 516 audit.')
  }
  await page.waitForTimeout(250)
  return sourceTitle
}
