...
const outputDir = process.env.RECALL_STAGE426_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE426_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE426_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE426_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE426_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE426_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage426-post-stage425-home-active-summary-copy-audit-failure.png'), {
  force: true,
})
await rm(path.join(outputDir, 'stage426-post-stage425-home-active-summary-copy-audit-failure-reader.png'), {
  force: true,
})
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage426-home-wide-top.png')
  const homeControlSeamWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage426-home-control-seam-wide-top.png',
    220,
  )
  const homeOrganizerRailWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage426-home-organizer-rail-wide-top.png',
    980,
  )
  const homeOrganizerSelectedStateWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage426-home-organizer-selected-state-wide-top.png',
    360,
    'locator',
  )
  const homePrimaryFlowWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage426-home-primary-flow-wide-top.png',
    1040,
  )
  const homeSelectedGroupBoardWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage426-home-selected-group-board-wide-top.png',
    1040,
    'locator',
  )
  const homePinnedReopenShelfWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage426-home-pinned-reopen-shelf-wide-top.png',
    920,
    'locator',
  )
  ...
  const homeOrganizerRailCollapsedWideTop = await captureLocatorTopScreenshot(
    ...
    await getVisibleLocator(desktopPage, ['[aria-label="Home browse strip"]']),
    outputDir,
    'stage426-home-organizer-rail-collapsed-wide-top.png',
    980,
  )
  ...
  const homeAlternateGroupBoardWideTop = alternateOrganizerGroup
    ? await captureLocatorTopScreenshot(
        ...
        await getVisibleLocator(desktopPage, ['[aria-label="Saved library"]']),
        outputDir,
        'stage426-home-selected-group-board-alternate-wide-top.png',
        1040,
        'locator',
      )
    : null
  ...
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage426-graph-wide-top.png')
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopReaderPage,
    outputDir,
    'stage426-reader-original-wide-top.png',
  )

  await writeFile(
    path.join(outputDir, 'stage426-post-stage425-home-active-summary-copy-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 425 active summary copy-weight reset',
          'Home evidence covers the lighter active bridge copy, the calmer selected-row rhythm, the stable pinned next-source sidecar, and the unchanged broader utility-first entry',
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage426-post-stage425-home-active-summary-copy-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (desktopReaderPage && desktopReaderPage !== desktopPage) {
    await desktopReaderPage
      .screenshot({
        path: path.join(outputDir, 'stage426-post-stage425-home-active-summary-copy-audit-failure-reader.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 426 audit.')
  }
