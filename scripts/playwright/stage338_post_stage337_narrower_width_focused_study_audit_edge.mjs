...
const outputDir = process.env.RECALL_STAGE338_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE338_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE338_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE338_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE338_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE338_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage338-post-stage337-narrower-width-focused-study-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage338-focused-overview-narrow-top.png',
  )
  ...
  const focusedStudyNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage338-focused-study-narrow-top.png')
  ...
  const focusedStudyRightLaneNarrowPanelTop = await captureLocatorTopScreenshot(
    ...
    focusedStudyPanel,
    outputDir,
    'stage338-focused-study-right-lane-narrow-panel-top.png',
    760,
  )
  ...
  const focusedStudyAnswerShownNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage338-focused-study-answer-shown-narrow-top.png',
  )
  const focusedStudyAnswerShownRightLaneNarrowPanelTop = await captureLocatorTopScreenshot(
    page,
    focusedStudyPanel,
    outputDir,
    'stage338-focused-study-answer-shown-right-lane-narrow-panel-top.png',
    760,
  )
  ...
  await page.waitForTimeout(350)
  const focusedGraphNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage338-focused-graph-narrow-top.png')
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage338-focused-notes-drawer-open-empty-narrow-top.png',
  )
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage338-post-stage337-narrower-width-focused-study-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
