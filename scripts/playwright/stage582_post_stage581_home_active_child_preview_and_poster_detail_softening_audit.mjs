...
const outputDir = process.env.RECALL_STAGE582_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE582_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE582_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE582_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE582_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE582_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(outputDir, 'stage582-post-stage581-home-active-child-preview-and-poster-detail-softening-audit-failure.png'),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage582-post-stage581-home-active-child-preview-and-poster-detail-softening-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeAudit(page, outputDir, 'stage582')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage582-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const readerEvidence = await openOriginalReaderFromHome(readerPage, outputDir, 'stage582')

  await writeFile(
    path.join(
      outputDir,
      'stage582-post-stage581-home-active-child-preview-and-poster-detail-softening-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 581 active child-preview and poster-detail softening pass',
          'the Home surface should keep the Stage 563 structure while calming the active child-preview seam and softening poster detail lines',
          'Graph and original-only Reader remain regression surfaces only after the Home-only Stage 581 pass',
        ],
        baseUrl,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(
          outputDir,
          'stage582-post-stage581-home-active-child-preview-and-poster-detail-softening-audit-failure.png',
        ),
        fullPage: true,
      })
      .catch(() => undefined)
...
async function openOriginalReaderFromHome(page, directory, stagePrefix) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not keep Reader in Original mode for the Stage 582 audit.')
  }
