...
const outputDir = process.env.RECALL_STAGE270_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE270_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE270_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE270_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE270_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE270_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage270-post-stage269-narrower-width-focused-study-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage270-focused-overview-narrow-top.png',
  )
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage270-focused-notes-drawer-open-empty-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const focusedGraphNarrowTop = await captureTopScreenshot(page, outputDir, 'stage270-focused-graph-narrow-top.png')
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureTopScreenshot(page, outputDir, 'stage270-focused-study-narrow-top.png')
  ...
  const focusedStudyAnswerShownNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage270-focused-study-answer-shown-narrow-top.png',
  )
  const focusedStudyAnswerShownNarrowFull = await captureFullPageScreenshot(
    page,
    outputDir,
    'stage270-focused-study-answer-shown-narrow-full.png',
  )
  ...
  const focusedStudyQueueOpenNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage270-focused-study-queue-open-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureTopScreenshot(page, outputDir, 'stage270-reader-narrow-top.png')

  await writeFile(
    path.join(outputDir, 'stage270-post-stage269-narrower-width-focused-study-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'bundled narrower-width focused Study right-lane panel fusion beside live reading',
          'retention of the Stage 223 shell correction and Stage 227 focused-source strip compression',
          'retention of the Stage 225 Reader gains, Stage 249 focused Notes gains, Stage 251 focused overview gains, Stage 253 focused Study gains, Stage 257 focused Study gains, Stage 259 focused Notes empty-rail gains, Stage 261 focused Notes drawer-open browse-empty gains, Stage 263 focused Notes drawer-open empty-detail gains, and Stage 267 focused Graph gains',
          'neighboring focused Notes and Graph stability after the bundled focused Study lane pass',
        ],
        baseUrl,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage270-post-stage269-narrower-width-focused-study-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
