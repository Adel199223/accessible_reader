...
const outputDir = process.env.RECALL_STAGE542_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE542_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE542_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE542_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE542_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE542_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const desktopViewport = { width: 1600, height: 1080 }
const minimumPrimaryWidthDelta = 80
const maximumSecondaryColumnXSpread = 12
const minimumSecondaryRowTopDelta = 60
const maximumOverviewHeaderHeight = 70
const maximumGridOffsetFromOverviewTop = 84
const maximumTitleRowTopOffsetFromOverviewTop = 18
const maximumTitleStatusTopDelta = 8
const maximumStatusBlockHeight = 18

await mkdir(outputDir, { recursive: true })
await rm(
  path.join(
    outputDir,
    'stage542-post-stage541-home-grouped-overview-eyebrow-retirement-and-status-seam-integration-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage542-post-stage541-home-grouped-overview-eyebrow-retirement-and-status-seam-integration-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeOverviewEvidence = await captureHomeOverviewEvidence(page, outputDir, 'stage542')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage542-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const { sourceTitle } = await openOriginalReaderFromHome(readerPage)
  const readerOriginalWideTop = await captureViewportScreenshot(readerPage, outputDir, 'stage542-reader-original-wide-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage542-post-stage541-home-grouped-overview-eyebrow-retirement-and-status-seam-integration-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 541 grouped-overview eyebrow retirement and status-seam integration',
          'the grouped overview should now start with All collections plus attached status metadata while the Stage 537 primary-lane and secondary-stack composition stays intact',
...
async function captureHomeOverviewEvidence(page, directory, stagePrefix) {
  ...
  const overviewHeader = savedLibraryOverview.locator('.recall-home-library-stream-header-stage541-reset').first()
  const overviewGrid = savedLibraryOverview.locator('.recall-home-library-stream-grid-stage541-reset').first()
  const overviewTitleRow = savedLibraryOverview.locator('.recall-home-library-stream-title-row-stage541-reset').first()
  const overviewMeta = savedLibraryOverview.locator('.recall-home-library-stream-meta-stage541-reset').first()
  ...
  if (!primaryMetric || secondaryMetrics.length !== 2) {
    throw new Error('Could not measure the primary lane and secondary stack cards for the Stage 542 audit.')
  }
  ...
  if (!overviewBox || !headerBox || !gridBox || !titleRowBox || !metaBox) {
    throw new Error('Could not measure the grouped-overview shell geometry for the Stage 542 audit.')
  }
  ...
  if (titleStatusTopDelta > maximumTitleStatusTopDelta) {
    throw new Error(
      `Title and status top delta (${titleStatusTopDelta}px) stayed too large; the status summary did not attach to the title seam.`,
    )
  }
  if (statusBlockHeight > maximumStatusBlockHeight) {
    throw new Error(
      `Status block height (${statusBlockHeight}px) stayed too tall; the overview status still reads like a chip strip instead of seam metadata.`,
    )
  }
