...
const outputDir = process.env.RECALL_STAGE539_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE539_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE539_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE539_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE539_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE539_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage539-home-grouped-overview-header-seam-compaction-and-earlier-board-start-reset-after-stage538-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeOverviewEvidence = await captureHomeOverviewEvidence(page, outputDir, 'stage539')

  await writeFile(
    path.join(
      outputDir,
      'stage539-home-grouped-overview-header-seam-compaction-and-earlier-board-start-reset-after-stage538-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        runtimeBrowser,
        validationFocus: [
          'wide desktop Home keeps the grouped overview open while the title-helper seam compacts and the working cards start earlier',
          'the Stage 537 primary-lane and secondary-stack composition should stay intact while the overview shell reads less like a padded intro panel',
          'the shell compaction should improve the working-board read without widening into selected-group or generated-content Reader work',
        ],
      },
      null,
      2,
...
async function captureHomeOverviewEvidence(page, directory, stagePrefix) {
  ...
  if (!primaryMetric || secondaryMetrics.length !== 2) {
    throw new Error('Could not measure the primary lane and secondary stack cards for the Stage 539 validation.')
  }
  ...
  if (!overviewBox || !headerBox || !gridBox) {
    throw new Error('Could not measure the grouped-overview shell geometry for the Stage 539 validation.')
  }
