...
const outputDir = process.env.RECALL_STAGE532_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE532_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE532_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE532_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE532_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE532_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage532-post-stage531-reader-dock-tray-and-glance-note-retirement-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage532-post-stage531-reader-dock-tray-and-glance-note-retirement-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage532-home-wide-top.png')

  await openGraph(desktopPage)
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage532-graph-wide-top.png')
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopReaderPage,
    outputDir,
    'stage532-reader-original-wide-top.png',
  )
  const readerDeckJoinWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage532-reader-deck-join-wide-top.png',
    430,
  )
  const readerArticleLeadWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage532-reader-article-lead-wide-top.png',
    430,
  )
  const readerDockTrayWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage532-reader-dock-tray-wide-top.png',
    460,
  )
  const readerMetrics = await getReaderSupportTrayMetrics(desktopReaderPage)

  await writeFile(
    path.join(
      outputDir,
      'stage532-post-stage531-reader-dock-tray-and-glance-note-retirement-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop original-only Reader is audited first after the Stage 531 support-tray reset',
          'Reader deck crops should confirm a calmer attached tray and a retired default glance note without reopening generated-content work',
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 532 audit.')
  }
