...
const outputDir = process.env.RECALL_STAGE219_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE219_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE219_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE219_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE219_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE219_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true

await mkdir(outputDir, { recursive: true })
await rm(path.join(outputDir, 'stage219-home-collection-selectivity-and-reopen-hierarchy-failure.png'), { force: true })
...
try {
  ...
  const homeDesktop = await captureScreenshot(page, outputDir, 'stage219-home-desktop.png')
  ...
  if ((await showMoreEarlierButton.count()) > 0) {
    ...
    homeExpandedDesktop = await captureScreenshot(page, outputDir, 'stage219-home-expanded-desktop.png')
  }
  ...
  const focusedOverviewDesktop = await captureScreenshot(page, outputDir, 'stage219-focused-source-overview-desktop.png')

  await writeFile(
    path.join(outputDir, 'stage219-home-collection-selectivity-and-reopen-hierarchy-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        captures: {
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage219-home-collection-selectivity-and-reopen-hierarchy-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
