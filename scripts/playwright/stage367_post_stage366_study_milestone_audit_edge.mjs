...
const outputDir = process.env.RECALL_STAGE367_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE367_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE367_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE367_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE367_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE367_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage367-post-stage366-study-milestone-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage367-home-wide-top.png')
  ...
  await desktopPage.waitForTimeout(350)
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage367-graph-wide-top.png')
  ...
  await desktopPage.waitForTimeout(350)
  const studyWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage367-study-wide-top.png')

  await revealStudyAnswer(desktopPage)
  const studyAnswerShownWideTop = await captureViewportScreenshot(
    desktopPage,
    outputDir,
    'stage367-study-answer-shown-wide-top.png',
  )
  ...
  await desktopPage.waitForTimeout(350)
  const notesWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage367-notes-wide-top.png')
  ...
  const readerWideTop = await captureViewportScreenshot(desktopReaderPage, outputDir, 'stage367-reader-wide-top.png')
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage367-focused-overview-narrow-top.png',
  )
  ...
  const focusedGraphNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage367-focused-graph-narrow-top.png',
  )
  ...
  const focusedStudyNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage367-focused-study-narrow-top.png',
  )
  ...
  const focusedStudyAnswerShownNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage367-focused-study-answer-shown-narrow-top.png',
  )
  ...
  const focusedNotesNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage367-focused-notes-narrow-top.png',
  )
  ...
  const focusedReaderNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage367-reader-narrow-top.png',
  )

  await writeFile(
    path.join(outputDir, 'stage367-post-stage366-study-milestone-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'wide-desktop Study milestone audit with Study judged first against Home, Graph, Reader, and Notes as regression baselines',
          ...
          'focused overview plus focused Graph, Study, Notes, and Reader regressions after the desktop-first Study milestone',
        ],
        baseUrl,
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage367-post-stage366-study-milestone-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (desktopReaderPage && desktopReaderPage !== desktopPage) {
    await desktopReaderPage
      .screenshot({
        path: path.join(outputDir, 'stage367-post-stage366-study-milestone-audit-failure-reader.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (focusedPage && focusedPage !== desktopPage) {
    await focusedPage
      .screenshot({
        path: path.join(outputDir, 'stage367-post-stage366-study-milestone-audit-failure-focused.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
