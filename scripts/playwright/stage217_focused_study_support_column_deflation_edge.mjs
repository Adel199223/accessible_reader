...
const outputDir = process.env.RECALL_STAGE217_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE217_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE217_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE217_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE217_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE217_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true

await mkdir(outputDir, { recursive: true })
await rm(path.join(outputDir, 'stage217-focused-study-support-column-deflation-failure.png'), { force: true })
...
try {
  ...
  const focusedOverviewDesktop = await captureScreenshot(page, outputDir, 'stage217-focused-source-overview-desktop.png')
  ...
  const focusedGraphDesktop = await captureScreenshot(page, outputDir, 'stage217-focused-graph-desktop.png')
  ...
  await page.waitForTimeout(900)
  const focusedStudyDesktop = await captureScreenshot(page, outputDir, 'stage217-focused-study-desktop.png')
  ...
  await page.waitForTimeout(900)
  const focusedNotesDesktop = await captureScreenshot(page, outputDir, 'stage217-focused-notes-desktop.png')
  ...
  await page.waitForTimeout(900)
  const readerDesktop = await captureScreenshot(page, outputDir, 'stage217-reader-desktop.png')

  await writeFile(
    path.join(outputDir, 'stage217-focused-study-support-column-deflation-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        captures: {
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage217-focused-study-support-column-deflation-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
