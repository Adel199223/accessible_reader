...
const outputDir = process.env.RECALL_STAGE754_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE754_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE754_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE754_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE754_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE754_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true

await mkdir(outputDir, { recursive: true })
await rm(path.join(outputDir, 'stage754-reader-notebook-trigger-relocation-failure.png'), { force: true })
await rm(path.join(outputDir, 'stage754-reader-notebook-trigger-relocation-regression-failure.png'), { force: true })
...
try {
  ...
  const defaultThemePanelCapture = path.join(outputDir, 'stage754-reader-theme-panel.png')
  ...
  const summaryThemePanelCapture = path.join(outputDir, 'stage754-reader-summary-theme-panel.png')
  ...
  await writeFile(
    path.join(outputDir, 'stage754-reader-notebook-trigger-relocation-after-stage753-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'Reader should retire the compact overflow Notebook action and move nearby Notebook opening to the source-strip note chip.',
          'The compact overflow should keep Theme plus read-aloud voice and rate controls, while the Theme panel still exposes only Light and Dark.',
          'Notebook support should reopen from the source-strip note chip, and Source support should still reopen inside the expanded Reader support dock after switching tabs.',
  ...
  if (readerPage) {
    await readerPage
      .screenshot({
        fullPage: true,
        path: path.join(outputDir, 'stage754-reader-notebook-trigger-relocation-failure.png'),
      })
      .catch(() => undefined)
  }
  if (regressionPage && regressionPage !== readerPage) {
    await regressionPage
      .screenshot({
        fullPage: true,
        path: path.join(outputDir, 'stage754-reader-notebook-trigger-relocation-regression-failure.png'),
      })
      .catch(() => undefined)
  }
  throw error
