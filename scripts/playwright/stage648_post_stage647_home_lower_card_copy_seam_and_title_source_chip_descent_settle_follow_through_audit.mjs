...
const outputDir = process.env.RECALL_STAGE648_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE648_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE648_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE648_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE648_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE648_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const desktopViewport = { width: 1600, height: 1080 }
const stage645PreviewRatioBaseline = 0.568598848368522
const stage645CopyRatioBaseline = 0.30978886756238005
const stage645TitleLineHeightPxBaseline = 19.9764
const stage645TitleToSourceGapPxBaseline = 5.75
const stage645SourceToChipGapPxBaseline = 5.078125
...
const previewRatioCeiling = 0.568
const copyRatioFloor = 0.31
const titleLineHeightPxFloor = 20.08
const titleToSourceGapPxFloor = 5.96
const sourceToChipGapPxFloor = 5.28
const sourceFontSizePxFloor = 10.57
const chipFontSizePxFloor = 7.88
const sourceAlphaFloor = 0.78
const chipAlphaFloor = 0.61
...
await rm(
    path.join(
      outputDir,
      'stage648-post-stage647-home-lower-card-copy-seam-and-title-source-chip-descent-settle-follow-through-audit-failure.png',
    ),
  { force: true },
)
await rm(
    path.join(
      outputDir,
      'stage648-post-stage647-home-lower-card-copy-seam-and-title-source-chip-descent-settle-follow-through-audit-failure-reader.png',
    ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeAudit(page, outputDir, 'stage648')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage648-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const readerEvidence = await openOriginalReaderFromHome(readerPage, outputDir, 'stage648')

  await writeFile(
    path.join(
      outputDir,
      'stage648-post-stage647-home-lower-card-copy-seam-and-title-source-chip-descent-settle-follow-through-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 647 lower-card copy-seam and title/source/chip settle follow-through pass',
          'the Home surface should keep the Stage 563 structure while the lower hierarchy reads slightly cleaner than the Stage 645 finish baseline across title, source row, and collection chip',
          'Graph and original-only Reader remain regression surfaces only after the Home-only Stage 647 pass',
        ],
        baseUrl,
...
async function captureHomeAudit(page, directory, stagePrefix) {
  ...
  const representativeCard = dayGroup.locator('.recall-home-parity-card-stage647').first()
  const preview = representativeCard.locator('.recall-home-parity-card-preview-stage647').first()
  const copy = representativeCard.locator('.recall-home-parity-card-copy-stage647').first()
  const title = representativeCard.locator('.recall-home-parity-card-title-stage647').first()
  const sourceRow = representativeCard.locator('.recall-home-parity-card-source-row-stage647').first()
  const source = representativeCard.locator('.recall-home-parity-card-source-stage647').first()
  const chip = representativeCard.locator('.recall-home-parity-card-chip-stage647').first()
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 648 card'),
    readMeasuredBox(preview, 'Stage 648 card preview'),
    readMeasuredBox(copy, 'Stage 648 card copy block'),
    readMeasuredBox(title, 'Stage 648 card title'),
    readMeasuredBox(sourceRow, 'Stage 648 card source row'),
    readMeasuredBox(chip, 'Stage 648 card chip'),
  ])
  ...
  const [titleState, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 648 Home canvas, day header, and grid.')
      }
  ...
  if (!(previewRatio < stage645PreviewRatioBaseline && previewRatio <= previewRatioCeiling)) {
    throw new Error(`Expected preview ratio below the Stage 645 baseline and within the target band, found ${previewRatio}.`)
  }
  if (!(copyRatio > stage645CopyRatioBaseline && copyRatio >= copyRatioFloor)) {
    throw new Error(`Expected copy ratio above the Stage 645 baseline and within the target band, found ${copyRatio}.`)
  }
  if (!(titleState.lineHeightPx > stage645TitleLineHeightPxBaseline && titleState.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 645 baseline, found ${titleState.lineHeightPx}.`)
  }
  if (!(titleToSourceGapPx > stage645TitleToSourceGapPxBaseline && titleToSourceGapPx >= titleToSourceGapPxFloor)) {
    throw new Error(`Expected title-to-source gap above the Stage 645 baseline, found ${titleToSourceGapPx}.`)
  }
  if (!(sourceToChipGapPx > stage645SourceToChipGapPxBaseline && sourceToChipGapPx >= sourceToChipGapPxFloor)) {
    throw new Error(`Expected source-to-chip gap above the Stage 645 baseline, found ${sourceToChipGapPx}.`)
  }
  if (!(sourceState.fontSizePx >= sourceFontSizePxFloor && sourceState.alpha >= sourceAlphaFloor)) {
    throw new Error(`Expected source-row legibility to stay at or above the Stage 645 band, found ${sourceState.fontSizePx}px and alpha ${sourceState.alpha}.`)
  }
  if (!(chipState.fontSizePx >= chipFontSizePxFloor && chipState.alpha >= chipAlphaFloor)) {
    throw new Error(`Expected chip legibility to stay at or above the Stage 645 band, found ${chipState.fontSizePx}px and alpha ${chipState.alpha}.`)
  }
  ...
  const webCard = canvas.locator('.recall-home-parity-card-stage647').first()
...
async function openOriginalReaderFromHome(page, directory, stagePrefix) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not keep Reader in Original mode for the Stage 648 audit.')
  }
