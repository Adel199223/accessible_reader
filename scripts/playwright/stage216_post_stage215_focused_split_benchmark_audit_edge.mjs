...
const outputDir = process.env.RECALL_STAGE216_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE216_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE216_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE216_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE216_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE216_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true

await mkdir(outputDir, { recursive: true })
await rm(path.join(outputDir, 'stage216-post-stage215-focused-split-benchmark-audit-failure.png'), { force: true })
...
try {
  ...
  const homeDesktop = await captureScreenshot(page, outputDir, 'stage216-home-desktop.png')
  const { sourceTitle, sourceWorkspace } = await openFirstSource(page)
  const focusedOverviewDesktop = await captureScreenshot(page, outputDir, 'stage216-focused-source-overview-desktop.png')
  ...
  const focusedGraphDesktop = await captureScreenshot(page, outputDir, 'stage216-focused-graph-desktop.png')
  ...
  await page.waitForTimeout(900)
  const focusedStudyDesktop = await captureScreenshot(page, outputDir, 'stage216-focused-study-desktop.png')
  ...
  await page.waitForTimeout(900)
  const focusedNotesDesktop = await captureScreenshot(page, outputDir, 'stage216-focused-notes-desktop.png')
  ...
  await page.waitForTimeout(900)
  const readerDesktop = await captureScreenshot(page, outputDir, 'stage216-reader-desktop.png')

  await writeFile(
    path.join(outputDir, 'stage216-post-stage215-focused-split-benchmark-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'focused source-strip compactness',
          'reader dominance across focused graph, notes, and study',
          'secondary-pane weight and spacing',
          'retention of calmer shell and reader chrome gains',
        ],
        baseUrl,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage216-post-stage215-focused-split-benchmark-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
