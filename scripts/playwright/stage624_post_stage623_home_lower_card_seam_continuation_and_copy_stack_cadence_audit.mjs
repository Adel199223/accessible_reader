...
const outputDir = process.env.RECALL_STAGE624_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE624_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE624_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE624_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE624_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE624_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const desktopViewport = { width: 1600, height: 1080 }
const stage621PreviewRatioBaseline = 0.6334740882917467
const stage621CopyRatioBaseline = 0.25689059500959693
const stage621TitleLineHeightPxBaseline = 16.4736
const stage621TitleToSourceGapPxBaseline = 2.875
const stage621SourceToChipGapPxBaseline = 2.234375
...
const previewRatioCeiling = 0.6325
const copyRatioFloor = 0.2585
const titleLineHeightPxFloor = 16.55
const titleToSourceGapPxFloor = 2.95
const sourceToChipGapPxFloor = 2.3
...
await rm(
  path.join(
    outputDir,
    'stage624-post-stage623-home-lower-card-seam-continuation-and-copy-stack-cadence-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage624-post-stage623-home-lower-card-seam-continuation-and-copy-stack-cadence-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeAudit(page, outputDir, 'stage624')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage624-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const readerEvidence = await openOriginalReaderFromHome(readerPage, outputDir, 'stage624')

  await writeFile(
    path.join(
      outputDir,
      'stage624-post-stage623-home-lower-card-seam-continuation-and-copy-stack-cadence-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 623 lower-card seam continuation pass',
          'the Home surface should keep the Stage 563 structure while the lower title/source/chip seam owns slightly more rhythm than the Stage 621 balance',
          'Graph and original-only Reader remain regression surfaces only after the Home-only Stage 623 pass',
        ],
        baseUrl,
...
async function captureHomeAudit(page, directory, stagePrefix) {
  ...
  const representativeCard = dayGroup.locator('.recall-home-parity-card-stage623').first()
  const preview = representativeCard.locator('.recall-home-parity-card-preview-stage623').first()
  const copy = representativeCard.locator('.recall-home-parity-card-copy-stage623').first()
  const title = representativeCard.locator('.recall-home-parity-card-title-stage623').first()
  const sourceRow = representativeCard.locator('.recall-home-parity-card-source-row-stage623').first()
  const source = representativeCard.locator('.recall-home-parity-card-source-stage623').first()
  const chip = representativeCard.locator('.recall-home-parity-card-chip-stage623').first()
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 624 card'),
    readMeasuredBox(preview, 'Stage 624 card preview'),
    readMeasuredBox(copy, 'Stage 624 card copy block'),
    readMeasuredBox(title, 'Stage 624 card title'),
    readMeasuredBox(sourceRow, 'Stage 624 card source row'),
    readMeasuredBox(chip, 'Stage 624 card chip'),
  ])
  ...
  const [titleState, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 624 Home canvas, day header, and grid.')
      }
  ...
  if (!(previewRatio < stage621PreviewRatioBaseline && previewRatio <= previewRatioCeiling)) {
    throw new Error(`Expected preview ratio below the Stage 621 baseline and within the target band, found ${previewRatio}.`)
  }
  if (!(copyRatio > stage621CopyRatioBaseline && copyRatio >= copyRatioFloor)) {
    throw new Error(`Expected copy ratio above the Stage 621 baseline and within the target band, found ${copyRatio}.`)
  }
  if (!(titleState.lineHeightPx > stage621TitleLineHeightPxBaseline && titleState.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 621 baseline, found ${titleState.lineHeightPx}.`)
  }
  if (!(titleToSourceGapPx > stage621TitleToSourceGapPxBaseline && titleToSourceGapPx >= titleToSourceGapPxFloor)) {
    throw new Error(`Expected title-to-source gap above the Stage 621 baseline, found ${titleToSourceGapPx}.`)
  }
  if (!(sourceToChipGapPx > stage621SourceToChipGapPxBaseline && sourceToChipGapPx >= sourceToChipGapPxFloor)) {
    throw new Error(`Expected source-to-chip gap above the Stage 621 baseline, found ${sourceToChipGapPx}.`)
  }
  if (!(sourceState.fontSizePx >= sourceFontSizePxFloor && sourceState.alpha >= sourceAlphaFloor)) {
    throw new Error(`Expected source-row legibility to stay at or above the Stage 621 band, found ${sourceState.fontSizePx}px and alpha ${sourceState.alpha}.`)
  }
  if (!(chipState.fontSizePx >= chipFontSizePxFloor && chipState.alpha >= chipAlphaFloor)) {
    throw new Error(`Expected chip legibility to stay at or above the Stage 621 band, found ${chipState.fontSizePx}px and alpha ${chipState.alpha}.`)
  }
  ...
  const webCard = canvas.locator('.recall-home-parity-card-stage623').first()
...
async function openOriginalReaderFromHome(page, directory, stagePrefix) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not keep Reader in Original mode for the Stage 624 audit.')
  }
...
async function readTextNodeState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible text content for the measured Stage 624 node.')
  }
  return {
...
async function readChipState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible chip text for the measured Stage 624 chip.')
  }
  return {
    alpha: parseColorAlpha(styles.color),
