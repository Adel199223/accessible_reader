...
const outputDir = process.env.RECALL_STAGE438_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE438_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE438_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE438_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE438_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE438_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage438-post-stage437-graph-corner-pods-and-overlay-drawer-audit-failure.png'), {
  force: true,
})
await rm(path.join(outputDir, 'stage438-post-stage437-graph-corner-pods-and-overlay-drawer-audit-failure-reader.png'), {
  force: true,
})
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage438-home-wide-top.png')
  ...
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage438-graph-wide-top.png')
  const graphCornerPodsWideTop = await captureViewportClip(
    desktopPage,
    outputDir,
    'stage438-graph-corner-pods-wide-top.png',
    { x: 92, y: 76, width: 1380, height: 180 },
  )
  const graphPathRailWideTop = await captureViewportClip(
    desktopPage,
    outputDir,
    'stage438-graph-path-rail-wide-top.png',
  ...
  const graphToolsOverlayWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage438-graph-tools-overlay-wide-top.png',
    980,
  )
  ...
  const graphDetailDockPeekWideTop = await captureLocatorTopScreenshot(
    ...
    graphDetailDock,
    outputDir,
    'stage438-graph-detail-dock-peek-wide-top.png',
    980,
    'locator',
  )
  ...
  const graphDetailDockExpandedWideTop = await captureLocatorTopScreenshot(
    ...
    graphDetailDock,
    outputDir,
    'stage438-graph-detail-dock-expanded-wide-top.png',
    980,
    'locator',
  )
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopReaderPage,
    outputDir,
    'stage438-reader-original-wide-top.png',
  )

  await writeFile(
    path.join(outputDir, 'stage438-post-stage437-graph-corner-pods-and-overlay-drawer-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Graph audit judged first after the Stage 437 corner-pods and overlay-drawer reset',
          'supporting Graph crops cover compact corner pods, the overlay tools drawer, the floating path rail, and the inspect drawer in peek and expanded states',
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage438-post-stage437-graph-corner-pods-and-overlay-drawer-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (desktopReaderPage && desktopReaderPage !== desktopPage) {
    await desktopReaderPage
      .screenshot({
        path: path.join(outputDir, 'stage438-post-stage437-graph-corner-pods-and-overlay-drawer-audit-failure-reader.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 438 audit.')
  }
...
async function selectFirstGraphNode(page) {
  ...
  if (bestIndex < 0) {
    throw new Error('Could not find a visible graph node to select during the Stage 438 audit.')
  }
