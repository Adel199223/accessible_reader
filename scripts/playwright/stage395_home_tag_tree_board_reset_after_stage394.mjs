...
const outputDir = process.env.RECALL_STAGE395_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE395_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE395_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE395_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE395_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE395_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage395-home-tag-tree-board-reset-after-stage394-failure.png'), {
  force: true,
})
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage395-home-wide-top.png')
  const homeControlSeamWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage395-home-control-seam-wide-top.png',
    320,
  )
  const homeOrganizerRailWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage395-home-organizer-rail-wide-top.png',
    980,
  )
  const homePrimaryFlowWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage395-home-primary-flow-wide-top.png',
    980,
  )
  const homeSelectedGroupBoardWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage395-home-selected-group-board-wide-top.png',
    920,
    'locator',
  )
  const homePinnedReopenShelfWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage395-home-pinned-reopen-shelf-wide-top.png',
    920,
    'locator',
  )
  ...
  const homeOrganizerRailCollapsedWideTop = await captureLocatorTopScreenshot(
    ...
    await getVisibleLocator(desktopPage, ['[aria-label="Home browse strip"]']),
    outputDir,
    'stage395-home-organizer-rail-collapsed-wide-top.png',
    980,
  )
  ...
  const homeAlternateGroupBoardWideTop = alternateOrganizerGroup
    ? await captureLocatorTopScreenshot(
        ...
        await getVisibleLocator(desktopPage, ['[aria-label="Saved library"]']),
        outputDir,
        'stage395-home-selected-group-board-alternate-wide-top.png',
        920,
        'locator',
      )
    : null
  ...
  await writeFile(
    path.join(outputDir, 'stage395-home-tag-tree-board-reset-after-stage394-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        ...
        validationFocus: [
          'wide desktop Home is judged first after the Stage 394 Reader checkpoint',
          'evidence centers the denser tag-tree rail, the stronger selected-group board, the companion reopen lane, and the reduced dead-center layout',
          'one collapsed-rail capture proves the tree can stay compact without dropping the active group completely',
          'when another organizer group is available, the capture set confirms the left rail is actively driving the main board instead of leaving Home as a static hero composition',
        ],
      },
      null,
      2,
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage395-home-tag-tree-board-reset-after-stage394-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
