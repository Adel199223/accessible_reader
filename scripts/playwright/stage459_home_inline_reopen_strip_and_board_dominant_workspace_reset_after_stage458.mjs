...
const outputDir = process.env.RECALL_STAGE459_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE459_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE459_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE459_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE459_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE459_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage459-home-inline-reopen-strip-and-board-dominant-workspace-reset-after-stage458-failure.png',
  ),
  {
    force: true,
  },
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage459-home-wide-top.png')
  const homeOrganizerRailWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage459-home-organizer-rail-wide-top.png',
    840,
    'locator',
  )
  const homeBoardShellWideTop = await captureLocatorTopScreenshot(
    desktopPage,
    await getVisibleLocator(desktopPage, [
      '.recall-home-primary-board-direct-main-selected-reset',
      '[aria-label="Primary saved source flow"]',
    ]),
    outputDir,
    'stage459-home-board-shell-wide-top.png',
    920,
    'locator',
  )
  const homeInlineReopenStripWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage459-home-inline-reopen-strip-wide-top.png',
    520,
    'locator',
  )
  const homeActiveBoardStateWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage459-home-active-board-state-wide-top.png',
    920,
    'locator',
  )
  ...
  const homeFilteredResultsWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage459-home-filtered-results-wide-top.png',
    920,
    'locator',
  )
  ...
  await writeFile(
    path.join(
      outputDir,
      'stage459-home-inline-reopen-strip-and-board-dominant-workspace-reset-after-stage458-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        runtimeBrowser,
        validationFocus: [
          'wide desktop Home is judged first after the Stage 458 checkpoint',
          'evidence centers an organizer-led rail and a board-dominant right side instead of a board-plus-side-lane split',
          'the pinned reopen continuity now reads as an inline strip inside the active board while grouped and filtered cards start sooner',
        ],
      },
      null,
      2,
