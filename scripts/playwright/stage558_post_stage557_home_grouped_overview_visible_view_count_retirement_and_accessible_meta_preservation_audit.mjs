...
const outputDir = process.env.RECALL_STAGE558_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE558_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE558_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE558_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE558_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE558_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage558-post-stage557-home-grouped-overview-visible-view-count-retirement-and-accessible-meta-preservation-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage558-post-stage557-home-grouped-overview-visible-view-count-retirement-and-accessible-meta-preservation-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeOverviewEvidence = await captureHomeOverviewEvidence(page, outputDir, 'stage558')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage558-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const { sourceTitle } = await openOriginalReaderFromHome(readerPage)
  const readerOriginalWideTop = await captureViewportScreenshot(readerPage, outputDir, 'stage558-reader-original-wide-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage558-post-stage557-home-grouped-overview-visible-view-count-retirement-and-accessible-meta-preservation-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 557 grouped-overview visible metadata reset',
          'the grouped overview should now retire the visible views suffix while keeping useful file-format context in Documents and preserving available-view-count context in accessible labels',
...
async function captureHomeOverviewEvidence(page, directory, stagePrefix) {
  ...
  const overviewCards = await getVisibleLocators(savedLibraryOverview, '.recall-home-library-card-stage557-reset')
  ...
  if (!primaryMetric || secondaryMetrics.length !== 2) {
    throw new Error('Could not measure the primary lane and secondary stack cards for the Stage 558 audit.')
  }
  ...
  for (const metric of cardMetrics) {
    ...
    if (metric.title === 'Captures' && metric.footerButtonHeight == null) {
      throw new Error(
        'The Captures grouped-overview card no longer exposed a measurable Show all footer button for the Stage 558 audit.',
      )
    }
  }
  ...
  if (!overviewBox || !headerBox || !gridBox || !titleRowBox || !titleHeadingBox || !metaBox) {
    throw new Error('Could not measure the grouped-overview shell geometry for the Stage 558 audit.')
  }
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 558 audit.')
  }
  return { sourceTitle }
}
...
async function getCardMetric(card) {
  const [box, firstRowBox, firstTitleBox, rowTitleMinHeight, rowDetailCount, rowOverlineCount, footerButtonBox, rowCount, metaText, metaBox, rowAriaLabel] = await Promise.all([
    card.boundingBox(),
...
async function getSourceTitle(sourceButton) {
  ...
  if (ariaLabel) {
    const match = ariaLabel.match(/^Open (.+?)(?: \(.+\))?(?:, \d+ views? available)?(?: from organizer)?$/i)
    if (match?.[1]) {
      return match[1]
    }
  }
