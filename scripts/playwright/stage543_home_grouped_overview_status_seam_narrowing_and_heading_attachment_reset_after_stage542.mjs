...
const outputDir = process.env.RECALL_STAGE543_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE543_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE543_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE543_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE543_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE543_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage543-home-grouped-overview-status-seam-narrowing-and-heading-attachment-reset-after-stage542-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeOverviewEvidence = await captureHomeOverviewEvidence(page, outputDir, 'stage543')

  await writeFile(
    path.join(
      outputDir,
      'stage543-home-grouped-overview-status-seam-narrowing-and-heading-attachment-reset-after-stage542-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        runtimeBrowser,
        validationFocus: [
          'wide desktop Home keeps the grouped overview open while the status seam narrows and reads more like a smaller attached heading note',
          'the Stage 537 primary-lane and secondary-stack composition should stay intact while the overview shell loses the remaining broad top-cap feel',
          'the seam narrowing should improve the working-board read without widening into selected-group or generated-content Reader work',
        ],
      },
      null,
      2,
...
async function captureHomeOverviewEvidence(page, directory, stagePrefix) {
  ...
  if (!primaryMetric || secondaryMetrics.length !== 2) {
    throw new Error('Could not measure the primary lane and secondary stack cards for the Stage 543 validation.')
  }
  ...
  if (!overviewBox || !headerBox || !gridBox || !titleRowBox || !metaBox) {
    throw new Error('Could not measure the grouped-overview shell geometry for the Stage 543 validation.')
  }
