...
const outputDir = process.env.RECALL_STAGE463_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE463_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE463_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE463_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE463_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE463_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage463-home-organizer-deck-and-results-sheet-reset-after-stage462-failure.png',
  ),
  {
    force: true,
  },
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage463-home-wide-top.png')
  const homeOrganizerDeckWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage463-home-organizer-deck-wide-top.png',
    460,
    'locator',
  )
  const homeOrganizerListStartWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage463-home-organizer-list-start-wide-top.png',
    760,
    'locator',
  )
  const homeInlineReopenStripWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage463-home-inline-reopen-strip-wide-top.png',
    420,
    'locator',
  )
  const homeActiveBoardStateWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage463-home-active-board-state-wide-top.png',
    880,
    'locator',
  )
  ...
  const homeFilteredResultsWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage463-home-filtered-results-wide-top.png',
    880,
    'locator',
  )
  ...
  await writeFile(
    path.join(
      outputDir,
      'stage463-home-organizer-deck-and-results-sheet-reset-after-stage462-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        runtimeBrowser,
        validationFocus: [
          'wide desktop Home is judged first after the Stage 462 checkpoint',
          'evidence centers a flatter organizer deck where search, sort, collapse, and hide sit closer to the group list',
          'the active board and attached reopen strip now begin sooner as one working sheet instead of a staged top stack',
        ],
      },
      null,
      2,
