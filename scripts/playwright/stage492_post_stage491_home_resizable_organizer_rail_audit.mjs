...
const outputDir = process.env.RECALL_STAGE492_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE492_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE492_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE492_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE492_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE492_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage492-post-stage491-home-resizable-organizer-rail-audit-failure.png',
  ),
  {
    force: true,
  },
...
await rm(
  path.join(
    outputDir,
    'stage492-post-stage491-home-resizable-organizer-rail-audit-failure-reader.png',
  ),
  {
    force: true,
  },
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage492-home-wide-top.png')
  ...
  const homeOrganizerDefaultWideTop = await captureLocatorTopScreenshot(
    ...
    browseStrip,
    outputDir,
    'stage492-home-organizer-default-wide-top.png',
    980,
    'locator',
  )
  ...
  const homeOrganizerWideRailWideTop = await captureLocatorTopScreenshot(
    ...
    browseStrip,
    outputDir,
    'stage492-home-organizer-wide-rail-wide-top.png',
    980,
    'locator',
  )
  ...
  const homeOrganizerWideBranchWideTop = await captureLocatorTopScreenshot(
    ...
    group,
    outputDir,
    'stage492-home-organizer-wide-branch-wide-top.png',
    860,
    'locator',
  )
  ...
  const homeCompactControlsWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage492-home-compact-controls-wide-top.png',
    320,
    'locator',
  )
  ...
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage492-graph-wide-top.png')
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopReaderPage,
    outputDir,
    'stage492-reader-original-wide-top.png',
  )

  await writeFile(
    path.join(
      outputDir,
      'stage492-post-stage491-home-resizable-organizer-rail-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 491 organizer-rail resize reset',
          'Home evidence should show a real resizable organizer rail that widens for browse work without displacing the right-side board',
...
async function setHomeOrganizerWidthToMax(page) {
  ...
  await waitForCondition(async () => (await getHomeOrganizerWidth(page.getByRole('complementary', { name: 'Home browse strip' }))) >= 388, 'The Home organizer did not widen to its maximum width during the Stage 492 audit.')
  await page.waitForTimeout(250)
}
...
async function selectHomeBrowseBranchForCapture(page) {
  ...
  if (!groupCount) {
    throw new Error('Could not find any Home organizer groups to capture during the Stage 492 audit.')
  }
  ...
  throw new Error('Could not find an active Home organizer branch to capture during the Stage 492 audit.')
}
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 492 audit.')
  }
