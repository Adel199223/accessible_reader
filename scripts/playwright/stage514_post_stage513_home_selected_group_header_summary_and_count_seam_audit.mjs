...
const outputDir = process.env.RECALL_STAGE514_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE514_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE514_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE514_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE514_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE514_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage514-post-stage513-home-selected-group-header-summary-and-count-seam-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage514-post-stage513-home-selected-group-header-summary-and-count-seam-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeWideTop = path.join(outputDir, 'stage514-home-wide-top.png')
  await desktopPage.screenshot({ path: homeWideTop })

  const homeSelectedGroupHeaderWideTop = path.join(outputDir, 'stage514-home-selected-group-header-wide-top.png')
  await header.screenshot({ path: homeSelectedGroupHeaderWideTop })

  const homeSelectedGroupHeaderSummaryWideTop = path.join(
    outputDir,
    'stage514-home-selected-group-header-summary-wide-top.png',
  )
  await summary.screenshot({ path: homeSelectedGroupHeaderSummaryWideTop })

  const homeSelectedGroupHeaderCountWideTop = path.join(
    outputDir,
    'stage514-home-selected-group-header-count-wide-top.png',
  )
  await count.screenshot({ path: homeSelectedGroupHeaderCountWideTop })

  const homeSelectedGroupWideTop = path.join(outputDir, 'stage514-home-selected-group-wide-top.png')
  await savedLibrarySection.screenshot({ path: homeSelectedGroupWideTop })

  await openGraph(desktopPage)
  const graphWideTop = path.join(outputDir, 'stage514-graph-wide-top.png')
  ...
  const readerOriginalWideTop = path.join(outputDir, 'stage514-reader-original-wide-top.png')
  await desktopReaderPage.screenshot({ path: readerOriginalWideTop })

  await writeFile(
    path.join(
      outputDir,
      'stage514-post-stage513-home-selected-group-header-summary-and-count-seam-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 513 selected-group header summary and count-seam deflation pass',
          'the selected-group header should now start faster while keeping branch identity explicit',
          'the top-right count should now feel more attached to the heading seam while Graph and original-only Reader remain regression surfaces',
        ],
        baseUrl,
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 514 audit.')
  }
  await page.waitForTimeout(250)
  return { sourceTitle }
}
