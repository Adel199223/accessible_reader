...
const outputDir = process.env.RECALL_STAGE613_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE613_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE613_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE613_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE613_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE613_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage613-home-lower-card-vertical-share-and-title-source-chip-rhythm-reset-after-stage612-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeImplementation(page, outputDir, 'stage613')

  await writeFile(
    path.join(
      outputDir,
      'stage613-home-lower-card-vertical-share-and-title-source-chip-rhythm-reset-after-stage612-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        implementationFocus: [
          ...
          'The next polish pass should reduce the remaining March 25, 2026 Recall homepage mismatch by reclaiming a small amount of card height from the preview and returning that space to the lower title/source/chip seam.',
          'Toolbar, rail, grouping, and source-aware poster treatments should stay intact while the lower card rhythm reads slightly more like Recall at page scale.',
        ],
...
async function captureHomeImplementation(page, directory, stagePrefix) {
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 613 card'),
    readMeasuredBox(preview, 'Stage 613 card preview'),
    readMeasuredBox(copy, 'Stage 613 card copy block'),
    readMeasuredBox(title, 'Stage 613 card title'),
    readMeasuredBox(sourceRow, 'Stage 613 card source row'),
    readMeasuredBox(chip, 'Stage 613 card chip'),
  ])
  ...
  if (!(titleToSourceGapPx >= titleToSourceGapPxFloor)) {
    throw new Error(`Expected title-to-source gap to open slightly in Stage 613, found ${titleToSourceGapPx}.`)
  }
...
async function readTextNodeState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible text content for the measured Stage 613 node.')
  }
  return {
...
async function readChipState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible collection-chip text content for the Stage 613 measurement.')
  }
  return {
    alpha: parseColorAlpha(styles.color),
