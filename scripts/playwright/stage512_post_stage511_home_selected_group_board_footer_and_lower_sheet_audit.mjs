...
const outputDir = process.env.RECALL_STAGE512_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE512_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE512_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE512_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE512_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE512_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage512-post-stage511-home-selected-group-board-footer-and-lower-sheet-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage512-post-stage511-home-selected-group-board-footer-and-lower-sheet-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeWideTop = path.join(outputDir, 'stage512-home-wide-top.png')
  await desktopPage.screenshot({ path: homeWideTop })

  const homeSelectedGroupBoardLowerSheetWideTop = path.join(
    outputDir,
    'stage512-home-selected-group-board-lower-sheet-wide-top.png',
  )
  ...
  const homeSelectedGroupBoardFooterWideTop = path.join(
    outputDir,
    'stage512-home-selected-group-board-footer-wide-top.png',
  )
  await footer.screenshot({ path: homeSelectedGroupBoardFooterWideTop })

  const homeSelectedGroupWideTop = path.join(outputDir, 'stage512-home-selected-group-wide-top.png')
  await savedLibrarySection.screenshot({ path: homeSelectedGroupWideTop })

  await openGraph(desktopPage)
  const graphWideTop = path.join(outputDir, 'stage512-graph-wide-top.png')
  ...
  const readerOriginalWideTop = path.join(outputDir, 'stage512-reader-original-wide-top.png')
  await desktopReaderPage.screenshot({ path: readerOriginalWideTop })

  await writeFile(
    path.join(
      outputDir,
      'stage512-post-stage511-home-selected-group-board-footer-and-lower-sheet-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 511 selected-group board-footer and lower-sheet continuation deflation pass',
          'the right-side selected-group board footer should now feel more attached to the results sheet without losing explicit show-all control',
          'the lower edge of the selected-group board should read calmer while Graph and original-only Reader remain regression surfaces',
        ],
        baseUrl,
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 512 audit.')
  }
  await page.waitForTimeout(250)
  return { sourceTitle }
}
