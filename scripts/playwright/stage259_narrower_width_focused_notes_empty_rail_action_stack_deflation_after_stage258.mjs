...
const outputDir = process.env.RECALL_STAGE259_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE259_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE259_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE259_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE259_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE259_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage259-narrower-width-focused-notes-empty-rail-action-stack-deflation-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage259-focused-overview-narrow-top.png',
  )
  ...
  const focusedNotesEmptyNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage259-focused-notes-empty-narrow-top.png',
  )
  ...
  const focusedNotesDrawerOpenNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage259-focused-notes-drawer-open-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureTopScreenshot(page, outputDir, 'stage259-reader-narrow-top.png')

  await writeFile(
    path.join(outputDir, 'stage259-narrower-width-focused-notes-empty-rail-action-stack-deflation-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        captures: {
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage259-narrower-width-focused-notes-empty-rail-action-stack-deflation-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
