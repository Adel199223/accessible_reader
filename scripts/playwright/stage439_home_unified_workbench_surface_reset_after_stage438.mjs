...
const outputDir = process.env.RECALL_STAGE439_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE439_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE439_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE439_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE439_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE439_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage439-home-unified-workbench-surface-reset-after-stage438-failure.png'), {
  force: true,
})
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage439-home-wide-top.png')
  const homeControlWorkbenchBarWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage439-home-control-workbench-bar-wide-top.png',
    180,
  )
  const homeOrganizerDockWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage439-home-organizer-dock-wide-top.png',
    960,
  )
  const homePrimaryWorkbenchWideTop = await captureLocatorTopScreenshot(
    desktopPage,
    await getVisibleLocator(desktopPage, ['[aria-label="Primary saved source flow"]']),
    outputDir,
    'stage439-home-primary-workbench-wide-top.png',
    980,
  )
  const homeReopenDockWideTop = await captureLocatorTopScreenshot(
    desktopPage,
    await getVisibleLocator(desktopPage, ['[aria-label="Pinned reopen shelf"]']),
    outputDir,
    'stage439-home-reopen-dock-wide-top.png',
    500,
    'locator',
  )
  const homeActiveBoardWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage439-home-active-board-wide-top.png',
    760,
    'locator',
  )
  ...
  const homeFilteredWorkbenchWideTop = await captureLocatorTopScreenshot(
    desktopPage,
    await getVisibleLocator(desktopPage, ['[aria-label="Primary saved source flow"]']),
    outputDir,
    'stage439-home-filtered-workbench-wide-top.png',
    980,
  )
  const homeMatchingSourcesWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage439-home-matching-sources-wide-top.png',
    720,
    'locator',
  )
  ...
  const homeOrganizerHiddenWideTop = await captureViewportScreenshot(
    desktopPage,
    outputDir,
    'stage439-home-organizer-hidden-wide-top.png',
  )
  const homeCompactControlDeckWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage439-home-compact-control-deck-wide-top.png',
    320,
    'locator',
  )

  await writeFile(
    path.join(outputDir, 'stage439-home-unified-workbench-surface-reset-after-stage438-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        ...
        captures: {
          ...
          homeFilteredWorkbenchWideTop,
          homeMatchingSourcesWideTop,
          homeOrganizerDockWideTop,
          homeOrganizerHiddenWideTop,
          homePrimaryWorkbenchWideTop,
          homeReopenDockWideTop,
          homeWideTop,
        },
        desktopViewport,
        headless,
        runtimeBrowser,
        validationFocus: [
          'wide desktop Home is judged first after the Stage 438 checkpoint',
          'evidence centers the slimmer workbench bar, the more docked organizer rail, and the flatter single-shell library board',
          'filtered matches stay inside the same Home workbench and the organizer-hidden state remains a fallback without widening into Graph or generated-content Reader work',
        ],
      },
      null,
      2,
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage439-home-unified-workbench-surface-reset-after-stage438-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
