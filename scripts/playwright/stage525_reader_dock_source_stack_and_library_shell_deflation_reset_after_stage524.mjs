...
const outputDir = process.env.RECALL_STAGE525_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE525_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE525_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE525_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE525_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE525_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage525-reader-dock-source-stack-and-library-shell-deflation-reset-after-stage524-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopPage,
    outputDir,
    'stage525-reader-original-wide-top.png',
  )
  const readerDockHeaderWideTop = await captureLocatorTopScreenshot(
    desktopPage,
    await getVisibleLocator(desktopPage, ['.reader-support-dock-header']),
    outputDir,
    'stage525-reader-dock-header-wide-top.png',
    280,
  )
  const readerSourceStackWideTop = await captureLocatorTopScreenshot(
    desktopPage,
    await getVisibleLocator(desktopPage, ['.reader-support-pane-source-original-parity', '.reader-support-pane-source']),
    outputDir,
    'stage525-reader-source-stack-wide-top.png',
    980,
    'locator',
  )
  const readerMetrics = await getReaderDockMetrics(desktopPage)
  ...
  const readerDockNotesWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage525-reader-dock-notes-wide-top.png',
    980,
    'locator',
  )

  await writeFile(
    path.join(outputDir, 'stage525-reader-dock-source-stack-and-library-shell-deflation-reset-after-stage524-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        ...
        sourceTitle,
        validationFocus: [
          'original-only Reader is reset again immediately after the Stage 524 dock audit rather than reopening another surface',
          'the dock header crop should show a flatter title, metadata, and tabs seam inside the original-only companion rail',
          'the source-stack crop should show the active source plus embedded library reading as one calmer support column',
          'the notes-state dock crop confirms note continuity still holds while generated-content Reader work stays fully out of scope',
        ],
      },
      null,
      2,
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 525 validation.')
  }
