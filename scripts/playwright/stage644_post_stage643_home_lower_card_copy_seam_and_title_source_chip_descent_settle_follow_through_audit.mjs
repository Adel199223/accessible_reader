...
const outputDir = process.env.RECALL_STAGE644_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE644_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE644_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE644_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE644_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE644_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const desktopViewport = { width: 1600, height: 1080 }
const stage641PreviewRatioBaseline = 0.5787332053742802
const stage641CopyRatioBaseline = 0.301573896353167
const stage641TitleLineHeightPxBaseline = 19.423
const stage641TitleToSourceGapPxBaseline = 5.296875
const stage641SourceToChipGapPxBaseline = 4.625
...
const previewRatioCeiling = 0.578
const copyRatioFloor = 0.302
const titleLineHeightPxFloor = 19.55
const titleToSourceGapPxFloor = 5.4
const sourceToChipGapPxFloor = 4.74
const sourceFontSizePxFloor = 10.54
const chipFontSizePxFloor = 7.85
const sourceAlphaFloor = 0.76
const chipAlphaFloor = 0.59
...
await rm(
    path.join(
      outputDir,
      'stage644-post-stage643-home-lower-card-copy-seam-and-title-source-chip-descent-settle-follow-through-audit-failure.png',
    ),
  { force: true },
)
await rm(
    path.join(
      outputDir,
      'stage644-post-stage643-home-lower-card-copy-seam-and-title-source-chip-descent-settle-follow-through-audit-failure-reader.png',
    ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeAudit(page, outputDir, 'stage644')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage644-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const readerEvidence = await openOriginalReaderFromHome(readerPage, outputDir, 'stage644')

  await writeFile(
    path.join(
      outputDir,
      'stage644-post-stage643-home-lower-card-copy-seam-and-title-source-chip-descent-settle-follow-through-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 643 lower-card copy-seam and title/source/chip settle follow-through pass',
          'the Home surface should keep the Stage 563 structure while the lower hierarchy reads slightly cleaner than the Stage 641 baseline across title, source row, and collection chip',
          'Graph and original-only Reader remain regression surfaces only after the Home-only Stage 643 pass',
        ],
        baseUrl,
...
async function captureHomeAudit(page, directory, stagePrefix) {
  ...
  const representativeCard = dayGroup.locator('.recall-home-parity-card-stage643').first()
  const preview = representativeCard.locator('.recall-home-parity-card-preview-stage643').first()
  const copy = representativeCard.locator('.recall-home-parity-card-copy-stage643').first()
  const title = representativeCard.locator('.recall-home-parity-card-title-stage643').first()
  const sourceRow = representativeCard.locator('.recall-home-parity-card-source-row-stage643').first()
  const source = representativeCard.locator('.recall-home-parity-card-source-stage643').first()
  const chip = representativeCard.locator('.recall-home-parity-card-chip-stage643').first()
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 644 card'),
    readMeasuredBox(preview, 'Stage 644 card preview'),
    readMeasuredBox(copy, 'Stage 644 card copy block'),
    readMeasuredBox(title, 'Stage 644 card title'),
    readMeasuredBox(sourceRow, 'Stage 644 card source row'),
    readMeasuredBox(chip, 'Stage 644 card chip'),
  ])
  ...
  const [titleState, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 644 Home canvas, day header, and grid.')
      }
  ...
  if (!(previewRatio < stage641PreviewRatioBaseline && previewRatio <= previewRatioCeiling)) {
    throw new Error(`Expected preview ratio below the Stage 641 baseline and within the target band, found ${previewRatio}.`)
  }
  if (!(copyRatio > stage641CopyRatioBaseline && copyRatio >= copyRatioFloor)) {
    throw new Error(`Expected copy ratio above the Stage 641 baseline and within the target band, found ${copyRatio}.`)
  }
  if (!(titleState.lineHeightPx > stage641TitleLineHeightPxBaseline && titleState.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 641 baseline, found ${titleState.lineHeightPx}.`)
  }
  if (!(titleToSourceGapPx > stage641TitleToSourceGapPxBaseline && titleToSourceGapPx >= titleToSourceGapPxFloor)) {
    throw new Error(`Expected title-to-source gap above the Stage 641 baseline, found ${titleToSourceGapPx}.`)
  }
  if (!(sourceToChipGapPx > stage641SourceToChipGapPxBaseline && sourceToChipGapPx >= sourceToChipGapPxFloor)) {
    throw new Error(`Expected source-to-chip gap above the Stage 641 baseline, found ${sourceToChipGapPx}.`)
  }
  if (!(sourceState.fontSizePx >= sourceFontSizePxFloor && sourceState.alpha >= sourceAlphaFloor)) {
    throw new Error(`Expected source-row legibility to stay at or above the Stage 641 band, found ${sourceState.fontSizePx}px and alpha ${sourceState.alpha}.`)
  }
  if (!(chipState.fontSizePx >= chipFontSizePxFloor && chipState.alpha >= chipAlphaFloor)) {
    throw new Error(`Expected chip legibility to stay at or above the Stage 641 band, found ${chipState.fontSizePx}px and alpha ${chipState.alpha}.`)
  }
  ...
  const webCard = canvas.locator('.recall-home-parity-card-stage643').first()
...
async function openOriginalReaderFromHome(page, directory, stagePrefix) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not keep Reader in Original mode for the Stage 644 audit.')
  }
