...
const outputDir = process.env.RECALL_STAGE572_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE572_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE572_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE572_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE572_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE572_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(outputDir, 'stage572-post-stage571-home-rail-header-restraint-and-board-card-meta-flattening-audit-failure.png'),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage572-post-stage571-home-rail-header-restraint-and-board-card-meta-flattening-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeAudit(page, outputDir, 'stage572')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage572-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const readerEvidence = await openOriginalReaderFromHome(readerPage, outputDir, 'stage572')

  await writeFile(
    path.join(
      outputDir,
      'stage572-post-stage571-home-rail-header-restraint-and-board-card-meta-flattening-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 571 rail-header restraint and board-card meta flattening pass',
          'the Home surface should keep the Stage 563 structure while calming the rail header, flattening the active rail child row, and removing the redundant board-card date seam',
          'Graph and original-only Reader remain regression surfaces only after the Home-only Stage 571 pass',
        ],
        baseUrl,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(
          outputDir,
          'stage572-post-stage571-home-rail-header-restraint-and-board-card-meta-flattening-audit-failure.png',
        ),
        fullPage: true,
      })
      .catch(() => undefined)
...
async function openOriginalReaderFromHome(page, directory, stagePrefix) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not keep Reader in Original mode for the Stage 572 audit.')
  }
...
async function ensureSortMenuClosed(toolbar, canvas) {
  ...
  if (await sortPanel.isVisible().catch(() => false)) {
    throw new Error('Could not close the Home sort popover before the Stage 572 capture.')
  }
}
