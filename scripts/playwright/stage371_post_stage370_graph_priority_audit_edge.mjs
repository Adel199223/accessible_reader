...
const outputDir = process.env.RECALL_STAGE371_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE371_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE371_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE371_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE371_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE371_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage371-post-stage370-graph-priority-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage371-home-wide-top.png')
  ...
  await desktopPage.waitForTimeout(350)
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage371-graph-wide-top.png')
  ...
  const graphUtilityRailWideTop = await captureLocatorTopScreenshot(
    ...
    graphUtilityRail,
    outputDir,
    'stage371-graph-utility-rail-wide-top.png',
    980,
  )
  ...
  const graphDetailDockWideTop = await captureLocatorTopScreenshot(
    ...
    graphDock,
    outputDir,
    'stage371-graph-detail-dock-wide-top.png',
    980,
  )
  ...
  const graphEvidenceFlowWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage371-graph-evidence-flow-wide-top.png',
    980,
  )
  ...
  const readerWideTop = await captureViewportScreenshot(desktopReaderPage, outputDir, 'stage371-reader-wide-top.png')
  ...
  await desktopPage.waitForTimeout(350)
  const notesWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage371-notes-wide-top.png')
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage371-focused-overview-narrow-top.png',
  )
  ...
  const focusedGraphNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage371-focused-graph-narrow-top.png',
  )

  await writeFile(
    path.join(outputDir, 'stage371-post-stage370-graph-priority-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'wide-desktop Graph audit judged first after the Stage 370 finish milestone',
          'wide desktop regression captures for Home, Reader, and Notes behind Graph',
          'supporting Graph crops for the lighter utility rail, attached detail tray, and grouped evidence flow, plus focused overview and focused Graph regressions',
          'Study remains parked and excluded from the active queue unless a later explicit unlock happens',
        ],
        baseUrl,
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage371-post-stage370-graph-priority-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (desktopReaderPage && desktopReaderPage !== desktopPage) {
    await desktopReaderPage
      .screenshot({
        path: path.join(outputDir, 'stage371-post-stage370-graph-priority-audit-failure-reader.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (focusedPage && focusedPage !== desktopPage) {
    await focusedPage
      .screenshot({
        path: path.join(outputDir, 'stage371-post-stage370-graph-priority-audit-failure-focused.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
