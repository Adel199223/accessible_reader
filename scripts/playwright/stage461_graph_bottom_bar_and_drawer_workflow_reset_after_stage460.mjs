...
const outputDir = process.env.RECALL_STAGE461_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE461_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE461_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE461_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE461_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE461_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage461-graph-bottom-bar-and-drawer-workflow-reset-after-stage460-failure.png'), {
  force: true,
})
...
try {
  ...
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage461-graph-wide-top.png')
  const graphUtilityCornerWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage461-graph-utility-corner-wide-top.png',
    'locator',
  )
  ...
  const graphSettingsDrawerWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage461-graph-settings-drawer-wide-top.png',
    'locator',
  )
  ...
  const graphUtilityCornerSelectedWideTop = await captureLocatorTopScreenshot(
    desktopPage,
    desktopPage.locator('.recall-graph-browser-control-actions-overlay').first(),
    outputDir,
    'stage461-graph-utility-corner-selected-wide-top.png',
    'locator',
  )
  const graphSettingsDrawerSelectedWideTop = await captureLocatorTopScreenshot(
    ...
    desktopPage.getByRole('complementary', { name: 'Graph settings sidebar' }).first(),
    outputDir,
    'stage461-graph-settings-drawer-selected-wide-top.png',
    'locator',
  )
  const graphBottomRailSelectedWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage461-graph-bottom-rail-selected-wide-top.png',
    'locator',
  )
  ...
  const graphDetailDockPeekWideTop = await captureLocatorTopScreenshot(
    ...
    graphDetailDock,
    outputDir,
    'stage461-graph-detail-dock-peek-wide-top.png',
    'locator',
  )
  ...
  const graphDetailDockExpandedWideTop = await captureLocatorTopScreenshot(
    ...
    graphDetailDock,
    outputDir,
    'stage461-graph-detail-dock-expanded-wide-top.png',
    'locator',
  )

  await writeFile(
    path.join(outputDir, 'stage461-graph-bottom-bar-and-drawer-workflow-reset-after-stage460-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        ...
        captures: {
          ...
          graphUtilityCornerSelectedWideTop,
        ...
        selectedNodeLabel,
        validationFocus: [
          'wide desktop Graph judged first after the Stage 460 checkpoint',
          'top-right utility corner should read slimmer while the bottom rail becomes the primary selected-node and path workspace',
          'settings drawer should keep filters and quick picks without reviving a full pinned-node card because the right detail drawer and bottom rail now own that workflow',
        ],
      },
      null,
      2,
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage461-graph-bottom-bar-and-drawer-workflow-reset-after-stage460-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
...
async function getPreferredGraphNode(page) {
  ...
  if (bestIndex < 0) {
    throw new Error('Could not find a visible graph node during the Stage 461 run.')
  }
