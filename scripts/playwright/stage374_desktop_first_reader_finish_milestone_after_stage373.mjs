...
const outputDir = process.env.RECALL_STAGE374_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE374_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE374_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE374_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE374_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE374_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage374-desktop-first-reader-finish-milestone-after-stage373-failure.png'), {
  force: true,
})
...
try {
  ...
  const {
    ...
    readerWideTop,
  } = await captureWideReaderArtifacts(desktopReaderPage, outputDir, 'stage374')
  ...
  const {
    ...
    readerNarrowTop,
  } = await captureReaderNarrowRegressionArtifacts(focusedReaderPage, outputDir, 'stage374')

  await writeFile(
    path.join(outputDir, 'stage374-desktop-first-reader-finish-milestone-after-stage373-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        ...
        focusedSourceTitle,
        validationFocus: [
          'desktop-first Reader finish milestone validation with a wide Reader top-state plus explicit main-panel and dock/control crops',
          'focused and narrow Reader regression captures covering top-state, overflow-open controls, and full-page reading flow',
          'neighboring Home, Graph, Notes, and Study stay as regression surfaces for the Stage 375 milestone audit while Reader remains the active milestone',
        ],
      },
      null,
      2,
  ...
  if (desktopReaderPage) {
    await desktopReaderPage
      .screenshot({
        path: path.join(outputDir, 'stage374-desktop-first-reader-finish-milestone-after-stage373-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (focusedReaderPage && focusedReaderPage !== desktopReaderPage) {
    await focusedReaderPage
      .screenshot({
        path: path.join(
          outputDir,
          'stage374-desktop-first-reader-finish-milestone-after-stage373-failure-focused.png',
        ),
        fullPage: true,
      })
      .catch(() => undefined)
