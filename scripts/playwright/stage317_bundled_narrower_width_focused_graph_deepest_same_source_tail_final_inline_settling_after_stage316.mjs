...
const outputDir = process.env.RECALL_STAGE317_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE317_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE317_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE317_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE317_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE317_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(
  path.join(
    outputDir,
    'stage317-bundled-narrower-width-focused-graph-deepest-same-source-tail-final-inline-settling-failure.png',
  ),
  {
    force: true,
  },
...
try {
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage317-focused-overview-narrow-top.png',
  )
  ...
  const focusedGraphNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage317-focused-graph-narrow-top.png')
  const focusedGraphNarrowFull = await captureFullPageScreenshot(
    page,
    outputDir,
    'stage317-focused-graph-narrow-full.png',
  )
  ...
  const focusedGraphDeepestSameSourceTailNarrowTop = await captureCurrentViewportScreenshot(
    page,
    outputDir,
    'stage317-focused-graph-deepest-same-source-tail-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage317-reader-narrow-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage317-bundled-narrower-width-focused-graph-deepest-same-source-tail-final-inline-settling-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
