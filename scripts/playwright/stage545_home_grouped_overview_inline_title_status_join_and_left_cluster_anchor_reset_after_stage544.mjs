...
const outputDir = process.env.RECALL_STAGE545_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE545_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE545_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE545_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE545_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE545_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage545-home-grouped-overview-inline-title-status-join-and-left-cluster-anchor-reset-after-stage544-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeOverviewEvidence = await captureHomeOverviewEvidence(page, outputDir, 'stage545')

  await writeFile(
    path.join(
      outputDir,
      'stage545-home-grouped-overview-inline-title-status-join-and-left-cluster-anchor-reset-after-stage544-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        runtimeBrowser,
        validationFocus: [
          'wide desktop Home keeps the grouped overview open while the condensed status note joins the All collections title row instead of floating away as a separate seam artifact',
          ...
          'the title-status join should improve the working-board read without widening into selected-group or generated-content Reader work',
        ],
      },
      null,
      2,
...
async function captureHomeOverviewEvidence(page, directory, stagePrefix) {
  ...
  if (!primaryMetric || secondaryMetrics.length !== 2) {
    throw new Error('Could not measure the primary lane and secondary stack cards for the Stage 545 validation.')
  }
  ...
  if (!overviewBox || !headerBox || !gridBox || !titleRowBox || !titleHeadingBox || !metaBox) {
    throw new Error('Could not measure the grouped-overview shell geometry for the Stage 545 validation.')
  }
