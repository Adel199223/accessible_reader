...
const outputDir = process.env.RECALL_STAGE456_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE456_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE456_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE456_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE456_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE456_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(outputDir, 'stage456-post-stage455-home-direct-board-and-organizer-audit-failure.png'),
  {
    force: true,
  },
)
await rm(
  path.join(outputDir, 'stage456-post-stage455-home-direct-board-and-organizer-audit-failure-reader.png'),
  {
    force: true,
  },
)
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage456-home-wide-top.png')
  const homeOrganizerRailWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage456-home-organizer-rail-wide-top.png',
    840,
    'locator',
  )
  const homeDirectBoardShellWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage456-home-direct-board-shell-wide-top.png',
    920,
    'locator',
  )
  const homeAttachedReopenRailWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage456-home-attached-reopen-rail-wide-top.png',
    620,
    'locator',
  )
  const homeActiveBoardStateWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage456-home-active-board-state-wide-top.png',
    920,
    'locator',
  )
  ...
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage456-graph-wide-top.png')
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopReaderPage,
    outputDir,
    'stage456-reader-original-wide-top.png',
  )

  await writeFile(
    path.join(outputDir, 'stage456-post-stage455-home-direct-board-and-organizer-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 455 direct-board and organizer-hierarchy reset',
          'Home evidence centers an organizer-owned rail, a direct working board, and a compact attached reopen rail instead of a featured shelf stacked above the real board',
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage456-post-stage455-home-direct-board-and-organizer-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (desktopReaderPage && desktopReaderPage !== desktopPage) {
    await desktopReaderPage
      .screenshot({
        path: path.join(
          outputDir,
          'stage456-post-stage455-home-direct-board-and-organizer-audit-failure-reader.png',
        ),
        fullPage: true,
      })
      .catch(() => undefined)
...
async function openSourceWorkspaceFromHome(page) {
  ...
  const sourceButton = await getVisibleLocator(page, [
    '.recall-home-primary-board-direct-rail button[aria-label^="Open "]',
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 456 audit.')
  }
