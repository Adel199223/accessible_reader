...
const requiredOverflowActions = ['Theme']

function matchesActions(actions, expected) {
  return JSON.stringify(actions) === JSON.stringify(expected)
}

function looksLikeNoteCount(label) {
  return typeof label === 'string' && /^\d+ notes?$/.test(label)
}
...
const outputDir = process.env.RECALL_STAGE755_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE755_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE755_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE755_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE755_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE755_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true

await mkdir(outputDir, { recursive: true })
await rm(path.join(outputDir, 'stage755-reader-notebook-trigger-relocation-audit-failure.png'), { force: true })
await rm(path.join(outputDir, 'stage755-reader-notebook-trigger-relocation-regression-failure.png'), { force: true })
...
try {
  ...
  const {
    ...
    defaultReaderSourceStripNoteChipTriggerText,
    ...
    summaryReaderSourceStripNoteChipTriggerText,
    summaryReaderSourceStripNoteChipTriggerVisible,
  ...
  if (!Array.isArray(defaultAvailableModes) || defaultAvailableModes.length === 0) {
    throw new Error('Expected the Stage 755 audit to resolve the default Reader document available modes.')
  }
  ...
  if (!matchesActions(defaultReaderOverflowActionLabels, requiredOverflowActions)) {
    throw new Error(
      `Expected the default Reader overflow to keep only ${requiredOverflowActions.join(', ')}, found ${Array.isArray(defaultReaderOverflowActionLabels) ? defaultReaderOverflowActionLabels.join(', ') : defaultReaderOverflowActionLabels}.`,
    )
  }
  ...
  if (!defaultReaderSourceStripNoteChipTriggerVisible || !looksLikeNoteCount(defaultReaderSourceStripNoteChipTriggerText)) {
    throw new Error(
      `Expected the default Reader source strip to expose a note-count Notebook trigger, found ${defaultReaderSourceStripNoteChipTriggerText}.`,
    )
  }
  ...
  if (!sourceOpenReaderSourceLibraryVisible || !sourceOpenReaderSourceTabsVisible) {
    throw new Error('Expected Source support to keep reopening normally during the Stage 755 audit.')
  }
  if (!notebookOpenWorkbenchVisible) {
    throw new Error('Expected Notebook support to keep reopening normally during the Stage 755 audit.')
  }
  ...
  if (!matchesActions(summaryReaderOverflowActionLabels, requiredOverflowActions)) {
    throw new Error(
      `Expected the Summary Reader overflow to keep only ${requiredOverflowActions.join(', ')}, found ${Array.isArray(summaryReaderOverflowActionLabels) ? summaryReaderOverflowActionLabels.join(', ') : summaryReaderOverflowActionLabels}.`,
    )
  }
  ...
  if (!summaryReaderSourceStripNoteChipTriggerVisible || !looksLikeNoteCount(summaryReaderSourceStripNoteChipTriggerText)) {
    throw new Error(
      `Expected the Summary Reader source strip to expose a note-count Notebook trigger, found ${summaryReaderSourceStripNoteChipTriggerText}.`,
    )
  }
  ...
  if (!summaryReaderGeneratedEmptyStateVisible || !summaryReaderCreateSummaryVisible) {
    throw new Error('Expected Summary to keep the generated empty state contract in the live Stage 755 audit.')
  }

  await writeFile(
    path.join(outputDir, 'stage755-post-stage754-reader-notebook-trigger-relocation-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'Reader should retire the compact overflow Notebook action and keep the overflow focused on Theme plus read-aloud controls.',
          'The source-strip note chip should become the nearby Notebook trigger without changing the visible note-count seam.',
          'Source and Notebook support should still reopen normally inside the expanded Reader support dock after the note-chip handoff.',
          'Theme should still expose only Light and Dark, and the wider Recall regression surfaces should stay stable in the same live browser pass.',
        ],
        baseUrl,
  ...
  if (readerPage) {
    await readerPage
      .screenshot({
        fullPage: true,
        path: path.join(outputDir, 'stage755-reader-notebook-trigger-relocation-audit-failure.png'),
      })
      .catch(() => undefined)
  }
  if (regressionPage && regressionPage !== readerPage) {
    await regressionPage
      .screenshot({
        fullPage: true,
        path: path.join(outputDir, 'stage755-reader-notebook-trigger-relocation-regression-failure.png'),
      })
      .catch(() => undefined)
  }
  throw error
