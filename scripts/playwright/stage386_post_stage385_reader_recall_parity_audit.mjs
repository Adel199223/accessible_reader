...
const outputDir = process.env.RECALL_STAGE386_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE386_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE386_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE386_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE386_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE386_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage386-post-stage385-reader-recall-parity-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage386-home-wide-top.png')
  ...
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage386-graph-wide-top.png')
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopReaderPage,
    outputDir,
    'stage386-reader-original-wide-top.png',
  )
  const readerControlSeamWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage386-reader-control-seam-wide-top.png',
    380,
  )
  const readerArticleLaneWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage386-reader-article-lane-wide-top.png',
    980,
  )
  const readerSupportDockWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage386-reader-support-dock-wide-top.png',
    980,
  )
  ...
  const readerOriginalNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage386-reader-original-narrow-top.png',
  )

  await writeFile(
    path.join(outputDir, 'stage386-post-stage385-reader-recall-parity-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'original-only Reader is judged first on wide desktop after the Stage 385 parity reset',
          'Reader-specific crops cover the compressed top seam, dominant article lane, and attached support dock',
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage386-post-stage385-reader-recall-parity-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (desktopReaderPage && desktopReaderPage !== desktopPage) {
    await desktopReaderPage
      .screenshot({
        path: path.join(outputDir, 'stage386-post-stage385-reader-recall-parity-audit-failure-reader.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (focusedPage && focusedPage !== desktopPage) {
    await focusedPage
      .screenshot({
        path: path.join(outputDir, 'stage386-post-stage385-reader-recall-parity-audit-failure-focused.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 386 audit.')
  }
