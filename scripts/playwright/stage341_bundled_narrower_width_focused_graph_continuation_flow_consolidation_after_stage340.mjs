...
const outputDir = process.env.RECALL_STAGE341_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE341_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE341_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE341_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE341_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE341_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(
  path.join(
    outputDir,
    'stage341-bundled-narrower-width-focused-graph-continuation-flow-consolidation-after-stage340-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage341-focused-overview-narrow-top.png',
  )
  ...
  const focusedGraphNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage341-focused-graph-narrow-top.png')
  ...
  const focusedGraphNodeDetailRailNarrowTop = await captureLocatorTopScreenshot(
    ...
    graphFocusedDetail,
    outputDir,
    'stage341-focused-graph-node-detail-rail-narrow-top.png',
    920,
  )
  const focusedGraphLeadingEvidenceMetaDensityNarrowPanelTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage341-focused-graph-leading-evidence-meta-density-narrow-panel-top.png',
    500,
  )
  const focusedGraphContinuationFlowBundleNarrowPanelTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage341-focused-graph-continuation-flow-bundle-narrow-panel-top.png',
    760,
  )
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage341-focused-study-narrow-top.png')
  ...
  const focusedStudyAnswerShownNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage341-focused-study-answer-shown-narrow-top.png',
  )
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage341-focused-notes-drawer-open-empty-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage341-reader-narrow-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage341-bundled-narrower-width-focused-graph-continuation-flow-consolidation-after-stage340-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
