...
const outputDir = process.env.RECALL_STAGE360_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE360_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE360_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE360_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE360_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE360_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage360-post-stage359-reader-milestone-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const desktopHomeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage360-home-wide-top.png')
  ...
  await desktopPage.waitForTimeout(350)
  const desktopGraphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage360-graph-wide-top.png')
  ...
  const {
    ...
    readerWideTop,
  } = await captureWideReaderArtifacts(desktopReaderPage, outputDir, 'stage360')
  ...
  await desktopPage.waitForTimeout(350)
  const desktopNotesWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage360-notes-wide-top.png')
  ...
  await desktopPage.waitForTimeout(350)
  const desktopStudyWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage360-study-wide-top.png')
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage360-focused-overview-narrow-top.png',
  )
  ...
  const focusedGraphNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage360-focused-graph-narrow-top.png',
  )
  ...
  const focusedNotesNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage360-focused-notes-narrow-top.png',
  )
  ...
  const focusedStudyNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage360-focused-study-narrow-top.png',
  )
  ...
  const {
    ...
    readerNarrowTop,
  } = await captureReaderNarrowRegressionArtifacts(focusedPage, outputDir, 'stage360')

  await writeFile(
    path.join(outputDir, 'stage360-post-stage359-reader-milestone-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'full Reader milestone audit with wide desktop Home, Graph, Reader, Notes, and Study captures in that order',
          'Reader-specific wide desktop milestone crops for the main panel and reading-context panel while Home, Graph, Notes, and Study stay regression-only',
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage360-post-stage359-reader-milestone-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (desktopReaderPage && desktopReaderPage !== desktopPage) {
    await desktopReaderPage
      .screenshot({
        path: path.join(outputDir, 'stage360-post-stage359-reader-milestone-audit-failure-reader.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (focusedPage && focusedPage !== desktopPage) {
    await focusedPage
      .screenshot({
        path: path.join(outputDir, 'stage360-post-stage359-reader-milestone-audit-failure-focused.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
