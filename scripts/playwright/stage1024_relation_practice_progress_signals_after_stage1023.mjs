...
const outputDir = process.env.RECALL_STAGE1024_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE1024_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE1024_BASE_URL ?? readCliOption('base-url') ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE1024_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE1024_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE1024_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true

await mkdir(outputDir, { recursive: true })
await rm(path.join(outputDir, 'stage1024-relation-practice-progress-signals-failure.png'), { force: true })
...
let harness = null
try {
  harness = await createRelationPracticeReviewHarness({ baseUrl, stageLabel: 'Stage 1024' })
  ...
  const metrics = {
    ...
    cleanupUtilityDryRunMatchedAfterStage1024: null,
  ...
  captures.homePracticeConnection = await captureLocatorScreenshot(
    ...
    homeQueueRow,
    outputDir,
    'stage1024-home-practice-connection.png',
  )
  ...
  captures.collectionPracticeConnection = await captureLocatorScreenshot(
    ...
    collectionQueueRow,
    outputDir,
    'stage1024-collection-practice-connection.png',
  )
  ...
  captures.homePracticeActiveSession = await captureViewportScreenshot(
    page,
    outputDir,
    'stage1024-home-practice-active-session.png',
  )
  ...
  captures.relationPracticeRecap = await captureViewportScreenshot(
    page,
    outputDir,
    'stage1024-relation-practice-recap.png',
  )
  ...
  captures.relationPracticeReturnToSource = await captureLocatorScreenshot(
    ...
    returnedRelationRow,
    outputDir,
    'stage1024-relation-practice-return-to-source.png',
  )
  ...
  captures.homePracticedScheduledState = await captureLocatorScreenshot(
    ...
    outputDir,
    'stage1024-home-practiced-scheduled-state.png',
  )
  ...
  captures.sourcePracticeRelation = await captureLocatorScreenshot(
    ...
    sourceRelationRow,
    outputDir,
    'stage1024-source-practice-relation.png',
  )
  ...
  captures.sourcePracticeActiveSession = await captureViewportScreenshot(
    page,
    outputDir,
    'stage1024-source-practice-active-session.png',
  )

  await cleanupRelationPracticeReviewHarness({ baseUrl, harness })
  harness = null
  const cleanupDryRun = await findRecallNoteAuditArtifacts({ baseUrl })
  metrics.cleanupUtilityDryRunMatchedAfterStage1024 = cleanupDryRun.matchedCount

  for (const [metricName, expected] of Object.entries({
    cleanupUtilityDryRunMatchedAfterStage1024: 0,
    ...
    if (metrics[metricName] !== expected) {
      throw new Error(
        `Stage 1024 expected ${metricName}: ${expected}, got ${metrics[metricName]}. Metrics: ${JSON.stringify(
          metrics,
        )}`,
      )
    }
  ...
  const validation = {
    ...
    metrics,
    runtimeBrowser,
    stage: 'stage1024-relation-practice-progress-signals-after-stage1023',
  }
  await writeFile(
    path.join(outputDir, 'stage1024-relation-practice-progress-signals-validation.json'),
  ...
  await captureViewportScreenshot(
    page,
    outputDir,
    'stage1024-relation-practice-progress-signals-failure.png',
  ).catch(() => null)
  throw error
} finally {
  await cleanupRelationPracticeReviewHarness({ baseUrl, harness }).catch((error) => {
    console.error(`Stage 1024 cleanup failed: ${error instanceof Error ? error.message : String(error)}`)
  })
...
async function createRelationPracticeReviewHarness({ baseUrl, stageLabel }) {
  ...
  const collectionId = `collection:stage1024-relation-review-${timestamp}`
...
async function waitForHarnessRelatedEdges({ baseUrl, harness, timeoutMs = 30000 }) {
  ...
  throw new Error(
    `Timed out waiting for Stage 1024 related edges ${harness.sourceLabel} -> ${harness.targetLabel}: ${JSON.stringify(
      lastSnapshot,
    )}`,
  )
}
...
async function insertHarnessRelationPracticeCard({ edge, harness, prompt, suffix }) {
  ...
  const card = {
    ...
    id: `card:stage1024-relation-review:${suffix}:${edge.id.replace(/[^a-zA-Z0-9:_-]/g, '-')}:${Date.now()}`,
    prompt,
    scheduling_state: {
