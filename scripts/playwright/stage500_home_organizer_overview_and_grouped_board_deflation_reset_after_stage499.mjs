...
const outputDir = process.env.RECALL_STAGE500_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE500_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE500_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE500_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE500_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE500_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage500-home-organizer-overview-and-grouped-board-deflation-reset-after-stage499-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeSelectedGroupWideTop = path.join(outputDir, 'stage500-home-selected-group-wide-top.png')
  await desktopPage.screenshot({ path: homeSelectedGroupWideTop })

  await writeFile(
    path.join(
      outputDir,
      'stage500-home-organizer-overview-and-grouped-board-deflation-reset-after-stage499-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        selectedGroupLabel,
        validationFocus: [
          'wide desktop Home should now expose a clearer organizer-owned overview/reset row instead of a heavier helper paragraph',
          'the grouped overview board should read like a calmer working-set summary with lighter grouped cards above the fold',
