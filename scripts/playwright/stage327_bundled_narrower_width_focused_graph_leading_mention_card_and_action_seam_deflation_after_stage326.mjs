...
const outputDir = process.env.RECALL_STAGE327_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE327_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE327_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE327_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE327_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE327_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(
  path.join(
    outputDir,
    'stage327-bundled-narrower-width-focused-graph-leading-mention-card-and-action-seam-deflation-after-stage326-failure.png',
  ),
  {
    force: true,
  },
...
try {
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage327-focused-overview-narrow-top.png',
  )
  ...
  const focusedGraphNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage327-focused-graph-narrow-top.png')
  const focusedGraphNarrowFull = await captureFullPageScreenshot(
    page,
    outputDir,
    'stage327-focused-graph-narrow-full.png',
  )

  const leadingMentionCardActionSeam = page.locator('.recall-graph-focused-mention-card-leading').first()
  await leadingMentionCardActionSeam.waitFor({ state: 'visible', timeout: 20000 })
  await leadingMentionCardActionSeam.scrollIntoViewIfNeeded()
  await page.waitForTimeout(350)
  const focusedGraphLeadingMentionCardActionSeamNarrowTop = await captureCurrentViewportScreenshot(
    page,
    outputDir,
    'stage327-focused-graph-leading-mention-card-action-seam-narrow-top.png',
  )
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage327-focused-notes-drawer-open-empty-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage327-focused-study-narrow-top.png')
  ...
  const focusedStudyAnswerShownNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage327-focused-study-answer-shown-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage327-reader-narrow-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage327-bundled-narrower-width-focused-graph-leading-mention-card-and-action-seam-deflation-after-stage326-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        captures: {
          focusedGraphLeadingMentionCardActionSeamNarrowTop,
