...
const outputDir = process.env.RECALL_STAGE440_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE440_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE440_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE440_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE440_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE440_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage440-post-stage439-home-unified-workbench-audit-failure.png'), {
  force: true,
})
await rm(path.join(outputDir, 'stage440-post-stage439-home-unified-workbench-audit-failure-reader.png'), {
  force: true,
})
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage440-home-wide-top.png')
  const homeOrganizerDockWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage440-home-organizer-dock-wide-top.png',
    960,
  )
  const homePrimaryWorkbenchWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage440-home-primary-workbench-wide-top.png',
    980,
  )
  const homeReopenDockWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage440-home-reopen-dock-wide-top.png',
    500,
    'locator',
  )
  const homeActiveBoardWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage440-home-active-board-wide-top.png',
    760,
    'locator',
  )
  ...
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage440-graph-wide-top.png')
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopReaderPage,
    outputDir,
    'stage440-reader-original-wide-top.png',
  )

  await writeFile(
    path.join(outputDir, 'stage440-post-stage439-home-unified-workbench-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 439 unified workbench reset',
          'Home evidence centers the slimmer workbench bar, the docked organizer rail, and the flatter single-shell library board',
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage440-post-stage439-home-unified-workbench-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (desktopReaderPage && desktopReaderPage !== desktopPage) {
    await desktopReaderPage
      .screenshot({
        path: path.join(outputDir, 'stage440-post-stage439-home-unified-workbench-audit-failure-reader.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 440 audit.')
  }
