...
const outputDir = process.env.RECALL_STAGE531_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE531_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE531_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE531_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE531_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE531_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage531-reader-dock-tray-and-glance-note-retirement-deflation-reset-after-stage530-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopPage,
    outputDir,
    'stage531-reader-original-wide-top.png',
  )
  const readerDeckJoinWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage531-reader-deck-join-wide-top.png',
    430,
  )
  const readerArticleLeadWideTop = await captureLocatorTopScreenshot(
    desktopPage,
    await getVisibleLocator(desktopPage, ['.reader-article-shell-original-parity', '.reader-article-shell']),
    outputDir,
    'stage531-reader-article-lead-wide-top.png',
    430,
  )
  const readerDockTrayWideTop = await captureLocatorTopScreenshot(
    desktopPage,
    await getVisibleLocator(desktopPage, ['.reader-support-dock-original-parity', '.reader-support-dock']),
    outputDir,
    'stage531-reader-dock-tray-wide-top.png',
    460,
  )
  const readerMetrics = await getReaderSupportTrayMetrics(desktopPage)

  await writeFile(
    path.join(
      outputDir,
      'stage531-reader-dock-tray-and-glance-note-retirement-deflation-reset-after-stage530-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        sourceTitle,
        validationFocus: [
          'original-only Reader is reset again immediately after the Stage 530 audit instead of hopping to a new surface',
          'the default original-only support treatment should demote the dock into a calmer attached tray instead of a full-height article-matched rail',
          'the wide-top, deck-join, article-lead, and dock-tray captures should show the duplicated glance note retired by default while the tray stays attached',
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 531 validation.')
  }
...
async function getReaderSupportTrayMetrics(page) {
  return page.evaluate(() => {
    ...
    const stageGlance = rect('.reader-stage-glance-bar-original-parity', '.reader-stage-glance-bar')
    ...
    const stageGlanceNote = getElement('.reader-stage-glance-bar-original-parity .reader-stage-glance-note')
    ...
    return {
      ...
      dockToArticleHeightRatio: dock && article ? round(dock.height / article.height) : null,
      ...
      stageGlanceNoteVisible:
        stageGlanceNote instanceof HTMLElement ? round(stageGlanceNote.getBoundingClientRect().height) > 0 : false,
      stageGlanceRect: stageGlance,
    }
  })
}
