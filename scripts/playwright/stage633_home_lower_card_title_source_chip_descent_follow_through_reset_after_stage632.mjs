...
const outputDir = process.env.RECALL_STAGE633_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE633_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE633_BASE_URL ?? 'http://172.27.18.251:8002'
const headless = process.env.RECALL_STAGE633_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE633_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE633_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
    path.join(
      outputDir,
      'stage633-home-lower-card-title-source-chip-descent-follow-through-reset-after-stage632-failure.png',
    ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeImplementation(page, outputDir, 'stage633')

  await writeFile(
    path.join(
      outputDir,
      'stage633-home-lower-card-title-source-chip-descent-follow-through-reset-after-stage632-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        implementationFocus: [
          ...
          'The next polish pass should reduce the remaining March 25, 2026 Recall homepage mismatch by giving the lower title/source/chip descent one subtler follow-through.',
          'Shell, rail, toolbar, column cadence, and source-aware poster variants should stay intact while the lower copy stack gains one steadier descending finish.',
        ],
...
async function captureHomeImplementation(page, directory, stagePrefix) {
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 633 card'),
    readMeasuredBox(preview, 'Stage 633 card preview'),
    readMeasuredBox(copy, 'Stage 633 card copy block'),
    readMeasuredBox(title, 'Stage 633 card title'),
    readMeasuredBox(sourceRow, 'Stage 633 card source row'),
    readMeasuredBox(chip, 'Stage 633 card chip'),
  ])
  ...
  const [titleStyles, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 633 Home canvas, day header, and grid.')
      }
  ...
  if (!(titleStyles.lineHeightPx > stage631TitleLineHeightPxBaseline && titleStyles.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 631 baseline, found ${titleStyles.lineHeightPx}.`)
  }
...
async function readTextNodeState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible text content for the measured Stage 633 node.')
  }
  return {
...
async function readChipState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible chip text for the measured Stage 633 chip.')
  }
  return {
    alpha: parseColorAlpha(styles.color),
