...
const outputDir = process.env.RECALL_STAGE458_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE458_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE458_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE458_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE458_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE458_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage458-post-stage457-graph-node-language-and-hover-hierarchy-audit-failure.png'), {
  force: true,
})
await rm(path.join(outputDir, 'stage458-post-stage457-graph-node-language-and-hover-hierarchy-audit-failure-reader.png'), {
  force: true,
})
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage458-home-wide-top.png')
  ...
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage458-graph-wide-top.png')
  const graphCanvasWideTop = await captureLocatorTopScreenshot(
    desktopPage,
    desktopPage.locator('.recall-graph-canvas').first(),
    outputDir,
    'stage458-graph-canvas-wide-top.png',
    'locator',
  )
  ...
  const graphHoverPreviewWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage458-graph-hover-preview-wide-top.png',
    'locator',
  )
  ...
  const graphSelectedCanvasWideTop = await captureLocatorTopScreenshot(
    ...
    desktopPage.locator('.recall-graph-canvas').first(),
    outputDir,
    'stage458-graph-selected-canvas-wide-top.png',
    'locator',
  )
  const graphWorkingTrailWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage458-graph-working-trail-wide-top.png',
    'locator',
  )
  ...
  const graphDetailDockPeekWideTop = await captureLocatorTopScreenshot(
    ...
    graphDetailDock,
    outputDir,
    'stage458-graph-detail-dock-peek-wide-top.png',
    'locator',
  )
  ...
  const graphDetailDockExpandedWideTop = await captureLocatorTopScreenshot(
    ...
    graphDetailDock,
    outputDir,
    'stage458-graph-detail-dock-expanded-wide-top.png',
    'locator',
  )
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopReaderPage,
    outputDir,
    'stage458-reader-original-wide-top.png',
  )

  await writeFile(
    path.join(outputDir, 'stage458-post-stage457-graph-node-language-and-hover-hierarchy-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Graph audit judged first after the Stage 457 node-language and hover-hierarchy reset',
          'supporting Graph crops cover the calmer at-rest canvas, the hovered node preview, the selected-node canvas state, the bottom working trail, and peek plus expanded inspect drawer states',
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage458-post-stage457-graph-node-language-and-hover-hierarchy-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (desktopReaderPage && desktopReaderPage !== desktopPage) {
    await desktopReaderPage
      .screenshot({
        path: path.join(outputDir, 'stage458-post-stage457-graph-node-language-and-hover-hierarchy-audit-failure-reader.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 458 audit.')
  }
...
async function getPreferredGraphNode(page) {
  ...
  if (bestIndex < 0) {
    throw new Error('Could not find a visible graph node during the Stage 458 audit.')
  }
