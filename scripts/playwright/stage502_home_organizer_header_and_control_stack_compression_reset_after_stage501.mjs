...
const outputDir = process.env.RECALL_STAGE502_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE502_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE502_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE502_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE502_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE502_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage502-home-organizer-header-and-control-stack-compression-reset-after-stage501-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeSelectedGroupWideTop = path.join(outputDir, 'stage502-home-selected-group-wide-top.png')
  await desktopPage.screenshot({ path: homeSelectedGroupWideTop })

  await writeFile(
    path.join(
      outputDir,
      'stage502-home-organizer-header-and-control-stack-compression-reset-after-stage501-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        selectedGroupLabel,
        validationFocus: [
          'wide desktop Home should now compress the organizer header and helper line into a lighter utility-style upper rail',
          'search plus organizer actions should sit in a tighter upper stack while the control deck reads denser and less like stacked form sections',
          'drilling into one organizer branch should still hand off cleanly into the selected-group board without reopening detached shelf layouts',
        ],
      },
      null,
      2,
