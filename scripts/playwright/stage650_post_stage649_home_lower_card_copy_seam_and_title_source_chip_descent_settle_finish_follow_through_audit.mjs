...
const outputDir = process.env.RECALL_STAGE650_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE650_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE650_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE650_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE650_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE650_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const desktopViewport = { width: 1600, height: 1080 }
const stage647PreviewRatioBaseline = 0.563531669865643
const stage647CopyRatioBaseline = 0.31393474088291745
const stage647TitleLineHeightPxBaseline = 20.2546
const stage647TitleToSourceGapPxBaseline = 5.96875
const stage647SourceToChipGapPxBaseline = 5.296875
...
const previewRatioCeiling = 0.563
const copyRatioFloor = 0.314
const titleLineHeightPxFloor = 20.32
const titleToSourceGapPxFloor = 6.1
const sourceToChipGapPxFloor = 5.42
const sourceFontSizePxFloor = 10.6
const chipFontSizePxFloor = 7.92
const sourceAlphaFloor = 0.79
const chipAlphaFloor = 0.62
...
await rm(
  path.join(
    outputDir,
    'stage650-post-stage649-home-lower-card-copy-seam-and-title-source-chip-descent-settle-finish-follow-through-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage650-post-stage649-home-lower-card-copy-seam-and-title-source-chip-descent-settle-finish-follow-through-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeAudit(page, outputDir, 'stage650')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage650-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const readerEvidence = await openOriginalReaderFromHome(readerPage, outputDir, 'stage650')

  await writeFile(
    path.join(
      outputDir,
      'stage650-post-stage649-home-lower-card-copy-seam-and-title-source-chip-descent-settle-finish-follow-through-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 649 lower-card copy-seam and title/source/chip settle finish follow-through pass',
          'the Home surface should keep the Stage 563 structure while the lower hierarchy reads slightly cleaner than the Stage 647 follow-through baseline across title, source row, and collection chip',
          'Graph and original-only Reader remain regression surfaces only after the Home-only Stage 649 pass',
        ],
        baseUrl,
...
async function captureHomeAudit(page, directory, stagePrefix) {
  ...
  const representativeCard = dayGroup.locator('.recall-home-parity-card-stage649').first()
  const preview = representativeCard.locator('.recall-home-parity-card-preview-stage649').first()
  const copy = representativeCard.locator('.recall-home-parity-card-copy-stage649').first()
  const title = representativeCard.locator('.recall-home-parity-card-title-stage649').first()
  const sourceRow = representativeCard.locator('.recall-home-parity-card-source-row-stage649').first()
  const source = representativeCard.locator('.recall-home-parity-card-source-stage649').first()
  const chip = representativeCard.locator('.recall-home-parity-card-chip-stage649').first()
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 650 card'),
    readMeasuredBox(preview, 'Stage 650 card preview'),
    readMeasuredBox(copy, 'Stage 650 card copy block'),
    readMeasuredBox(title, 'Stage 650 card title'),
    readMeasuredBox(sourceRow, 'Stage 650 card source row'),
    readMeasuredBox(chip, 'Stage 650 card chip'),
  ])
  ...
  const [titleState, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 650 Home canvas, day header, and grid.')
      }
  ...
  if (!(previewRatio < stage647PreviewRatioBaseline && previewRatio <= previewRatioCeiling)) {
    throw new Error(`Expected preview ratio below the Stage 647 baseline and within the target band, found ${previewRatio}.`)
  }
  if (!(copyRatio > stage647CopyRatioBaseline && copyRatio >= copyRatioFloor)) {
    throw new Error(`Expected copy ratio above the Stage 647 baseline and within the target band, found ${copyRatio}.`)
  }
  if (!(titleState.lineHeightPx > stage647TitleLineHeightPxBaseline && titleState.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 647 baseline, found ${titleState.lineHeightPx}.`)
  }
  if (!(titleToSourceGapPx > stage647TitleToSourceGapPxBaseline && titleToSourceGapPx >= titleToSourceGapPxFloor)) {
    throw new Error(`Expected title-to-source gap above the Stage 647 baseline, found ${titleToSourceGapPx}.`)
  }
  if (!(sourceToChipGapPx > stage647SourceToChipGapPxBaseline && sourceToChipGapPx >= sourceToChipGapPxFloor)) {
    throw new Error(`Expected source-to-chip gap above the Stage 647 baseline, found ${sourceToChipGapPx}.`)
  }
  if (!(sourceState.fontSizePx >= sourceFontSizePxFloor && sourceState.alpha >= sourceAlphaFloor)) {
    throw new Error(`Expected source-row legibility to stay at or above the Stage 647 band, found ${sourceState.fontSizePx}px and alpha ${sourceState.alpha}.`)
  }
  if (!(chipState.fontSizePx >= chipFontSizePxFloor && chipState.alpha >= chipAlphaFloor)) {
    throw new Error(`Expected chip legibility to stay at or above the Stage 647 band, found ${chipState.fontSizePx}px and alpha ${chipState.alpha}.`)
  }
  ...
  const webCard = canvas.locator('.recall-home-parity-card-stage649').first()
...
async function openOriginalReaderFromHome(page, directory, stagePrefix) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not keep Reader in Original mode for the Stage 650 audit.')
  }
...
async function readTextNodeState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible text content for the measured Stage 650 node.')
  }
  return {
...
async function readChipState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible chip text for the measured Stage 650 chip.')
  }
  return {
    alpha: parseColorAlpha(styles.color),
