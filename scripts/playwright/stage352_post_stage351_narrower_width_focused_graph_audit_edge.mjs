...
const outputDir = process.env.RECALL_STAGE352_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE352_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE352_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE352_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE352_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE352_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage352-post-stage351-narrower-width-focused-graph-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage352-focused-overview-narrow-top.png',
  )
  ...
  const focusedGraphNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage352-focused-graph-narrow-top.png')
  ...
  const focusedGraphNodeDetailRailNarrowTop = await captureLocatorTopScreenshot(
    ...
    graphFocusedDetail,
    outputDir,
    'stage352-focused-graph-node-detail-rail-narrow-top.png',
    920,
  )
  const focusedGraphRightRailReadabilityNarrowPanelTop = await captureLocatorTopScreenshot(
    ...
    graphFocusedBody,
    outputDir,
    'stage352-focused-graph-right-rail-readability-narrow-panel-top.png',
    840,
  )
  const focusedGraphLeadingEvidenceMetaDensityNarrowPanelTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage352-focused-graph-leading-evidence-meta-density-narrow-panel-top.png',
    500,
  )
  const focusedGraphFollowOnBundleNarrowPanelTop = await captureLocatorTopScreenshot(
    ...
    followOnBundle,
    outputDir,
    'stage352-focused-graph-follow-on-bundle-narrow-panel-top.png',
    760,
  )
  const focusedGraphRelationsFollowOnNarrowPanelTop =
    (await relationsFollowOn.count()) > 0
      ? await captureLocatorTopScreenshot(
          ...
          relationsFollowOn,
          outputDir,
          'stage352-focused-graph-relations-follow-on-narrow-panel-top.png',
          760,
        )
      : null
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage352-focused-study-narrow-top.png')
  ...
  const focusedStudyAnswerShownNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage352-focused-study-answer-shown-narrow-top.png',
  )
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage352-focused-notes-drawer-open-empty-narrow-top.png',
  )
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage352-post-stage351-narrower-width-focused-graph-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
