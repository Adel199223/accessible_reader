...
const outputDir = process.env.RECALL_STAGE323_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE323_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE323_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE323_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE323_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE323_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(
  path.join(
    outputDir,
    'stage323-bundled-narrower-width-focused-study-prompt-reveal-and-support-stack-fusion-failure.png',
  ),
  {
    force: true,
  },
...
try {
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage323-focused-overview-narrow-top.png',
  )
  ...
  const focusedStudyNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage323-focused-study-narrow-top.png')
  ...
  const focusedStudyAnswerShownNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage323-focused-study-answer-shown-narrow-top.png',
  )
  const focusedStudyAnswerShownNarrowFull = await captureFullPageScreenshot(
    page,
    outputDir,
    'stage323-focused-study-answer-shown-narrow-full.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage323-reader-narrow-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage323-bundled-narrower-width-focused-study-prompt-reveal-and-support-stack-fusion-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
