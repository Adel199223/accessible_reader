...
const outputDir = process.env.RECALL_STAGE622_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE622_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE622_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE622_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE622_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE622_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const desktopViewport = { width: 1600, height: 1080 }
const stage614PreviewRatioBaseline = 0.6415
const stage614CopyRatioBaseline = 0.2505
const stage614TitleLineHeightPxBaseline = 16.008
const stage614TitleToSourceGapPxBaseline = 2.55
const stage614SourceToChipGapPxBaseline = 2.08
...
const previewRatioCeiling = 0.638
const copyRatioFloor = 0.255
const titleLineHeightPxFloor = 16.2
const titleToSourceGapPxFloor = 2.7
const sourceToChipGapPxFloor = 2.2
...
await rm(
  path.join(
    outputDir,
    'stage622-post-stage621-home-lower-card-share-and-title-source-chip-follow-through-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage622-post-stage621-home-lower-card-share-and-title-source-chip-follow-through-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeAudit(page, outputDir, 'stage622')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage622-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const readerEvidence = await openOriginalReaderFromHome(readerPage, outputDir, 'stage622')

  await writeFile(
    path.join(
      outputDir,
      'stage622-post-stage621-home-lower-card-share-and-title-source-chip-follow-through-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 621 lower-card share follow-through pass',
          'the Home surface should keep the Stage 563 structure while the lower title/source/chip seam owns slightly more rhythm than the older Stage 620 poster-led balance',
          'Graph and original-only Reader remain regression surfaces only after the Home-only Stage 621 pass',
        ],
        baseUrl,
...
async function captureHomeAudit(page, directory, stagePrefix) {
  ...
  const representativeCard = dayGroup.locator('.recall-home-parity-card-stage621').first()
  const preview = representativeCard.locator('.recall-home-parity-card-preview-stage621').first()
  const copy = representativeCard.locator('.recall-home-parity-card-copy-stage621').first()
  const title = representativeCard.locator('.recall-home-parity-card-title-stage621').first()
  const sourceRow = representativeCard.locator('.recall-home-parity-card-source-row-stage621').first()
  const source = representativeCard.locator('.recall-home-parity-card-source-stage621').first()
  const chip = representativeCard.locator('.recall-home-parity-card-chip-stage621').first()
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 622 card'),
    readMeasuredBox(preview, 'Stage 622 card preview'),
    readMeasuredBox(copy, 'Stage 622 card copy block'),
    readMeasuredBox(title, 'Stage 622 card title'),
    readMeasuredBox(sourceRow, 'Stage 622 card source row'),
    readMeasuredBox(chip, 'Stage 622 card chip'),
  ])
  ...
  const [titleState, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 622 Home canvas, day header, and grid.')
      }
  ...
  if (!(previewRatio < stage614PreviewRatioBaseline && previewRatio <= previewRatioCeiling)) {
    throw new Error(`Expected preview ratio below the Stage 614 baseline and within the target band, found ${previewRatio}.`)
  }
  if (!(copyRatio > stage614CopyRatioBaseline && copyRatio >= copyRatioFloor)) {
    throw new Error(`Expected copy ratio above the Stage 614 baseline and within the target band, found ${copyRatio}.`)
  }
  if (!(titleState.lineHeightPx > stage614TitleLineHeightPxBaseline && titleState.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 614 baseline, found ${titleState.lineHeightPx}.`)
  }
  if (!(titleToSourceGapPx > stage614TitleToSourceGapPxBaseline && titleToSourceGapPx >= titleToSourceGapPxFloor)) {
    throw new Error(`Expected title-to-source gap above the Stage 614 baseline, found ${titleToSourceGapPx}.`)
  }
  if (!(sourceToChipGapPx > stage614SourceToChipGapPxBaseline && sourceToChipGapPx >= sourceToChipGapPxFloor)) {
    throw new Error(`Expected source-to-chip gap above the Stage 614 baseline, found ${sourceToChipGapPx}.`)
  }
  if (!(sourceState.fontSizePx >= sourceFontSizePxFloor && sourceState.alpha >= sourceAlphaFloor)) {
    throw new Error(`Expected source-row legibility to stay at or above the Stage 611 band, found ${sourceState.fontSizePx}px and alpha ${sourceState.alpha}.`)
  }
  if (!(chipState.fontSizePx >= chipFontSizePxFloor && chipState.alpha >= chipAlphaFloor)) {
    throw new Error(`Expected chip legibility to stay at or above the Stage 611 band, found ${chipState.fontSizePx}px and alpha ${chipState.alpha}.`)
  }
  ...
  const webCard = canvas.locator('.recall-home-parity-card-stage621').first()
...
async function openOriginalReaderFromHome(page, directory, stagePrefix) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not keep Reader in Original mode for the Stage 622 audit.')
  }
...
async function readTextNodeState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible text content for the measured Stage 622 node.')
  }
  return {
...
async function readChipState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible chip text for the measured Stage 622 chip.')
  }
  return {
    alpha: parseColorAlpha(styles.color),
