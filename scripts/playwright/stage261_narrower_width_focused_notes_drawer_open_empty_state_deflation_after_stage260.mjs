...
const outputDir = process.env.RECALL_STAGE261_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE261_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE261_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE261_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE261_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE261_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage261-narrower-width-focused-notes-drawer-open-empty-state-deflation-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage261-focused-overview-narrow-top.png',
  )
  ...
  const focusedNotesEmptyNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage261-focused-notes-empty-narrow-top.png',
  )
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage261-focused-notes-drawer-open-empty-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureTopScreenshot(page, outputDir, 'stage261-reader-narrow-top.png')

  await writeFile(
    path.join(outputDir, 'stage261-narrower-width-focused-notes-drawer-open-empty-state-deflation-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        captures: {
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage261-narrower-width-focused-notes-drawer-open-empty-state-deflation-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
