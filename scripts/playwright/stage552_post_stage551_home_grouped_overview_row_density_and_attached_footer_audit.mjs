...
const outputDir = process.env.RECALL_STAGE552_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE552_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE552_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE552_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE552_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE552_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage552-post-stage551-home-grouped-overview-row-density-and-attached-footer-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage552-post-stage551-home-grouped-overview-row-density-and-attached-footer-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeOverviewEvidence = await captureHomeOverviewEvidence(page, outputDir, 'stage552')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage552-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const { sourceTitle } = await openOriginalReaderFromHome(readerPage)
  const readerOriginalWideTop = await captureViewportScreenshot(readerPage, outputDir, 'stage552-reader-original-wide-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage552-post-stage551-home-grouped-overview-row-density-and-attached-footer-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 551 grouped-overview row-density and attached-footer reset',
          'the grouped overview should now keep denser source-list rows without the old preview-detail band while the Stage 547 title seam, Stage 549 lean card tops, and the Stage 537 primary-lane plus secondary-stack composition stay intact',
...
async function captureHomeOverviewEvidence(page, directory, stagePrefix) {
  ...
  if (!primaryMetric || secondaryMetrics.length !== 2) {
    throw new Error('Could not measure the primary lane and secondary stack cards for the Stage 552 audit.')
  }
  for (const metric of cardMetrics) {
    ...
    if (metric.rowDetailCount > 0) {
      throw new Error(
        `Grouped-overview card ${metric.title} still rendered ${metric.rowDetailCount} preview-detail node(s); the row-density audit failed.`,
      )
    }
    ...
    if (metric.footerButtonHeight != null && metric.footerButtonHeight > maximumOverviewFooterButtonHeight) {
      throw new Error(
        `Grouped-overview card ${metric.title} footer button height (${metric.footerButtonHeight}px) stayed too tall; the attached-footer audit failed.`,
      )
    }
    if (metric.title === 'Captures' && metric.footerButtonHeight == null) {
      throw new Error(
        'The Captures grouped-overview card no longer exposed a measurable Show all footer button for the Stage 552 audit.',
      )
    }
  }
  ...
  if (!overviewBox || !headerBox || !gridBox || !titleRowBox || !titleHeadingBox || !metaBox) {
    throw new Error('Could not measure the grouped-overview shell geometry for the Stage 552 audit.')
  }
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 552 audit.')
  }
  return { sourceTitle }
}
