...
const outputDir = process.env.RECALL_STAGE247_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE247_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE247_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE247_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE247_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE247_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage247-narrower-width-focused-study-right-lane-bundle-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage247-focused-overview-narrow-top.png',
  )
  ...
  const focusedStudyNarrowTop = await captureTopScreenshot(page, outputDir, 'stage247-focused-study-narrow-top.png')
  ...
  const focusedStudyAnswerShownNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage247-focused-study-answer-shown-narrow-top.png',
  )
  const focusedStudyAnswerShownNarrowFull = await captureFullPageScreenshot(
    page,
    outputDir,
    'stage247-focused-study-answer-shown-narrow-full.png',
  )
  ...
  const focusedStudyQueueOpenNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage247-focused-study-queue-open-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureTopScreenshot(page, outputDir, 'stage247-reader-narrow-top.png')

  await writeFile(
    path.join(outputDir, 'stage247-narrower-width-focused-study-right-lane-bundle-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        captures: {
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage247-narrower-width-focused-study-right-lane-bundle-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
