...
const outputDir = process.env.RECALL_STAGE549_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE549_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE549_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE549_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE549_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE549_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage549-home-grouped-overview-card-intro-copy-retirement-and-earlier-row-start-reset-after-stage548-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeOverviewEvidence = await captureHomeOverviewEvidence(page, outputDir, 'stage549')

  await writeFile(
    path.join(
      outputDir,
      'stage549-home-grouped-overview-card-intro-copy-retirement-and-earlier-row-start-reset-after-stage548-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        runtimeBrowser,
        validationFocus: [
          'wide desktop Home keeps the grouped overview open while each grouped-overview card retires its repeated intro-copy band and starts source rows earlier',
          'the Stage 547 lean title cluster, the Stage 545 inline title-status join, and the Stage 537 primary-lane plus secondary-stack composition should stay intact while the card tops get leaner',
          'the card intro-copy retirement should improve the working-board read without widening into selected-group or generated-content Reader work',
        ],
      },
      null,
      2,
...
async function captureHomeOverviewEvidence(page, directory, stagePrefix) {
  ...
  if (!primaryMetric || secondaryMetrics.length !== 2) {
    throw new Error('Could not measure the primary lane and secondary stack cards for the Stage 549 validation.')
  }
  ...
  for (const metric of cardMetrics) {
    ...
    if (metric.headerParagraphCount > 0) {
      throw new Error(
        `Grouped-overview card ${metric.title} still rendered ${metric.headerParagraphCount} intro paragraph(s); the intro-copy retirement did not hold.`,
      )
    }
  ...
  if (!overviewBox || !headerBox || !gridBox || !titleRowBox || !titleHeadingBox || !metaBox) {
    throw new Error('Could not measure the grouped-overview shell geometry for the Stage 549 validation.')
  }
