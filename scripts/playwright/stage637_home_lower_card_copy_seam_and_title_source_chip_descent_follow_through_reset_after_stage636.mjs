...
const outputDir = process.env.RECALL_STAGE637_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE637_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE637_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE637_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE637_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE637_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
    path.join(
      outputDir,
      'stage637-home-lower-card-copy-seam-and-title-source-chip-descent-follow-through-reset-after-stage636-failure.png',
    ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeImplementation(page, outputDir, 'stage637')

  await writeFile(
    path.join(
      outputDir,
      'stage637-home-lower-card-copy-seam-and-title-source-chip-descent-follow-through-reset-after-stage636-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        implementationFocus: [
          ...
          'The next polish pass should reduce the remaining March 25, 2026 Recall homepage mismatch by carrying the lower-card copy seam and title/source/chip descent one subtler step past the Stage 635 finish state.',
          'Shell, rail, toolbar, column cadence, and source-aware poster variants should stay intact while the lower copy stack gains a small additional share and a calmer descending cadence.',
        ],
...
async function captureHomeImplementation(page, directory, stagePrefix) {
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 637 card'),
    readMeasuredBox(preview, 'Stage 637 card preview'),
    readMeasuredBox(copy, 'Stage 637 card copy block'),
    readMeasuredBox(title, 'Stage 637 card title'),
    readMeasuredBox(sourceRow, 'Stage 637 card source row'),
    readMeasuredBox(chip, 'Stage 637 card chip'),
  ])
  ...
  const [titleStyles, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 637 Home canvas, day header, and grid.')
      }
  ...
  if (!(titleStyles.lineHeightPx > stage635TitleLineHeightPxBaseline && titleStyles.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 635 baseline, found ${titleStyles.lineHeightPx}.`)
  }
  ...
  if (!(representativeCardBox.height <= cardHeightPxCeiling)) {
    throw new Error(`Expected representative card height to stay compact after the Stage 635 pass, found ${representativeCardBox.height}.`)
  }
