...
const outputDir = process.env.RECALL_STAGE652_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE652_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE652_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE652_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE652_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE652_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage652-post-stage651-home-lower-card-copy-seam-and-title-source-chip-descent-settle-finish-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage652-post-stage651-home-lower-card-copy-seam-and-title-source-chip-descent-settle-finish-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeAudit(page, outputDir, 'stage652')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage650-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const readerEvidence = await openOriginalReaderFromHome(readerPage, outputDir, 'stage652')

  await writeFile(
    path.join(
      outputDir,
      'stage652-post-stage651-home-lower-card-copy-seam-and-title-source-chip-descent-settle-finish-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 651 lower-card copy-seam and title/source/chip descent settle finish pass',
          'the Home surface should keep the Stage 563 structure while the lower hierarchy reads slightly cleaner than the Stage 649 finish-follow-through baseline across title, source row, and collection chip',
          'Graph and original-only Reader remain regression surfaces only after the Home-only Stage 651 pass',
        ],
        baseUrl,
...
async function captureHomeAudit(page, directory, stagePrefix) {
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 652 card'),
    readMeasuredBox(preview, 'Stage 652 card preview'),
    readMeasuredBox(copy, 'Stage 652 card copy block'),
    readMeasuredBox(title, 'Stage 652 card title'),
    readMeasuredBox(sourceRow, 'Stage 652 card source row'),
    readMeasuredBox(chip, 'Stage 652 card chip'),
  ])
  ...
  const [titleState, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 652 Home canvas, day header, and grid.')
      }
...
async function openOriginalReaderFromHome(page, directory, stagePrefix) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not keep Reader in Original mode for the Stage 652 audit.')
  }
...
async function readTextNodeState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible text content for the measured Stage 652 node.')
  }
  return {
...
async function readChipState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible chip text for the measured Stage 652 chip.')
  }
  return {
    alpha: parseColorAlpha(styles.color),
