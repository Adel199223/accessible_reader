...
const outputDir = process.env.RECALL_STAGE625_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE625_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE625_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE625_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE625_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE625_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage625-home-lower-card-title-source-chip-hierarchy-refinement-reset-after-stage624-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeImplementation(page, outputDir, 'stage625')

  await writeFile(
    path.join(
      outputDir,
      'stage625-home-lower-card-title-source-chip-hierarchy-refinement-reset-after-stage624-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        implementationFocus: [
          ...
          'The next polish pass should reduce the remaining March 25, 2026 Recall homepage mismatch by giving the lower title/source/chip seam one subtler hierarchy refinement.',
          'Shell, rail, toolbar, column cadence, and source-aware poster variants should stay intact while the lower copy stack reads steadier and the title gains slightly clearer ownership at page scale.',
        ],
...
async function captureHomeImplementation(page, directory, stagePrefix) {
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 625 card'),
    readMeasuredBox(preview, 'Stage 625 card preview'),
    readMeasuredBox(copy, 'Stage 625 card copy block'),
    readMeasuredBox(title, 'Stage 625 card title'),
    readMeasuredBox(sourceRow, 'Stage 625 card source row'),
    readMeasuredBox(chip, 'Stage 625 card chip'),
  ])
  ...
  const [titleStyles, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 625 Home canvas, day header, and grid.')
      }
  ...
  if (!(titleStyles.lineHeightPx > stage623TitleLineHeightPxBaseline && titleStyles.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 623 baseline, found ${titleStyles.lineHeightPx}.`)
  }
  ...
  if (!(representativeCardBox.height <= cardHeightPxCeiling)) {
    throw new Error(`Expected representative card height to stay compact after the Stage 625 pass, found ${representativeCardBox.height}.`)
  }
...
async function readTextNodeState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible text content for the measured Stage 625 node.')
  }
  return {
...
async function readChipState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible chip text for the measured Stage 625 chip.')
  }
  return {
    alpha: parseColorAlpha(styles.color),
