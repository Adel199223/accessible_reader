...
const outputDir = process.env.RECALL_STAGE553_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE553_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE553_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE553_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE553_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE553_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage553-home-grouped-overview-overline-retirement-and-variable-title-floor-reset-after-stage552-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeOverviewEvidence = await captureHomeOverviewEvidence(page, outputDir, 'stage553')

  await writeFile(
    path.join(
      outputDir,
      'stage553-home-grouped-overview-overline-retirement-and-variable-title-floor-reset-after-stage552-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        runtimeBrowser,
        validationFocus: [
          'wide desktop Home keeps the grouped overview open while grouped-overview rows retire the visible source-type overline and stop reserving a fixed two-line title floor',
          'the Stage 551 attached footer, the Stage 549 lean card tops, and the Stage 537 primary-lane plus secondary-stack composition should stay intact while the row internals compress further',
          'the row-internal reset should improve the working-board read without widening into selected-group work or generated-content Reader scope',
        ],
      },
      null,
      2,
...
async function captureHomeOverviewEvidence(page, directory, stagePrefix) {
  ...
  if (!primaryMetric || secondaryMetrics.length !== 2) {
    throw new Error('Could not measure the primary lane and secondary stack cards for the Stage 553 validation.')
  }
  ...
  for (const metric of cardMetrics) {
    ...
    if (metric.title === 'Captures' && metric.footerButtonHeight == null) {
      throw new Error(
        'The Captures grouped-overview card no longer exposed a measurable Show all footer button for the Stage 553 validation.',
      )
    }
  }
  ...
  if (!overviewBox || !headerBox || !gridBox || !titleRowBox || !titleHeadingBox || !metaBox) {
    throw new Error('Could not measure the grouped-overview shell geometry for the Stage 553 validation.')
  }
