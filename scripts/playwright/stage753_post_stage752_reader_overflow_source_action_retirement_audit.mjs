...
const requiredOverflowActions = ['Theme', 'Notebook']
...
const outputDir = process.env.RECALL_STAGE753_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE753_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE753_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE753_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE753_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE753_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true

await mkdir(outputDir, { recursive: true })
await rm(path.join(outputDir, 'stage753-reader-overflow-source-action-retirement-audit-failure.png'), { force: true })
await rm(path.join(outputDir, 'stage753-reader-overflow-source-action-retirement-regression-failure.png'), { force: true })
...
try {
  ...
  if (!Array.isArray(defaultAvailableModes) || defaultAvailableModes.length === 0) {
    throw new Error('Expected the Stage 753 audit to resolve the default Reader document available modes.')
  }
  ...
  if (Array.isArray(defaultReaderOverflowActionLabels) && defaultReaderOverflowActionLabels.includes('Source')) {
    throw new Error('Expected the default Reader overflow to retire the duplicate Source action.')
  }
  ...
  if (!sourceOpenReaderSourceLibraryVisible || !sourceOpenReaderSourceTabsVisible) {
    throw new Error('Expected Source support to keep reopening normally during the Stage 753 audit.')
  }
  if (!notebookOpenWorkbenchVisible) {
    throw new Error('Expected Notebook support to keep reopening normally during the Stage 753 audit.')
  }
  ...
  if (Array.isArray(summaryReaderOverflowActionLabels) && summaryReaderOverflowActionLabels.includes('Source')) {
    throw new Error('Expected the Summary Reader overflow to retire the duplicate Source action.')
  }
  ...
  if (simplifiedViewAvailable) {
    throw new Error('Expected the live Stage 753 dataset to keep Simplified unavailable, but it was present during audit.')
  }

  await writeFile(
    path.join(outputDir, 'stage753-post-stage752-reader-overflow-source-action-retirement-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'Reader should keep Theme and Notebook in the overflow while retiring the duplicate Source quick action.',
          'The default and Summary Reader overflows should still keep voice and rate reachable and should still avoid passive copy.',
  ...
  if (readerPage) {
    await readerPage
      .screenshot({
        fullPage: true,
        path: path.join(outputDir, 'stage753-reader-overflow-source-action-retirement-audit-failure.png'),
      })
      .catch(() => undefined)
  }
  if (regressionPage && regressionPage !== readerPage) {
    await regressionPage
      .screenshot({
        fullPage: true,
        path: path.join(outputDir, 'stage753-reader-overflow-source-action-retirement-regression-failure.png'),
      })
      .catch(() => undefined)
  }
  throw error
