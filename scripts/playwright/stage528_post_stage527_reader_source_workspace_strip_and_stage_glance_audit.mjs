...
const outputDir = process.env.RECALL_STAGE528_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE528_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE528_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE528_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE528_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE528_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage528-post-stage527-reader-source-workspace-strip-and-stage-glance-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage528-post-stage527-reader-source-workspace-strip-and-stage-glance-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage528-home-wide-top.png')

  await openGraph(desktopPage)
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage528-graph-wide-top.png')
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopReaderPage,
    outputDir,
    'stage528-reader-original-wide-top.png',
  )
  const readerSourceWorkspaceWideTop = await captureLocatorTopScreenshot(
    desktopReaderPage,
    await getVisibleLocator(desktopReaderPage, ['.source-workspace-frame-reader-active', '.source-workspace-frame']),
    outputDir,
    'stage528-reader-source-workspace-wide-top.png',
    260,
  )
  const readerEntrySeamWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage528-reader-entry-seam-wide-top.png',
    420,
  )
  const readerArticleStartWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage528-reader-article-start-wide-top.png',
    520,
  )
  const readerMetrics = await getReaderEntryMetrics(desktopReaderPage)

  await writeFile(
    path.join(
      outputDir,
      'stage528-post-stage527-reader-source-workspace-strip-and-stage-glance-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop original-only Reader is audited first after the Stage 527 entry-seam reset',
          'Reader crops should show a slimmer source-workspace strip and calmer pre-article seam without losing dock continuity',
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 528 audit.')
  }
