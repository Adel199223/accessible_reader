...
const outputDir = process.env.RECALL_STAGE361_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE361_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE361_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE361_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE361_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE361_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage361-desktop-first-notes-redesign-milestone-after-stage360-failure.png'), {
  force: true,
})
...
try {
  ...
  const {
    ...
    notesWideTop,
  } = await captureWideNotesArtifacts(desktopPage, outputDir, 'stage361')
  ...
  } = await captureFocusedNotesRegressionArtifacts(focusedPage, outputDir, 'stage361')

  await writeFile(
    path.join(outputDir, 'stage361-desktop-first-notes-redesign-milestone-after-stage360-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        ...
        sourceTitle,
        validationFocus: [
          'desktop-first Notes milestone validation with wide desktop Notes top-state plus separate browse-rail and note-detail crops',
          'focused and narrow Notes regression captures for the default top-state and the drawer-open Search notes state',
          'neighboring Home, Graph, Reader, and Study remain regression surfaces for the Stage 362 audit while Notes stays the active milestone',
        ],
      },
      null,
      2,
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage361-desktop-first-notes-redesign-milestone-after-stage360-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (focusedPage && focusedPage !== desktopPage) {
    await focusedPage
      .screenshot({
        path: path.join(
          outputDir,
          'stage361-desktop-first-notes-redesign-milestone-after-stage360-failure-focused.png',
        ),
        fullPage: true,
      })
      .catch(() => undefined)
