...
const outputDir = process.env.RECALL_STAGE559_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE559_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE559_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE559_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE559_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE559_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage559-home-grouped-overview-visible-count-chip-retirement-and-accessible-count-preservation-reset-after-stage558-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeOverviewEvidence = await captureHomeOverviewEvidence(page, outputDir, 'stage559')

  await writeFile(
    path.join(
      outputDir,
      'stage559-home-grouped-overview-visible-count-chip-retirement-and-accessible-count-preservation-reset-after-stage558-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        runtimeBrowser,
        validationFocus: [
          'wide desktop Home keeps the grouped overview open while grouped-overview card headers retire the visible per-card source-count chip',
          'grouped-overview card regions should preserve source-count context accessibly, and the quieter heading seam should hold without reopening row-height, footer, or composition regressions',
          'the grouped-overview count-chip reset should stay bounded to Home board card headers without widening into selected-group work or generated-content Reader scope',
        ],
      },
      null,
      2,
...
async function captureHomeOverviewEvidence(page, directory, stagePrefix) {
  ...
  if (!primaryMetric || secondaryMetrics.length !== 2) {
    throw new Error('Could not measure the primary lane and secondary stack cards for the Stage 559 validation.')
  }
  ...
  for (const metric of cardMetrics) {
    ...
    if (metric.title === 'Captures' && metric.footerButtonHeight == null) {
      throw new Error(
        'The Captures grouped-overview card no longer exposed a measurable Show all footer button for the Stage 559 validation.',
      )
    }
  }
  ...
  if (!overviewBox || !headerBox || !gridBox || !titleRowBox || !titleHeadingBox || !metaBox) {
    throw new Error('Could not measure the grouped-overview shell geometry for the Stage 559 validation.')
  }
