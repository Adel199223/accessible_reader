...
const outputDir = process.env.RECALL_STAGE372_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE372_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE372_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE372_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE372_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE372_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage372-desktop-first-home-finish-milestone-after-stage371-failure.png'), {
  force: true,
})
...
try {
  ...
  const {
    ...
    homeWideTop,
  } = await captureDesktopHomeArtifacts(desktopPage, outputDir, 'stage372')
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage372-focused-overview-narrow-top.png',
  )

  await writeFile(
    path.join(outputDir, 'stage372-desktop-first-home-finish-milestone-after-stage371-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        ...
        sourceTitle,
        validationFocus: [
          'desktop-first Home finish milestone validation with wide desktop Home top-state plus a primary resume/library crop and a lower saved-source continuation crop',
          'focused overview and narrow Home regression capture after reopening one source from Home',
          'retention of neighboring Graph, Reader, and Notes surfaces for the Stage 373 milestone audit rather than this implementation harness',
        ],
      },
      null,
      2,
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage372-desktop-first-home-finish-milestone-after-stage371-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (focusedPage && focusedPage !== desktopPage) {
    await focusedPage
      .screenshot({
        path: path.join(
          outputDir,
          'stage372-desktop-first-home-finish-milestone-after-stage371-failure-focused.png',
        ),
        fullPage: true,
      })
      .catch(() => undefined)
