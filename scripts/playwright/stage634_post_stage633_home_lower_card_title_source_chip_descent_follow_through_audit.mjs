...
const outputDir = process.env.RECALL_STAGE634_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE634_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE634_BASE_URL ?? 'http://172.27.18.251:8002'
const headless = process.env.RECALL_STAGE634_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE634_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE634_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const desktopViewport = { width: 1600, height: 1080 }
const stage631PreviewRatioBaseline = 0.6041458733205374
const stage631CopyRatioBaseline = 0.28122840690978884
const stage631TitleLineHeightPxBaseline = 18.0288
const stage631TitleToSourceGapPxBaseline = 4.21875
const stage631SourceToChipGapPxBaseline = 3.515625
...
const previewRatioCeiling = 0.6039
const copyRatioFloor = 0.282
const titleLineHeightPxFloor = 18.12
const titleToSourceGapPxFloor = 4.28
const sourceToChipGapPxFloor = 3.56
const sourceFontSizePxFloor = 10.46
const chipFontSizePxFloor = 7.77
const sourceAlphaFloor = 0.71
const chipAlphaFloor = 0.54
...
await rm(
    path.join(
      outputDir,
      'stage634-post-stage633-home-lower-card-title-source-chip-descent-follow-through-audit-failure.png',
    ),
  { force: true },
)
await rm(
    path.join(
      outputDir,
      'stage634-post-stage633-home-lower-card-title-source-chip-descent-follow-through-audit-failure-reader.png',
    ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeAudit(page, outputDir, 'stage634')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage634-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const readerEvidence = await openOriginalReaderFromHome(readerPage, outputDir, 'stage634')

  await writeFile(
    path.join(
      outputDir,
      'stage634-post-stage633-home-lower-card-title-source-chip-descent-follow-through-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 633 lower-card title/source/chip descent follow-through pass',
          'the Home surface should keep the Stage 563 structure while the lower hierarchy reads slightly cleaner than the Stage 631 baseline across title, source row, and collection chip',
          'Graph and original-only Reader remain regression surfaces only after the Home-only Stage 633 pass',
        ],
        baseUrl,
...
async function captureHomeAudit(page, directory, stagePrefix) {
  ...
  const representativeCard = dayGroup.locator('.recall-home-parity-card-stage633').first()
  const preview = representativeCard.locator('.recall-home-parity-card-preview-stage633').first()
  const copy = representativeCard.locator('.recall-home-parity-card-copy-stage633').first()
  const title = representativeCard.locator('.recall-home-parity-card-title-stage633').first()
  const sourceRow = representativeCard.locator('.recall-home-parity-card-source-row-stage633').first()
  const source = representativeCard.locator('.recall-home-parity-card-source-stage633').first()
  const chip = representativeCard.locator('.recall-home-parity-card-chip-stage633').first()
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 634 card'),
    readMeasuredBox(preview, 'Stage 634 card preview'),
    readMeasuredBox(copy, 'Stage 634 card copy block'),
    readMeasuredBox(title, 'Stage 634 card title'),
    readMeasuredBox(sourceRow, 'Stage 634 card source row'),
    readMeasuredBox(chip, 'Stage 634 card chip'),
  ])
  ...
  const [titleState, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 634 Home canvas, day header, and grid.')
      }
  ...
  if (!(previewRatio < stage631PreviewRatioBaseline && previewRatio <= previewRatioCeiling)) {
    throw new Error(`Expected preview ratio below the Stage 631 baseline and within the target band, found ${previewRatio}.`)
  }
  if (!(copyRatio > stage631CopyRatioBaseline && copyRatio >= copyRatioFloor)) {
    throw new Error(`Expected copy ratio above the Stage 631 baseline and within the target band, found ${copyRatio}.`)
  }
  if (!(titleState.lineHeightPx > stage631TitleLineHeightPxBaseline && titleState.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 631 baseline, found ${titleState.lineHeightPx}.`)
  }
  if (!(titleToSourceGapPx > stage631TitleToSourceGapPxBaseline && titleToSourceGapPx >= titleToSourceGapPxFloor)) {
    throw new Error(`Expected title-to-source gap above the Stage 631 baseline, found ${titleToSourceGapPx}.`)
  }
  if (!(sourceToChipGapPx > stage631SourceToChipGapPxBaseline && sourceToChipGapPx >= sourceToChipGapPxFloor)) {
    throw new Error(`Expected source-to-chip gap above the Stage 631 baseline, found ${sourceToChipGapPx}.`)
  }
  if (!(sourceState.fontSizePx >= sourceFontSizePxFloor && sourceState.alpha >= sourceAlphaFloor)) {
    throw new Error(`Expected source-row legibility to stay at or above the Stage 631 band, found ${sourceState.fontSizePx}px and alpha ${sourceState.alpha}.`)
  }
  if (!(chipState.fontSizePx >= chipFontSizePxFloor && chipState.alpha >= chipAlphaFloor)) {
    throw new Error(`Expected chip legibility to stay at or above the Stage 631 band, found ${chipState.fontSizePx}px and alpha ${chipState.alpha}.`)
  }
  ...
  const webCard = canvas.locator('.recall-home-parity-card-stage633').first()
...
async function openOriginalReaderFromHome(page, directory, stagePrefix) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not keep Reader in Original mode for the Stage 634 audit.')
  }
...
async function readTextNodeState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible text content for the measured Stage 634 node.')
  }
  return {
...
async function readChipState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible chip text for the measured Stage 634 chip.')
  }
  return {
    alpha: parseColorAlpha(styles.color),
