...
const outputDir = process.env.RECALL_STAGE631_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE631_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE631_BASE_URL ?? 'http://172.27.18.251:8002'
const headless = process.env.RECALL_STAGE631_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE631_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE631_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
    path.join(
      outputDir,
      'stage631-home-lower-card-hierarchy-finish-and-title-source-chip-descent-reset-after-stage630-failure.png',
    ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeImplementation(page, outputDir, 'stage631')

  await writeFile(
    path.join(
      outputDir,
      'stage631-home-lower-card-hierarchy-finish-and-title-source-chip-descent-reset-after-stage630-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        implementationFocus: [
          ...
          'The next polish pass should reduce the remaining March 25, 2026 Recall homepage mismatch by finishing the lower hierarchy a little more cleanly across the title, source row, and collection chip.',
          'Shell, rail, toolbar, column cadence, and source-aware poster variants should stay intact while the lower copy stack gains one final calmer descent pass.',
        ],
...
async function captureHomeImplementation(page, directory, stagePrefix) {
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 631 card'),
    readMeasuredBox(preview, 'Stage 631 card preview'),
    readMeasuredBox(copy, 'Stage 631 card copy block'),
    readMeasuredBox(title, 'Stage 631 card title'),
    readMeasuredBox(sourceRow, 'Stage 631 card source row'),
    readMeasuredBox(chip, 'Stage 631 card chip'),
  ])
  ...
  const [titleStyles, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 631 Home canvas, day header, and grid.')
      }
  ...
  if (!(titleStyles.lineHeightPx > stage629TitleLineHeightPxBaseline && titleStyles.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 629 baseline, found ${titleStyles.lineHeightPx}.`)
  }
  ...
  if (!(representativeCardBox.height <= cardHeightPxCeiling)) {
    throw new Error(`Expected representative card height to stay compact after the Stage 631 pass, found ${representativeCardBox.height}.`)
  }
...
async function readTextNodeState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible text content for the measured Stage 631 node.')
  }
  return {
...
async function readChipState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible chip text for the measured Stage 631 chip.')
  }
  return {
    alpha: parseColorAlpha(styles.color),
