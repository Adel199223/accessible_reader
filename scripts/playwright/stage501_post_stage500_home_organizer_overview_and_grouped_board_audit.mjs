...
const outputDir = process.env.RECALL_STAGE501_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE501_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE501_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE501_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE501_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE501_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage501-post-stage500-home-organizer-overview-and-grouped-board-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage501-post-stage500-home-organizer-overview-and-grouped-board-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeWideTop = path.join(outputDir, 'stage501-home-wide-top.png')
  await desktopPage.screenshot({ path: homeWideTop })

  const homeOrganizerOverviewRowWideTop = path.join(
    outputDir,
    'stage501-home-organizer-overview-row-wide-top.png',
  )
  await overviewButton.screenshot({ path: homeOrganizerOverviewRowWideTop })

  const homeGroupedOverviewBoardWideTop = path.join(
    outputDir,
    'stage501-home-grouped-overview-board-wide-top.png',
  )
  await savedLibraryOverview.screenshot({ path: homeGroupedOverviewBoardWideTop })
  ...
  const homeSelectedGroupWideTop = path.join(outputDir, 'stage501-home-selected-group-wide-top.png')
  await desktopPage.screenshot({ path: homeSelectedGroupWideTop })

  await openGraph(desktopPage)
  const graphWideTop = path.join(outputDir, 'stage501-graph-wide-top.png')
  ...
  const readerOriginalWideTop = path.join(outputDir, 'stage501-reader-original-wide-top.png')
  await desktopReaderPage.screenshot({ path: readerOriginalWideTop })

  await writeFile(
    path.join(
      outputDir,
      'stage501-post-stage500-home-organizer-overview-and-grouped-board-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 500 overview/reset and grouped-board deflation pass',
          'the organizer overview row should now read like a clearer reset target while the grouped board stays calmer above the fold',
        ...
        captures: {
          graphWideTop,
          homeGroupedOverviewBoardWideTop,
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 501 audit.')
  }
  await page.waitForTimeout(250)
  return { sourceTitle }
}
