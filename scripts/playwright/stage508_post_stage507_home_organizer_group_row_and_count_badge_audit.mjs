...
const outputDir = process.env.RECALL_STAGE508_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE508_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE508_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE508_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE508_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE508_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage508-post-stage507-home-organizer-group-row-and-count-badge-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage508-post-stage507-home-organizer-group-row-and-count-badge-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeWideTop = path.join(outputDir, 'stage508-home-wide-top.png')
  await desktopPage.screenshot({ path: homeWideTop })

  const homeOrganizerOverviewRowWideTop = path.join(outputDir, 'stage508-home-organizer-overview-row-wide-top.png')
  await overviewButton.screenshot({ path: homeOrganizerOverviewRowWideTop })

  const homeOrganizerGroupRowsWideTop = path.join(outputDir, 'stage508-home-organizer-group-rows-wide-top.png')
  ...
  const homeOrganizerActiveGroupRowWideTop = path.join(
    outputDir,
    'stage508-home-organizer-active-group-row-wide-top.png',
  )
  await activeBranch.screenshot({ path: homeOrganizerActiveGroupRowWideTop })

  const homeSelectedGroupWideTop = path.join(outputDir, 'stage508-home-selected-group-wide-top.png')
  await desktopPage.screenshot({ path: homeSelectedGroupWideTop })

  await openGraph(desktopPage)
  const graphWideTop = path.join(outputDir, 'stage508-graph-wide-top.png')
  ...
  const readerOriginalWideTop = path.join(outputDir, 'stage508-reader-original-wide-top.png')
  await desktopReaderPage.screenshot({ path: readerOriginalWideTop })

  await writeFile(
    path.join(
      outputDir,
      'stage508-post-stage507-home-organizer-group-row-and-count-badge-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 507 organizer group-row and count-badge deflation pass',
          'the top-level organizer rows should now feel flatter and more continuous instead of lightly boxed mini-panels',
          'overview or reset readouts plus top-level counts should now read like lighter organizer readouts while Graph and original-only Reader remain regression surfaces',
        ],
        baseUrl,
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 508 audit.')
  }
  await page.waitForTimeout(250)
  return { sourceTitle }
}
