...
const outputDir = process.env.RECALL_STAGE448_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE448_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE448_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE448_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE448_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE448_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage448-post-stage447-home-organizer-owned-navigation-audit-failure.png'), {
  force: true,
})
await rm(path.join(outputDir, 'stage448-post-stage447-home-organizer-owned-navigation-audit-failure-reader.png'), {
  force: true,
})
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage448-home-wide-top.png')
  const homeShellStatusSeamWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage448-home-shell-status-seam-wide-top.png',
    180,
  )
  const homeOrganizerControlHeaderWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage448-home-organizer-control-header-wide-top.png',
    280,
    'locator',
  )
  const homeOrganizerListWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage448-home-organizer-list-wide-top.png',
    820,
    'locator',
  )
  const homePrimaryBoardShellWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage448-home-primary-board-shell-wide-top.png',
    980,
  )
  const homeActiveBoardStateWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage448-home-active-board-state-wide-top.png',
    860,
    'locator',
  )
  ...
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage448-graph-wide-top.png')
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopReaderPage,
    outputDir,
    'stage448-reader-original-wide-top.png',
  )

  await writeFile(
    path.join(outputDir, 'stage448-post-stage447-home-organizer-owned-navigation-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 447 organizer-owned navigation reset',
          'Home evidence centers a smaller shell-status seam, organizer-owned controls, and a quieter filtered board instead of a repeated control stack',
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage448-post-stage447-home-organizer-owned-navigation-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (desktopReaderPage && desktopReaderPage !== desktopPage) {
    await desktopReaderPage
      .screenshot({
        path: path.join(outputDir, 'stage448-post-stage447-home-organizer-owned-navigation-audit-failure-reader.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 448 audit.')
  }
