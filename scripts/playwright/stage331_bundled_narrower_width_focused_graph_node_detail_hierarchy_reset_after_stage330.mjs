...
const outputDir = process.env.RECALL_STAGE331_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE331_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE331_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE331_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE331_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE331_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(
  path.join(
    outputDir,
    'stage331-bundled-narrower-width-focused-graph-node-detail-hierarchy-reset-after-stage330-failure.png',
  ),
  {
    force: true,
  },
...
try {
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage331-focused-overview-narrow-top.png',
  )
  ...
  const focusedGraphNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage331-focused-graph-narrow-top.png')
  const focusedGraphNarrowFull = await captureFullPageScreenshot(
    page,
    outputDir,
    'stage331-focused-graph-narrow-full.png',
  )
  ...
  const focusedGraphNodeDetailRailNarrowTop = await captureLocatorTopScreenshot(
    ...
    graphFocusedDetail,
    outputDir,
    'stage331-focused-graph-node-detail-rail-narrow-top.png',
    920,
  )
  const focusedGraphSelectedNodeGlanceLeadingGroundedEvidencePreviewMetaNarrowPanelTop = await captureLocatorTopScreenshot(
    ...
    graphFocusedDetail,
    outputDir,
    'stage331-focused-graph-selected-node-glance-leading-grounded-evidence-preview-meta-narrow-panel-top.png',
    720,
  )
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage331-focused-notes-drawer-open-empty-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage331-focused-study-narrow-top.png')
  ...
  const focusedStudyAnswerShownNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage331-focused-study-answer-shown-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage331-reader-narrow-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage331-bundled-narrower-width-focused-graph-node-detail-hierarchy-reset-after-stage330-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
