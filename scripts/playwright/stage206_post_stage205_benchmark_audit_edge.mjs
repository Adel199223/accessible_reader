...
const outputDir = process.env.RECALL_STAGE206_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE206_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE206_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE206_HEADLESS === '0' ? false : true

await mkdir(outputDir, { recursive: true })
await rm(path.join(outputDir, 'stage206-post-stage205-benchmark-audit-failure.png'), { force: true })
...
try {
  ...
  const homeDesktop = path.join(outputDir, 'stage206-home-landing-desktop.png')
  ...
  const graphDesktop = path.join(outputDir, 'stage206-graph-browse-desktop.png')
  ...
  const studyDesktop = path.join(outputDir, 'stage206-study-browse-desktop.png')
  ...
  const focusedStudyDesktop = path.join(outputDir, 'stage206-focused-study-desktop.png')
  await page.screenshot({ path: focusedStudyDesktop, fullPage: true })

  await writeFile(
    path.join(outputDir, 'stage206-post-stage205-benchmark-audit-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        focusedStudyDesktop,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage206-post-stage205-benchmark-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
