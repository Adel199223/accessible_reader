...
const outputDir = process.env.RECALL_STAGE649_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE649_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE649_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE649_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE649_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE649_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage649-home-lower-card-copy-seam-and-title-source-chip-descent-settle-finish-follow-through-reset-after-stage648-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeImplementation(page, outputDir, 'stage649')

  await writeFile(
    path.join(
      outputDir,
      'stage649-home-lower-card-copy-seam-and-title-source-chip-descent-settle-finish-follow-through-reset-after-stage648-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        implementationFocus: [
          ...
          'The next polish pass should reduce the remaining March 25, 2026 Recall homepage mismatch by carrying one subtler lower-card copy-seam and title/source/chip settle finish follow-through beyond the Stage 647 follow-through baseline.',
...
async function captureHomeImplementation(page, directory, stagePrefix) {
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 649 card'),
    readMeasuredBox(preview, 'Stage 649 card preview'),
    readMeasuredBox(copy, 'Stage 649 card copy block'),
    readMeasuredBox(title, 'Stage 649 card title'),
    readMeasuredBox(sourceRow, 'Stage 649 card source row'),
    readMeasuredBox(chip, 'Stage 649 card chip'),
  ])
  ...
  const [titleStyles, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 649 Home canvas, day header, and grid.')
      }
  ...
  if (!(titleStyles.lineHeightPx > stage647TitleLineHeightPxBaseline && titleStyles.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 647 baseline, found ${titleStyles.lineHeightPx}.`)
  }
...
async function readTextNodeState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible text content for the measured Stage 649 node.')
  }
  return {
...
async function readChipState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible chip text for the measured Stage 649 chip.')
  }
  return {
    alpha: parseColorAlpha(styles.color),
