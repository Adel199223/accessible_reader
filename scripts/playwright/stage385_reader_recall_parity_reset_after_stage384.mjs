...
const outputDir = process.env.RECALL_STAGE385_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE385_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE385_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE385_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE385_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE385_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage385-reader-recall-parity-reset-after-stage384-failure.png'), { force: true })
...
try {
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopPage,
    outputDir,
    'stage385-reader-original-wide-top.png',
  )
  const readerControlSeamWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage385-reader-control-seam-wide-top.png',
    380,
  )
  const readerArticleLaneWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage385-reader-article-lane-wide-top.png',
    980,
  )
  const readerSupportDockWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage385-reader-support-dock-wide-top.png',
    980,
  )
  ...
  const readerOriginalNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage385-reader-original-narrow-top.png',
  )

  await writeFile(
    path.join(outputDir, 'stage385-reader-recall-parity-reset-after-stage384-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        ...
        focusedSourceTitle,
        validationFocus: [
          'wide desktop original-only Reader reset judged first after the Stage 384 Home parity closeout',
          'Reader-specific crops lock the slimmer control seam, dominant article lane, and calmer attached support dock',
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage385-reader-recall-parity-reset-after-stage384-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (focusedPage && focusedPage !== desktopPage) {
    await focusedPage
      .screenshot({
        path: path.join(outputDir, 'stage385-reader-recall-parity-reset-after-stage384-failure-focused.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 385 validation.')
  }
