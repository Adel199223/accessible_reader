...
const outputDir = process.env.RECALL_STAGE584_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE584_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE584_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE584_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE584_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE584_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const desktopViewport = { width: 1600, height: 1080 }
const stage582RailHeadingMetaFontSizeBaseline = 10.24
const stage582RailHeadingMetaAlphaBaseline = 0.46
const stage582RailSummaryFontSizeBaseline = 10.72
const stage582RailSummaryAlphaBaseline = 0.54
const stage582PreviewBadgeFontSizeBaseline = 9.28
const stage582PreviewBadgeBackgroundAlphaBaseline = 0.72
const stage582PreviewBadgeBorderAlphaBaseline = 0.07
const stage582PreviewBadgeColorAlphaBaseline = 0.9

await mkdir(outputDir, { recursive: true })
await rm(
  path.join(outputDir, 'stage584-post-stage583-home-collection-header-summary-and-poster-badge-softening-audit-failure.png'),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage584-post-stage583-home-collection-header-summary-and-poster-badge-softening-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeAudit(page, outputDir, 'stage584')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage584-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const readerEvidence = await openOriginalReaderFromHome(readerPage, outputDir, 'stage584')

  await writeFile(
    path.join(
      outputDir,
      'stage584-post-stage583-home-collection-header-summary-and-poster-badge-softening-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 583 collection-header summary and poster-badge softening pass',
          'the Home surface should keep the Stage 563 structure while calming the rail header summary seam and the poster badge chrome',
          'Graph and original-only Reader remain regression surfaces only after the Home-only Stage 583 pass',
        ],
        baseUrl,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(
          outputDir,
          'stage584-post-stage583-home-collection-header-summary-and-poster-badge-softening-audit-failure.png',
        ),
        fullPage: true,
      })
      .catch(() => undefined)
...
async function captureHomeAudit(page, directory, stagePrefix) {
  ...
  const activeRailCountText = normalizeText(
    await activeRailButton.locator('.recall-home-parity-rail-count-stage563').first().textContent(),
  )
  const activeRailCount = Number.parseInt(activeRailCountText, 10)
  ...
  const railHeadingMeta = rail.locator('.recall-home-parity-rail-heading-meta-stage571').first()
  const railSummary = rail.locator('.recall-home-parity-rail-note-stage563').first()
  await railHeadingMeta.waitFor({ state: 'visible', timeout: 20000 })
  await railSummary.waitFor({ state: 'visible', timeout: 20000 })
  const railHeadingMetaText = normalizeText(await railHeadingMeta.textContent())
  const railSummaryText = normalizeText(await railSummary.textContent())
  const railHeadingMetaStyles = await railHeadingMeta.evaluate((node) => {
  ...
  const railSummaryStyles = await railSummary.evaluate((node) => {
    const styles = window.getComputedStyle(node)
    return {
      color: styles.color,
      fontSize: styles.fontSize,
  ...
  const expectedRailSummary =
    Number.isFinite(activeRailCount) && activeRailCount > 0
      ...
      : ''
  const railHeadingMetaFontSize = parseFloat(railHeadingMetaStyles.fontSize)
  const railHeadingMetaAlpha = parseCssAlpha(railHeadingMetaStyles.color)
  const railSummaryFontSize = parseFloat(railSummaryStyles.fontSize)
  const railSummaryAlpha = parseCssAlpha(railSummaryStyles.color)
  if (
    !railHeadingMetaText ||
    !railSummaryText ||
    (expectedRailSummary && railSummaryText !== expectedRailSummary) ||
    railSummaryText.includes(activeRailLabel) ||
    railHeadingMetaFontSize >= stage582RailHeadingMetaFontSizeBaseline ||
    railHeadingMetaAlpha >= stage582RailHeadingMetaAlphaBaseline ||
    railSummaryFontSize >= stage582RailSummaryFontSizeBaseline ||
    railSummaryAlpha >= stage582RailSummaryAlphaBaseline
  ) {
    throw new Error(
      `Expected quieter collection-header summary chrome below the Stage 582 baseline, received ${JSON.stringify({
        activeRailLabel,
        railHeadingMetaText,
        railHeadingMetaStyles,
        railSummaryText,
        railSummaryStyles,
      })}.`,
    )
  ...
  const previewBadgeFontSize = parseFloat(previewBadgeStyles.fontSize)
  ...
  const previewBadgeColorAlpha = parseCssAlpha(previewBadgeStyles.color)
  if (
    !previewBadgeText ||
    previewBadgeFontSize >= stage582PreviewBadgeFontSizeBaseline ||
    previewBadgeBackgroundAlpha >= stage582PreviewBadgeBackgroundAlphaBaseline ||
    previewBadgeBorderAlpha >= stage582PreviewBadgeBorderAlphaBaseline ||
    previewBadgeColorAlpha >= stage582PreviewBadgeColorAlphaBaseline
  ) {
    throw new Error(
      `Expected subtler poster badge chrome below the Stage 582 baseline, received ${JSON.stringify({
        previewBadgeText,
        previewBadgeStyles,
      })}.`,
    )
...
async function openOriginalReaderFromHome(page, directory, stagePrefix) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not keep Reader in Original mode for the Stage 584 audit.')
  }
