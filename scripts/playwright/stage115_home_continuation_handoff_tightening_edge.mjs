...
const outputDir = process.env.RECALL_STAGE115_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE115_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE115_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE115_HEADLESS === '0' ? false : true

await mkdir(outputDir, { recursive: true })
await rm(path.join(outputDir, 'stage115-home-continuation-handoff-tightening-failure.png'), { force: true })
...
try {
  ...
  const homeDesktop = path.join(outputDir, 'stage115-home-landing-desktop.png')
  ...
  const graphDesktop = path.join(outputDir, 'stage115-graph-browse-desktop.png')
  ...
  await page.getByRole('heading', { name: 'Session', level: 2 }).waitFor({ state: 'visible', timeout: 20000 })
  await page.waitForTimeout(1000)

  const studyDesktop = path.join(outputDir, 'stage115-study-browse-desktop.png')
  ...
  const focusedStudyDesktop = path.join(outputDir, 'stage115-focused-study-desktop.png')
  await page.screenshot({ path: focusedStudyDesktop, fullPage: true })

  await writeFile(
    path.join(outputDir, 'stage115-home-continuation-handoff-tightening-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        focusedStudyDesktop,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage115-home-continuation-handoff-tightening-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
