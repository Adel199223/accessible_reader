...
const outputDir = process.env.RECALL_STAGE446_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE446_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE446_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE446_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE446_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE446_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage446-post-stage445-graph-working-control-hierarchy-audit-failure.png'), {
  force: true,
})
await rm(path.join(outputDir, 'stage446-post-stage445-graph-working-control-hierarchy-audit-failure-reader.png'), {
  force: true,
})
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage446-home-wide-top.png')
  ...
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage446-graph-wide-top.png')
  const graphLauncherPodWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage446-graph-launcher-pod-wide-top.png',
  )
  ...
  const graphToolsOverlayWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage446-graph-tools-overlay-wide-top.png',
  )
  ...
  const graphDetailDockPeekWideTop = await captureLocatorTopScreenshot(
    ...
    graphDetailDock,
    outputDir,
    'stage446-graph-detail-dock-peek-wide-top.png',
    'locator',
  )
  const graphWorkingTrailActiveWideTop = await captureLocatorTopScreenshot(
    ...
    desktopPage.locator('.recall-graph-focus-rail').first(),
    outputDir,
    'stage446-graph-working-trail-active-wide-top.png',
    'locator',
  )
  ...
  const graphDetailDockExpandedWideTop = await captureLocatorTopScreenshot(
    ...
    graphDetailDock,
    outputDir,
    'stage446-graph-detail-dock-expanded-wide-top.png',
    'locator',
  )
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopReaderPage,
    outputDir,
    'stage446-reader-original-wide-top.png',
  )

  await writeFile(
    path.join(outputDir, 'stage446-post-stage445-graph-working-control-hierarchy-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Graph audit judged first after the Stage 445 working-control reset',
          'supporting Graph crops cover the launcher pod, search-status pod, working trail, tools overlay, and inspect drawer states',
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage446-post-stage445-graph-working-control-hierarchy-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (desktopReaderPage && desktopReaderPage !== desktopPage) {
    await desktopReaderPage
      .screenshot({
        path: path.join(outputDir, 'stage446-post-stage445-graph-working-control-hierarchy-audit-failure-reader.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 446 audit.')
  }
...
async function selectFirstGraphNode(page) {
  ...
  if (bestIndex < 0) {
    throw new Error('Could not find a visible graph node to select during the Stage 446 audit.')
  }
