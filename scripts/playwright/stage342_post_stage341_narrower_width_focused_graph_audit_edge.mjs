...
const outputDir = process.env.RECALL_STAGE342_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE342_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE342_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE342_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE342_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE342_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage342-post-stage341-narrower-width-focused-graph-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage342-focused-overview-narrow-top.png',
  )
  ...
  const focusedGraphNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage342-focused-graph-narrow-top.png')
  ...
  const continuationFlowBundle = graphFocusedDetail.locator('.recall-graph-focused-detail-section-follow-on-bundle').first()
  ...
  await continuationFlowBundle.waitFor({ state: 'visible', timeout: 20000 })
  await continuationFlowBundle.locator('.recall-graph-focused-follow-on-entry').waitFor({ state: 'visible', timeout: 20000 })
  ...
  const focusedGraphNodeDetailRailNarrowTop = await captureLocatorTopScreenshot(
    ...
    graphFocusedDetail,
    outputDir,
    'stage342-focused-graph-node-detail-rail-narrow-top.png',
    920,
  )
  const focusedGraphLeadingEvidenceMetaDensityNarrowPanelTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage342-focused-graph-leading-evidence-meta-density-narrow-panel-top.png',
    500,
  )
  const focusedGraphContinuationFlowBundleNarrowPanelTop = await captureLocatorTopScreenshot(
    page,
    continuationFlowBundle,
    outputDir,
    'stage342-focused-graph-continuation-flow-bundle-narrow-panel-top.png',
    760,
  )
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage342-focused-study-narrow-top.png')
  ...
  const focusedStudyAnswerShownNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage342-focused-study-answer-shown-narrow-top.png',
  )
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage342-focused-notes-drawer-open-empty-narrow-top.png',
  )
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage342-post-stage341-narrower-width-focused-graph-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
