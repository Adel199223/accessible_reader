...
const outputDir = process.env.RECALL_STAGE354_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE354_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE354_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE354_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE354_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE354_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage354-post-stage353-narrower-width-focused-graph-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage354-focused-overview-narrow-top.png',
  )
  ...
  const focusedGraphNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage354-focused-graph-narrow-top.png')
  ...
  const focusedGraphNodeDetailRailNarrowTop = await captureLocatorTopScreenshot(
    ...
    graphFocusedDetail,
    outputDir,
    'stage354-focused-graph-node-detail-rail-narrow-top.png',
    920,
  )
  const focusedGraphFollowOnTextWallResetNarrowPanelTop = await captureLocatorBottomScreenshot(
    page,
    followOnBundle,
    outputDir,
    'stage354-focused-graph-follow-on-text-wall-reset-narrow-panel-top.png',
    560,
  )
  const focusedGraphLeadingEvidenceMetaDensityNarrowPanelTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage354-focused-graph-leading-evidence-meta-density-narrow-panel-top.png',
    500,
  )
  const focusedGraphFollowOnBundleNarrowPanelTop = await captureLocatorTopScreenshot(
    ...
    followOnBundle,
    outputDir,
    'stage354-focused-graph-follow-on-bundle-narrow-panel-top.png',
    760,
  )
  const focusedGraphRelationsFollowOnNarrowPanelTop =
    (await relationsFollowOn.count()) > 0
      ? await captureLocatorTopScreenshot(
          ...
          relationsFollowOn,
          outputDir,
          'stage354-focused-graph-relations-follow-on-narrow-panel-top.png',
          760,
        )
      : null
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage354-focused-study-narrow-top.png')
  ...
  const focusedStudyAnswerShownNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage354-focused-study-answer-shown-narrow-top.png',
  )
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage354-focused-notes-drawer-open-empty-narrow-top.png',
  )
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage354-post-stage353-narrower-width-focused-graph-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
...
async function captureLocatorBottomScreenshot(page, locator, directory, filename, maxHeight = 720) {
  await page.evaluate(() => window.scrollTo({ top: 0, behavior: 'instant' }))
  await page.waitForTimeout(150)
  const box = await locator.boundingBox()
  if (!box) {
  ...
  const clipX = Math.max(0, Math.floor(box.x))
  const clipWidth = Math.max(1, Math.min(Math.ceil(box.width), viewportSize.width - clipX))
  const clipHeight = Math.max(1, Math.min(Math.ceil(box.height), maxHeight, viewportSize.height - Math.floor(box.y)))
  const clipY = Math.max(
    0,
    Math.min(
      Math.floor(box.y) + Math.max(0, Math.floor(box.height) - clipHeight),
      viewportSize.height - clipHeight,
    ),
  )
  const screenshotPath = path.join(directory, filename)
  await page.screenshot({
