...
const outputDir = process.env.RECALL_STAGE273_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE273_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE273_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE273_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE273_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE273_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(
  path.join(outputDir, 'stage273-bundled-narrower-width-focused-study-answer-shown-right-lane-deflation-failure.png'),
  {
    force: true,
  },
)
...
try {
  ...
  const focusedOverviewNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage273-focused-overview-narrow-top.png',
  )
  ...
  const focusedStudyNarrowTop = await captureTopScreenshot(page, outputDir, 'stage273-focused-study-narrow-top.png')
  ...
  const focusedStudyAnswerShownNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage273-focused-study-answer-shown-narrow-top.png',
  )
  const focusedStudyAnswerShownNarrowFull = await captureFullPageScreenshot(
    page,
    outputDir,
    'stage273-focused-study-answer-shown-narrow-full.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureTopScreenshot(page, outputDir, 'stage273-reader-narrow-top.png')

  await writeFile(
    path.join(outputDir, 'stage273-bundled-narrower-width-focused-study-answer-shown-right-lane-deflation-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        captures: {
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage273-bundled-narrower-width-focused-study-answer-shown-right-lane-deflation-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
