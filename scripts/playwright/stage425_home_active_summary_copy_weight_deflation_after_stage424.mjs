...
const outputDir = process.env.RECALL_STAGE425_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE425_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE425_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE425_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE425_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE425_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage425-home-active-summary-copy-weight-deflation-after-stage424-failure.png'), {
  force: true,
})
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage425-home-wide-top.png')
  const homeControlSeamWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage425-home-control-seam-wide-top.png',
    220,
  )
  const homeOrganizerRailWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage425-home-organizer-rail-wide-top.png',
    980,
  )
  const homeOrganizerSelectedStateWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage425-home-organizer-selected-state-wide-top.png',
    360,
    'locator',
  )
  const homePrimaryFlowWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage425-home-primary-flow-wide-top.png',
    1040,
  )
  const homeSelectedGroupBoardWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage425-home-selected-group-board-wide-top.png',
    1040,
    'locator',
  )
  const homePinnedReopenShelfWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage425-home-pinned-reopen-shelf-wide-top.png',
    920,
    'locator',
  )
  ...
  const homeOrganizerRailCollapsedWideTop = await captureLocatorTopScreenshot(
    ...
    await getVisibleLocator(desktopPage, ['[aria-label="Home browse strip"]']),
    outputDir,
    'stage425-home-organizer-rail-collapsed-wide-top.png',
    980,
  )
  ...
  const homeAlternateGroupBoardWideTop = alternateOrganizerGroup
    ? await captureLocatorTopScreenshot(
        ...
        await getVisibleLocator(desktopPage, ['[aria-label="Saved library"]']),
        outputDir,
        'stage425-home-selected-group-board-alternate-wide-top.png',
        1040,
        'locator',
      )
    : null
  ...
  await writeFile(
    path.join(outputDir, 'stage425-home-active-summary-copy-weight-deflation-after-stage424-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        ...
        validationFocus: [
          'wide desktop Home is judged first after the Stage 424 checkpoint',
          'evidence centers the lighter active bridge copy, the calmer organizer rhythm, and the stable pinned next-source sidecar',
          'one dedicated selected-state crop proves the active organizer branch now uses a shorter, lower-weight hint instead of the earlier fuller bridge sentence',
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage425-home-active-summary-copy-weight-deflation-after-stage424-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
