...
const outputDir = process.env.RECALL_STAGE540_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE540_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE540_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE540_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE540_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE540_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const desktopViewport = { width: 1600, height: 1080 }
const minimumPrimaryWidthDelta = 80
const maximumSecondaryColumnXSpread = 12
const minimumSecondaryRowTopDelta = 60
const maximumOverviewHeaderHeight = 84
const maximumGridOffsetFromOverviewTop = 92

await mkdir(outputDir, { recursive: true })
await rm(
  path.join(
    outputDir,
    'stage540-post-stage539-home-grouped-overview-header-seam-compaction-and-earlier-board-start-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage540-post-stage539-home-grouped-overview-header-seam-compaction-and-earlier-board-start-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeOverviewEvidence = await captureHomeOverviewEvidence(page, outputDir, 'stage540')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage540-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const { sourceTitle } = await openOriginalReaderFromHome(readerPage)
  const readerOriginalWideTop = await captureViewportScreenshot(readerPage, outputDir, 'stage540-reader-original-wide-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage540-post-stage539-home-grouped-overview-header-seam-compaction-and-earlier-board-start-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 539 grouped-overview header-seam compaction',
          'the grouped overview should now start earlier while the Stage 537 primary-lane and secondary-stack composition stays intact',
...
async function captureHomeOverviewEvidence(page, directory, stagePrefix) {
  ...
  const overviewHeader = savedLibraryOverview.locator('.recall-home-library-stream-header-stage539-reset').first()
  const overviewGrid = savedLibraryOverview.locator('.recall-home-library-stream-grid-stage539-reset').first()
  ...
  if (!primaryMetric || secondaryMetrics.length !== 2) {
    throw new Error('Could not measure the primary lane and secondary stack cards for the Stage 540 audit.')
  }

  const [overviewBox, headerBox, gridBox] = await Promise.all([
    savedLibraryOverview.boundingBox(),
    overviewHeader.boundingBox(),
    overviewGrid.boundingBox(),
  ])

  if (!overviewBox || !headerBox || !gridBox) {
    throw new Error('Could not measure the grouped-overview shell geometry for the Stage 540 audit.')
  }
  ...
  if (overviewHeaderHeight > maximumOverviewHeaderHeight) {
    throw new Error(
      `Overview header height (${overviewHeaderHeight}px) stayed too tall; the grouped board still carries too much title-helper banding.`,
    )
  }
  ...
  const homeOverviewHeaderSeamWideTop = await captureLocatorTopScreenshot(
    page,
    savedLibraryOverview,
    directory,
    `${stagePrefix}-home-overview-header-seam-wide-top.png`,
    320,
  )
