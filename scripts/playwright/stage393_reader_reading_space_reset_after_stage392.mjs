...
const outputDir = process.env.RECALL_STAGE393_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE393_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE393_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE393_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE393_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE393_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage393-reader-reading-space-reset-after-stage392-failure.png'), { force: true })
await rm(path.join(outputDir, 'stage393-reader-reading-space-reset-after-stage392-failure-focused.png'), {
  force: true,
})
...
try {
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopPage,
    outputDir,
    'stage393-reader-original-wide-top.png',
  )
  const readerControlSeamWideTop = await captureLocatorTopScreenshot(
    desktopPage,
    await getVisibleLocator(desktopPage, ['.reader-stage-control-ribbon-original-parity', '.reader-stage-control-ribbon']),
    outputDir,
    'stage393-reader-control-seam-wide-top.png',
    380,
  )
  const readerArticleLaneWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage393-reader-article-lane-wide-top.png',
    980,
  )
  const readerSupportDockWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage393-reader-support-dock-wide-top.png',
    980,
  )
  ...
  const readerDockNotesWideTop = await captureLocatorTopScreenshot(
    ...
    await getVisibleLocator(desktopPage, ['.reader-support-dock-original-parity', '.reader-support-dock']),
    outputDir,
    'stage393-reader-dock-notes-wide-top.png',
    980,
  )
  ...
  const readerOriginalNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage393-reader-original-narrow-top.png',
  )

  await writeFile(
    path.join(outputDir, 'stage393-reader-reading-space-reset-after-stage392-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        ...
        focusedSourceTitle,
        validationFocus: [
          'wide desktop original-only Reader reset judged first after the Stage 392 Home checkpoint',
          'Reader-specific crops lock the compressed top seam, larger article lead, and calmer attached dock',
          'one dock-notes capture confirms note work still stays usable without overpowering the reading lane',
          'focused original-only Reader stays available as a regression surface while generated-content Reader workflows remain out of scope',
        ],
      },
      null,
      2,
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage393-reader-reading-space-reset-after-stage392-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (focusedPage && focusedPage !== desktopPage) {
    await focusedPage
      .screenshot({
        path: path.join(outputDir, 'stage393-reader-reading-space-reset-after-stage392-failure-focused.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 393 validation.')
  }
