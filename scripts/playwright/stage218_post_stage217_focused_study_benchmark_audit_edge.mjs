...
const outputDir = process.env.RECALL_STAGE218_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE218_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE218_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE218_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE218_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE218_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true

await mkdir(outputDir, { recursive: true })
await rm(path.join(outputDir, 'stage218-post-stage217-focused-study-benchmark-audit-failure.png'), { force: true })
...
try {
  ...
  const homeDesktop = await captureScreenshot(page, outputDir, 'stage218-home-desktop.png')
  const { sourceTitle, sourceWorkspace } = await openFirstSource(page)
  const focusedOverviewDesktop = await captureScreenshot(page, outputDir, 'stage218-focused-source-overview-desktop.png')
  ...
  const focusedGraphDesktop = await captureScreenshot(page, outputDir, 'stage218-focused-graph-desktop.png')
  ...
  await page.waitForTimeout(900)
  const focusedStudyDesktop = await captureScreenshot(page, outputDir, 'stage218-focused-study-desktop.png')
  ...
  await page.waitForTimeout(900)
  const focusedNotesDesktop = await captureScreenshot(page, outputDir, 'stage218-focused-notes-desktop.png')
  ...
  await page.waitForTimeout(900)
  const readerDesktop = await captureScreenshot(page, outputDir, 'stage218-reader-desktop.png')

  await writeFile(
    path.join(outputDir, 'stage218-post-stage217-focused-study-benchmark-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'focused study support-column weight',
          'retention of reader dominance in focused graph and notes',
          'retention of calmer source strip and shared reader chrome',
          'retention of Stage 213 top-level shell and home gains',
        ],
        baseUrl,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage218-post-stage217-focused-study-benchmark-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
