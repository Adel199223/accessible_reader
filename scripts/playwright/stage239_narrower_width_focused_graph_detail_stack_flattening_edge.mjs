...
const outputDir = process.env.RECALL_STAGE239_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE239_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE239_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE239_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE239_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE239_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage239-narrower-width-focused-graph-detail-stack-flattening-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage239-focused-overview-narrow-top.png',
  )
  ...
  const focusedGraphNarrowTop = await captureTopScreenshot(page, outputDir, 'stage239-focused-graph-narrow-top.png')
  const focusedGraphNarrowFull = await captureFullPageScreenshot(
    page,
    outputDir,
    'stage239-focused-graph-narrow-full.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureTopScreenshot(page, outputDir, 'stage239-reader-narrow-top.png')

  await writeFile(
    path.join(outputDir, 'stage239-narrower-width-focused-graph-detail-stack-flattening-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        captures: {
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage239-narrower-width-focused-graph-detail-stack-flattening-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
