...
const outputDir = process.env.RECALL_STAGE263_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE263_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE263_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE263_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE263_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE263_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(
  path.join(
    outputDir,
    'stage263-narrower-width-focused-notes-drawer-open-empty-detail-panel-deflation-failure.png',
  ),
  {
    force: true,
  },
...
try {
  ...
  const focusedOverviewNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage263-focused-overview-narrow-top.png',
  )
  ...
  const focusedNotesEmptyNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage263-focused-notes-empty-narrow-top.png',
  )
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage263-focused-notes-drawer-open-empty-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureTopScreenshot(page, outputDir, 'stage263-reader-narrow-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage263-narrower-width-focused-notes-drawer-open-empty-detail-panel-deflation-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
