...
const outputDir = process.env.RECALL_STAGE310_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE310_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE310_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE310_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE310_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE310_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage310-post-stage309-narrower-width-focused-graph-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage310-focused-overview-narrow-top.png',
  )
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage310-focused-notes-drawer-open-empty-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const focusedGraphNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage310-focused-graph-narrow-top.png')
  const focusedGraphNarrowFull = await captureFullPageScreenshot(
    page,
    outputDir,
    'stage310-focused-graph-narrow-full.png',
  )

  const sameSourceContinuationUtilityColumn = page
    .locator('.recall-graph-focused-evidence-actions-repeat-source-continuation')
    .first()
  await sameSourceContinuationUtilityColumn.waitFor({ state: 'visible', timeout: 20000 })
  await sameSourceContinuationUtilityColumn.scrollIntoViewIfNeeded()
  await page.waitForTimeout(350)
  const focusedGraphSameSourceContinuationUtilityColumnNarrowTop = await captureCurrentViewportScreenshot(
    page,
    outputDir,
    'stage310-focused-graph-same-source-continuation-utility-column-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage310-focused-study-narrow-top.png')
  ...
  const focusedStudyAnswerShownNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage310-focused-study-answer-shown-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage310-reader-narrow-top.png')

  await writeFile(
    path.join(outputDir, 'stage310-post-stage309-narrower-width-focused-graph-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'narrower-width focused Graph same-source continuation utility-column audit beside live reading',
          ...
          'neighboring focused Notes, Study, and Reader stability after the focused Graph same-source continuation utility-column follow-up',
        ],
        baseUrl,
        ...
        captures: {
          ...
          focusedGraphSameSourceContinuationUtilityColumnNarrowTop,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage310-post-stage309-narrower-width-focused-graph-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
