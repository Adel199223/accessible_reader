...
const outputDir = process.env.RECALL_STAGE588_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE588_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE588_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE588_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE588_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE588_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(outputDir, 'stage588-post-stage587-home-add-tile-icon-weight-and-card-shell-edge-softening-audit-failure.png'),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage588-post-stage587-home-add-tile-icon-weight-and-card-shell-edge-softening-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeAudit(page, outputDir, 'stage588')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage588-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const readerEvidence = await openOriginalReaderFromHome(readerPage, outputDir, 'stage588')

  await writeFile(
    path.join(
      outputDir,
      'stage588-post-stage587-home-add-tile-icon-weight-and-card-shell-edge-softening-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 587 add-tile-icon-weight and card-shell-edge softening pass',
          'the Home surface should keep the Stage 563 structure while calming the Add Content plus-mark weight and the outer board-card shell edge',
          'Graph and original-only Reader remain regression surfaces only after the Home-only Stage 587 pass',
        ],
        baseUrl,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(
          outputDir,
          'stage588-post-stage587-home-add-tile-icon-weight-and-card-shell-edge-softening-audit-failure.png',
        ),
        fullPage: true,
      })
      .catch(() => undefined)
...
async function openOriginalReaderFromHome(page, directory, stagePrefix) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not keep Reader in Original mode for the Stage 588 audit.')
  }
