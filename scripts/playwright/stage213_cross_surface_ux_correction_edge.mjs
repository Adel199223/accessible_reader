...
const outputDir = process.env.RECALL_STAGE213_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE213_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE213_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE213_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE213_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE213_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true

await mkdir(outputDir, { recursive: true })
await rm(path.join(outputDir, 'stage213-cross-surface-ux-correction-failure.png'), { force: true })
...
try {
  ...
  const homeDesktop = await captureScreenshot(page, outputDir, 'stage213-home-desktop.png')

  await page.getByRole('tab', { name: 'Graph', exact: true }).click()
  await page.getByRole('region', { name: 'Knowledge graph canvas' }).waitFor({ state: 'visible', timeout: 20000 })
  await page.waitForTimeout(1000)
  const graphBrowseDesktop = await captureScreenshot(page, outputDir, 'stage213-graph-browse-desktop.png')
  ...
  await page.waitForTimeout(1000)
  const studyBrowseDesktop = await captureScreenshot(page, outputDir, 'stage213-study-browse-desktop.png')
  ...
  await page.waitForTimeout(1000)
  const notesBrowseDesktop = await captureScreenshot(page, outputDir, 'stage213-notes-browse-desktop.png')
  ...
  const notesDetailDesktop = await captureScreenshot(page, outputDir, 'stage213-notes-detail-desktop.png')

  const { sourceTitle, sourceWorkspace } = await openFirstSource(page)
  const focusedOverviewDesktop = await captureScreenshot(page, outputDir, 'stage213-focused-source-overview-desktop.png')
  ...
  await page.waitForTimeout(1000)
  const focusedGraphDesktop = await captureScreenshot(page, outputDir, 'stage213-focused-graph-desktop.png')
  ...
  await page.waitForTimeout(1000)
  const focusedStudyDesktop = await captureScreenshot(page, outputDir, 'stage213-focused-study-desktop.png')

  await sourceWorkspace.getByRole('tab', { name: 'Source workspace Notes', exact: true }).click()
  await page.getByRole('heading', { name: 'Note detail', level: 2 }).waitFor({ state: 'visible', timeout: 20000 })
  await page.waitForTimeout(1000)
  const focusedNotesDesktop = await captureScreenshot(page, outputDir, 'stage213-focused-notes-desktop.png')
  ...
  await page.waitForTimeout(1000)
  const readerDesktop = await captureScreenshot(page, outputDir, 'stage213-reader-desktop.png')

  await writeFile(
    path.join(outputDir, 'stage213-cross-surface-ux-correction-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        captures: {
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage213-cross-surface-ux-correction-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
