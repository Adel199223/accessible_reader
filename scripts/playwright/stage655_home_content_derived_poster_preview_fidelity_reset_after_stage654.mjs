...
const outputDir = process.env.RECALL_STAGE655_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE655_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE655_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE655_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE655_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE655_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage655-home-content-derived-poster-preview-fidelity-reset-after-stage654-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeImplementation(page, outputDir, 'stage655')

  await writeFile(
    path.join(
      outputDir,
      'stage655-home-content-derived-poster-preview-fidelity-reset-after-stage654-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        implementationFocus: [
          ...
          'The Stage 655 pass upgrades Home posters with content-derived hero text from existing stored document views instead of relying only on metadata fallback copy.',
          'Shell, rail, toolbar, board width cadence, and the settled lower title/source/chip seam remain unchanged while paste, web, and file cards gain richer preview fidelity.',
        ],
...
async function captureHomeImplementation(page, directory, stagePrefix) {
  ...
  const pasteState = await readPreviewCardState(page, pasteCard, 'Stage 655 paste card')
  const [pasteCardBox, canvasMetrics] = await Promise.all([
    readMeasuredBox(pasteCard, 'representative Stage 655 paste card'),
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 655 Home canvas, day header, and grid.')
      }
  ...
  const webState = await readPreviewCardState(page, webCard, 'Stage 655 web card')
  ...
  const fileState = await readPreviewCardState(page, fileCard, 'Stage 655 file card')
