...
const outputDir = process.env.RECALL_STAGE240_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE240_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE240_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE240_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE240_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE240_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage240-post-stage239-narrower-width-focused-graph-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage240-focused-overview-narrow-top.png',
  )
  ...
  const focusedNotesNarrowTop = await captureTopScreenshot(page, outputDir, 'stage240-focused-notes-narrow-top.png')
  ...
  await page.waitForTimeout(350)
  const focusedGraphNarrowTop = await captureTopScreenshot(page, outputDir, 'stage240-focused-graph-narrow-top.png')
  const focusedGraphNarrowFull = await captureFullPageScreenshot(
    page,
    outputDir,
    'stage240-focused-graph-narrow-full.png',
  )
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureTopScreenshot(page, outputDir, 'stage240-focused-study-narrow-top.png')
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureTopScreenshot(page, outputDir, 'stage240-reader-narrow-top.png')

  await writeFile(
    path.join(outputDir, 'stage240-post-stage239-narrower-width-focused-graph-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'narrower-width focused Graph detail-stack flattening beside live reading',
          'retention of the Stage 223 shell correction and Stage 227 focused-source strip compression',
          'retention of the Stage 225 Reader gains, Stage 229 focused split balance, Stage 235 focused Study active-card gains, and Stage 237 focused Notes gains',
          'neighboring focused Notes and Study stability after the localized Graph detail-stack pass',
        ],
        baseUrl,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage240-post-stage239-narrower-width-focused-graph-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
