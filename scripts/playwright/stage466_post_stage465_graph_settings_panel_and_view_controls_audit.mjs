...
const outputDir = process.env.RECALL_STAGE466_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE466_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE466_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE466_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE466_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE466_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage466-post-stage465-graph-settings-panel-and-view-controls-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage466-post-stage465-graph-settings-panel-and-view-controls-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage466-home-wide-top.png')
  ...
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage466-graph-wide-top.png')
  ...
  const graphSettingsDrawerWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage466-graph-settings-drawer-wide-top.png',
    'locator',
  )
  ...
  const graphHoverFocusCanvasWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage466-graph-hover-focus-canvas-wide-top.png',
    'locator',
  )
  ...
  const graphBottomRailSelectedWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage466-graph-bottom-rail-selected-wide-top.png',
    'locator',
  )
  ...
  const graphDetailDockPeekWideTop = await captureLocatorTopScreenshot(
    ...
    graphDetailDock,
    outputDir,
    'stage466-graph-detail-dock-peek-wide-top.png',
    'locator',
  )
  ...
  const graphDetailDockExpandedWideTop = await captureLocatorTopScreenshot(
    ...
    graphDetailDock,
    outputDir,
    'stage466-graph-detail-dock-expanded-wide-top.png',
    'locator',
  )
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopReaderPage,
    outputDir,
    'stage466-reader-original-wide-top.png',
  )

  await writeFile(
    path.join(
      outputDir,
      'stage466-post-stage465-graph-settings-panel-and-view-controls-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Graph is audited first after the Stage 465 settings-panel and view-controls reset',
          'Graph evidence should show a true settings sidebar with live view controls while the selected-node workflow stays clearer in the bottom rail plus the right detail drawer',
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 466 audit.')
  }
...
async function getPreferredGraphNode(page) {
  ...
  if (bestIndex < 0) {
    throw new Error('Could not find a visible graph node during the Stage 466 audit.')
  }
