...
const outputDir = process.env.RECALL_STAGE109_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE109_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE109_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE109_HEADLESS === '0' ? false : true

await mkdir(outputDir, { recursive: true })
await rm(path.join(outputDir, 'stage109-home-spotlight-footprint-reduction-failure.png'), { force: true })
...
try {
  ...
  const homeDesktop = path.join(outputDir, 'stage109-home-landing-desktop.png')
  ...
  const graphDesktop = path.join(outputDir, 'stage109-graph-browse-desktop.png')
  ...
  await page.getByText('Queue', { exact: true }).waitFor({ state: 'visible', timeout: 20000 })
  await page.waitForTimeout(1000)

  const studyDesktop = path.join(outputDir, 'stage109-study-browse-desktop.png')
  ...
  const focusedStudyDesktop = path.join(outputDir, 'stage109-focused-study-desktop.png')
  await page.screenshot({ path: focusedStudyDesktop, fullPage: true })

  await writeFile(
    path.join(outputDir, 'stage109-home-spotlight-footprint-reduction-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        focusedStudyDesktop,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage109-home-spotlight-footprint-reduction-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
