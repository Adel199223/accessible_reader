...
const outputDir = process.env.RECALL_STAGE375_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE375_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE375_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE375_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE375_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE375_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage375-post-stage374-reader-priority-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage375-home-wide-top.png')
  ...
  const homePrimaryResumeLibraryWideTop = await captureLocatorTopScreenshot(
    ...
    homeDashboard,
    outputDir,
    'stage375-home-primary-resume-library-wide-top.png',
    820,
  )
  ...
  const homeLowerSavedSourceContinuationWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage375-home-lower-saved-source-continuation-wide-top.png',
    940,
    'locator',
  )
  ...
  await desktopPage.waitForTimeout(350)
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage375-graph-wide-top.png')
  ...
  const {
    readerDockControlWideTop,
    readerMainPanelWideTop,
    readerWideTop,
  } = await captureWideReaderArtifacts(desktopReaderPage, outputDir, 'stage375')
  ...
  await desktopPage.waitForTimeout(350)
  const notesWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage375-notes-wide-top.png')
  ...
  await desktopPage.waitForTimeout(350)
  const studyWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage375-study-wide-top.png')
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage375-focused-overview-narrow-top.png',
  )

  await focusedPage.getByRole('tab', { name: 'Graph', exact: true }).click()
  await focusedPage.getByLabel('Node detail dock').waitFor({ state: 'visible', timeout: 20000 })
  await focusedPage.waitForTimeout(350)
  const focusedGraphNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage375-focused-graph-narrow-top.png',
  )

  await focusedPage.getByRole('tab', { name: 'Notes', exact: true }).click()
  ...
  const focusedNotesNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage375-focused-notes-narrow-top.png',
  )

  await focusedPage.getByRole('tab', { name: 'Study', exact: true }).click()
  await focusedPage.getByRole('heading', { name: 'Review card', level: 2 }).waitFor({ state: 'visible', timeout: 20000 })
  await focusedPage.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage375-focused-study-narrow-top.png',
  )

  await focusedPage.getByRole('tab', { name: 'Reader', exact: true }).click()
  ...
  const {
    ...
    readerNarrowTop,
  } = await captureReaderNarrowRegressionArtifacts(focusedPage, outputDir, 'stage375')

  await writeFile(
    path.join(outputDir, 'stage375-post-stage374-reader-priority-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home, Graph, Reader, and Notes regression captures first, with Study parked as regression-only',
          'Reader-specific wide desktop milestone crops for the main panel and dock/control rail',
          'focused and narrow regressions second: overview, Graph, Notes, Study, and Reader top/overflow/full captures',
          'Reader remains the active milestone under judgment while neighboring surfaces are checked for drift only',
        ],
        baseUrl,
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage375-post-stage374-reader-priority-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (desktopReaderPage && desktopReaderPage !== desktopPage) {
    await desktopReaderPage
      .screenshot({
        path: path.join(outputDir, 'stage375-post-stage374-reader-priority-audit-failure-reader.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (focusedPage && focusedPage !== desktopPage) {
    await focusedPage
      .screenshot({
        path: path.join(outputDir, 'stage375-post-stage374-reader-priority-audit-failure-focused.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
...
async function captureWideReaderArtifacts(page, directory, stagePrefix) {
  ...
  const readerDockControlPanel = await getVisibleLocator(page, [
    '.reader-support-dock',
    '.reader-context-panel',
    '.reader-context-switcher',
  ])
  const readerDockControlWideTop = await captureLocatorTopScreenshot(
    page,
    readerDockControlPanel,
    directory,
    `${stagePrefix}-reader-dock-control-wide-top.png`,
    940,
  )
