...
const outputDir = process.env.RECALL_STAGE647_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE647_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE647_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE647_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE647_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE647_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
    path.join(
      outputDir,
      'stage647-home-lower-card-copy-seam-and-title-source-chip-descent-settle-follow-through-reset-after-stage646-failure.png',
    ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeImplementation(page, outputDir, 'stage647')

  await writeFile(
    path.join(
      outputDir,
      'stage647-home-lower-card-copy-seam-and-title-source-chip-descent-settle-follow-through-reset-after-stage646-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        implementationFocus: [
          ...
          'The next polish pass should reduce the remaining March 25, 2026 Recall homepage mismatch by carrying one subtler lower-card copy-seam and title/source/chip settle follow-through beyond the Stage 645 finish state.',
          'Shell, rail, toolbar, column cadence, and source-aware poster variants should stay intact while the lower copy stack gains one more small step of ownership and a slightly steadier descending cadence.',
        ],
...
async function captureHomeImplementation(page, directory, stagePrefix) {
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 647 card'),
    readMeasuredBox(preview, 'Stage 647 card preview'),
    readMeasuredBox(copy, 'Stage 647 card copy block'),
    readMeasuredBox(title, 'Stage 647 card title'),
    readMeasuredBox(sourceRow, 'Stage 647 card source row'),
    readMeasuredBox(chip, 'Stage 647 card chip'),
  ])
  ...
  const [titleStyles, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 647 Home canvas, day header, and grid.')
      }
  ...
  if (!(titleStyles.lineHeightPx > stage645TitleLineHeightPxBaseline && titleStyles.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 645 baseline, found ${titleStyles.lineHeightPx}.`)
  }
  ...
  if (!(representativeCardBox.height <= cardHeightPxCeiling)) {
    throw new Error(`Expected representative card height to stay compact after the Stage 645 pass, found ${representativeCardBox.height}.`)
  }
...
async function readTextNodeState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible text content for the measured Stage 635 node.')
  }
  return {
...
async function readChipState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible chip text for the measured Stage 635 chip.')
  }
  return {
    alpha: parseColorAlpha(styles.color),
