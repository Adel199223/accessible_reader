...
const outputDir = process.env.RECALL_STAGE46_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE46_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE46_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE46_HEADLESS === '0' ? false : true

await mkdir(outputDir, { recursive: true })
await rm(path.join(outputDir, 'stage46-benchmark-audit-failure.png'), { force: true })
...
try {
  ...
  const homeDesktop = path.join(outputDir, 'stage46-home-landing-desktop.png')
  ...
  const graphDesktop = path.join(outputDir, 'stage46-graph-browse-desktop.png')
  ...
  const focusedGraphDesktop = path.join(outputDir, 'stage46-focused-graph-desktop.png')
  await page.screenshot({ path: focusedGraphDesktop, fullPage: true })

  await page.getByRole('button', { name: 'Browse' }).click()
  await page.getByRole('region', { name: 'Knowledge graph canvas' }).waitFor({ state: 'visible', timeout: 15000 })
  ...
  const studyDesktop = path.join(outputDir, 'stage46-study-browse-desktop.png')
  await page.screenshot({ path: studyDesktop, fullPage: true })

  await writeFile(
    path.join(outputDir, 'stage46-benchmark-audit-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        focusedGraphDesktop,
  ...
  if (page) {
    await page.screenshot({
      path: path.join(outputDir, 'stage46-benchmark-audit-failure.png'),
      fullPage: true,
    }).catch(() => undefined)
  }
  throw error
