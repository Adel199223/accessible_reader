...
const outputDir = process.env.RECALL_STAGE334_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE334_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE334_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE334_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE334_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE334_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage334-post-stage333-narrower-width-focused-graph-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage334-focused-overview-narrow-top.png',
  )
  ...
  const focusedGraphNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage334-focused-graph-narrow-top.png')
  const focusedGraphNarrowFull = await captureFullPageScreenshot(
    page,
    outputDir,
    'stage334-focused-graph-narrow-full.png',
  )
  ...
  const focusedGraphNodeDetailRailNarrowTop = await captureLocatorTopScreenshot(
    ...
    graphFocusedDetail,
    outputDir,
    'stage334-focused-graph-node-detail-rail-narrow-top.png',
    920,
  )
  const focusedGraphLeadingEvidenceReadabilityBundleNarrowPanelTop = await captureLocatorTopScreenshot(
    page,
    leadingGroundedEvidenceCard,
    outputDir,
    'stage334-focused-graph-leading-evidence-readability-bundle-narrow-panel-top.png',
    460,
  )
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage334-focused-notes-drawer-open-empty-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage334-focused-study-narrow-top.png')
  ...
  const focusedStudyAnswerShownNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage334-focused-study-answer-shown-narrow-top.png',
  )
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage334-post-stage333-narrower-width-focused-graph-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
