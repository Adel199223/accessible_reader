...
const outputDir = process.env.RECALL_STAGE267_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE267_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE267_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE267_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE267_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE267_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(
  path.join(
    outputDir,
    'stage267-bundled-narrower-width-focused-graph-node-detail-lane-deflation-failure.png',
  ),
  {
    force: true,
  },
...
try {
  ...
  const focusedOverviewNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage267-focused-overview-narrow-top.png',
  )
  ...
  const focusedGraphNarrowTop = await captureTopScreenshot(page, outputDir, 'stage267-focused-graph-narrow-top.png')
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureTopScreenshot(page, outputDir, 'stage267-focused-study-narrow-top.png')
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureTopScreenshot(page, outputDir, 'stage267-reader-narrow-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage267-bundled-narrower-width-focused-graph-node-detail-lane-deflation-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
