...
const outputDir = process.env.RECALL_STAGE51_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE51_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE51_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE51_HEADLESS === '0' ? false : true

await mkdir(outputDir, { recursive: true })
await rm(path.join(outputDir, 'stage51-study-compression-failure.png'), { force: true })
...
try {
  ...
  const homeDesktop = path.join(outputDir, 'stage51-home-landing-desktop.png')
  ...
  const graphDesktop = path.join(outputDir, 'stage51-graph-browse-desktop.png')
  await page.screenshot({ path: graphDesktop, fullPage: true })

  await page.getByRole('tab', { name: 'Study', exact: true }).click()
  await page.getByRole('heading', { name: 'Recall review', level: 2 }).waitFor({ state: 'visible', timeout: 20000 })
  await page.getByRole('heading', { name: 'Review card', level: 2 }).waitFor({ state: 'visible', timeout: 20000 })
  await page.getByRole('button', { name: 'Hide queue' }).waitFor({ state: 'visible', timeout: 20000 })
  await page.waitForTimeout(1000)

  const studyDesktop = path.join(outputDir, 'stage51-study-browse-desktop.png')
  ...
  const focusedStudyDesktop = path.join(outputDir, 'stage51-focused-study-desktop.png')
  await page.screenshot({ path: focusedStudyDesktop, fullPage: true })

  await writeFile(
    path.join(outputDir, 'stage51-study-compression-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        focusedStudyDesktop,
  ...
  if (page) {
    await page.screenshot({
      path: path.join(outputDir, 'stage51-study-compression-failure.png'),
      fullPage: true,
    }).catch(() => undefined)
  }
  throw error
