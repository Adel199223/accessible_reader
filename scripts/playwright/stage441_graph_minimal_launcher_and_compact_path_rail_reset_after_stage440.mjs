...
const outputDir = process.env.RECALL_STAGE441_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE441_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE441_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE441_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE441_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE441_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage441-graph-minimal-launcher-and-compact-path-rail-reset-after-stage440-failure.png'), {
  force: true,
})
...
try {
  ...
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage441-graph-wide-top.png')
  const graphMinimalLauncherWideTop = await captureViewportClip(
    desktopPage,
    outputDir,
    'stage441-graph-minimal-launcher-wide-top.png',
    { x: 92, y: 74, width: 1380, height: 164 },
  )
  const graphCompactPathRailWideTop = await captureViewportClip(
    desktopPage,
    outputDir,
    'stage441-graph-compact-path-rail-wide-top.png',
  ...
  const graphToolsOverlayWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage441-graph-tools-overlay-wide-top.png',
    980,
  )
  ...
  const graphDetailDockPeekWideTop = await captureLocatorTopScreenshot(
    ...
    graphDetailDock,
    outputDir,
    'stage441-graph-detail-dock-peek-wide-top.png',
    980,
    'locator',
  )
  ...
  const graphDetailDockExpandedWideTop = await captureLocatorTopScreenshot(
    ...
    graphDetailDock,
    outputDir,
    'stage441-graph-detail-dock-expanded-wide-top.png',
    980,
    'locator',
  )

  await writeFile(
    path.join(outputDir, 'stage441-graph-minimal-launcher-and-compact-path-rail-reset-after-stage440-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        ...
        runtimeBrowser,
        validationFocus: [
          'wide desktop Graph judged first after the Stage 440 checkpoint',
          'evidence of a smaller launcher pod, a lighter search pod, and a more compact floating path rail over the stable canvas',
          'attached tools and inspect drawers still prove the canvas-first hierarchy in closed, peek, and expanded states',
        ],
      },
      null,
      2,
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage441-graph-minimal-launcher-and-compact-path-rail-reset-after-stage440-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
...
async function selectFirstGraphNode(page) {
  ...
  if (bestIndex < 0) {
    throw new Error('Could not find a visible graph node to select during the Stage 441 run.')
  }
