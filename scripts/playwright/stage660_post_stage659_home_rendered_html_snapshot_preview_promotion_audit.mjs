...
const outputDir = process.env.RECALL_STAGE660_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE660_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE660_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE660_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE660_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE660_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(outputDir, 'stage660-post-stage659-home-rendered-html-snapshot-preview-promotion-audit-failure.png'),
  { force: true },
)
await rm(
  path.join(outputDir, 'stage660-post-stage659-home-rendered-html-snapshot-preview-promotion-audit-failure-reader.png'),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeAudit(page, outputDir, 'stage660')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage660-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const readerEvidence = await openOriginalReaderFromHome(readerPage, outputDir, 'stage660')

  await writeFile(
    path.join(outputDir, 'stage660-post-stage659-home-rendered-html-snapshot-preview-promotion-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 659 rendered HTML snapshot preview promotion pass',
          'the remaining HTML-backed Web and Documents cards should now render cached real image thumbnails sourced from html-rendered-snapshot while paste/text cards keep the Stage 655 synthetic fallback path',
          'Graph and original-only Reader remain regression surfaces only after the Home-only Stage 659 implementation',
        ],
        baseUrl,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage660-post-stage659-home-rendered-html-snapshot-preview-promotion-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (readerPage && readerPage !== page) {
    await readerPage
      .screenshot({
        path: path.join(outputDir, 'stage660-post-stage659-home-rendered-html-snapshot-preview-promotion-audit-failure-reader.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
...
async function captureHomeAudit(page, directory, stagePrefix) {
  ...
  const fallbackPreviewCardCount = await canvas
    ...
    .count()
  const fallbackState = await readFallbackPreviewCardState(page, fallbackCard, 'Stage 660 fallback card')
  const [fallbackCardBox, canvasMetrics] = await Promise.all([
    readMeasuredBox(fallbackCard, 'representative Stage 660 fallback card'),
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 660 Home canvas, day header, and grid.')
      }
  ...
  const webState = await readImagePreviewCardState(
    page,
    webSelection.card,
    'Stage 660 rendered Web snapshot card',
  )
  ...
  const documentState = await readImagePreviewCardState(
    ...
    'Stage 660 rendered Documents snapshot card',
  )
...
async function openOriginalReaderFromHome(page, directory, stagePrefix) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not keep Reader in Original mode for the Stage 660 audit.')
  }
