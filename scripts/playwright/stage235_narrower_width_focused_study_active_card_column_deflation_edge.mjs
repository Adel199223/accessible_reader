...
const outputDir = process.env.RECALL_STAGE235_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE235_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE235_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE235_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE235_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE235_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage235-narrower-width-focused-study-active-card-column-deflation-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage235-focused-overview-narrow-top.png',
  )
  ...
  const focusedStudyNarrowTop = await captureTopScreenshot(page, outputDir, 'stage235-focused-study-narrow-top.png')
  const focusedStudyNarrowFull = await captureFullPageScreenshot(
    page,
    outputDir,
    'stage235-focused-study-narrow-full.png',
  )
  ...
  const focusedStudyQueueOpenNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage235-focused-study-queue-open-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureTopScreenshot(page, outputDir, 'stage235-reader-narrow-top.png')

  await writeFile(
    path.join(outputDir, 'stage235-narrower-width-focused-study-active-card-column-deflation-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        captures: {
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage235-narrower-width-focused-study-active-card-column-deflation-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
