...
const outputDir = process.env.RECALL_STAGE503_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE503_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE503_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE503_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE503_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE503_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage503-post-stage502-home-organizer-header-and-control-stack-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage503-post-stage502-home-organizer-header-and-control-stack-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const browseTop = browseStrip.locator('.recall-home-browse-strip-top-stage502-reset').first()
  ...
  const savedLibraryOverview = desktopPage.getByRole('region', { name: 'Saved library overview' })

  await browseTop.waitFor({ state: 'visible', timeout: 20000 })
  ...
  const homeWideTop = path.join(outputDir, 'stage503-home-wide-top.png')
  await desktopPage.screenshot({ path: homeWideTop })

  const homeOrganizerHeaderAndControlsWideTop = path.join(
    outputDir,
    'stage503-home-organizer-header-and-controls-wide-top.png',
  )
  await browseTop.screenshot({ path: homeOrganizerHeaderAndControlsWideTop })

  const homeOrganizerControlDeckWideTop = path.join(
    outputDir,
    'stage503-home-organizer-control-deck-wide-top.png',
  )
  ...
  const homeSelectedGroupWideTop = path.join(outputDir, 'stage503-home-selected-group-wide-top.png')
  await desktopPage.screenshot({ path: homeSelectedGroupWideTop })

  await openGraph(desktopPage)
  const graphWideTop = path.join(outputDir, 'stage503-graph-wide-top.png')
  ...
  const readerOriginalWideTop = path.join(outputDir, 'stage503-reader-original-wide-top.png')
  await desktopReaderPage.screenshot({ path: readerOriginalWideTop })

  await writeFile(
    path.join(
      outputDir,
      'stage503-post-stage502-home-organizer-header-and-control-stack-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 502 organizer-header and control-stack compression pass',
          'the organizer upper rail should now read like a tighter utility stack with a lighter helper line and denser control rhythm',
          'Graph and original-only Reader remain regression surfaces, and generated-content Reader work stays locked out of scope',
        ],
        baseUrl,
        ...
        captures: {
          ...
          homeOrganizerHeaderAndControlsWideTop,
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 503 audit.')
  }
  await page.waitForTimeout(250)
  return { sourceTitle }
}
