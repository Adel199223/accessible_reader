...
const outputDir = process.env.RECALL_STAGE434_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE434_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE434_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE434_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE434_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE434_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage434-post-stage433-graph-canvas-first-drawer-audit-failure.png'), {
  force: true,
})
await rm(path.join(outputDir, 'stage434-post-stage433-graph-canvas-first-drawer-audit-failure-reader.png'), {
  force: true,
})
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage434-home-wide-top.png')
  ...
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage434-graph-wide-top.png')
  const graphControlSeamWideTop = await captureLocatorTopScreenshot(
    desktopPage,
    desktopPage.getByLabel('Graph control seam').first(),
    outputDir,
    'stage434-graph-control-seam-wide-top.png',
    220,
  )
  const graphFocusRailWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage434-graph-focus-rail-wide-top.png',
    220,
    'locator',
  )
  ...
  const graphToolsSidebarWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage434-graph-tools-sidebar-wide-top.png',
    980,
  )
  ...
  const graphDetailDockPeekWideTop = await captureLocatorTopScreenshot(
    ...
    graphDetailDock,
    outputDir,
    'stage434-graph-detail-dock-peek-wide-top.png',
    980,
    'locator',
  )
  ...
  const graphDetailDockExpandedWideTop = await captureLocatorTopScreenshot(
    ...
    graphDetailDock,
    outputDir,
    'stage434-graph-detail-dock-expanded-wide-top.png',
    980,
    'locator',
  )
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopReaderPage,
    outputDir,
    'stage434-reader-original-wide-top.png',
  )

  await writeFile(
    path.join(outputDir, 'stage434-post-stage433-graph-canvas-first-drawer-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Graph audit judged first after the Stage 433 canvas-first drawer reset',
          'supporting Graph crops cover the slimmer top seam, hidden-by-default tools sidebar, bottom path rail, and inspect drawer in peek and expanded states',
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage434-post-stage433-graph-canvas-first-drawer-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (desktopReaderPage && desktopReaderPage !== desktopPage) {
    await desktopReaderPage
      .screenshot({
        path: path.join(outputDir, 'stage434-post-stage433-graph-canvas-first-drawer-audit-failure-reader.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 434 audit.')
  }
