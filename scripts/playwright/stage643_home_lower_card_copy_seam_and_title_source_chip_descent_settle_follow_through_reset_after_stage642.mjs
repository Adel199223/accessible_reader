...
const outputDir = process.env.RECALL_STAGE643_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE643_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE643_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE643_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE643_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE643_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
    path.join(
      outputDir,
      'stage643-home-lower-card-copy-seam-and-title-source-chip-descent-settle-follow-through-reset-after-stage642-failure.png',
    ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeImplementation(page, outputDir, 'stage643')

  await writeFile(
    path.join(
      outputDir,
      'stage643-home-lower-card-copy-seam-and-title-source-chip-descent-settle-follow-through-reset-after-stage642-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        implementationFocus: [
          ...
          'The next polish pass should reduce the remaining March 25, 2026 Recall homepage mismatch by carrying one subtler lower-card copy-seam and title/source/chip settle follow-through beyond the Stage 641 settle state.',
          'Shell, rail, toolbar, column cadence, and source-aware poster variants should stay intact while the lower copy stack gains a touch more ownership and a slightly steadier descending cadence.',
        ],
...
async function captureHomeImplementation(page, directory, stagePrefix) {
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 643 card'),
    readMeasuredBox(preview, 'Stage 643 card preview'),
    readMeasuredBox(copy, 'Stage 643 card copy block'),
    readMeasuredBox(title, 'Stage 643 card title'),
    readMeasuredBox(sourceRow, 'Stage 643 card source row'),
    readMeasuredBox(chip, 'Stage 643 card chip'),
  ])
  ...
  const [titleStyles, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 643 Home canvas, day header, and grid.')
      }
  ...
  if (!(titleStyles.lineHeightPx > stage641TitleLineHeightPxBaseline && titleStyles.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 641 baseline, found ${titleStyles.lineHeightPx}.`)
  }
  ...
  if (!(representativeCardBox.height <= cardHeightPxCeiling)) {
    throw new Error(`Expected representative card height to stay compact after the Stage 639 pass, found ${representativeCardBox.height}.`)
  }
