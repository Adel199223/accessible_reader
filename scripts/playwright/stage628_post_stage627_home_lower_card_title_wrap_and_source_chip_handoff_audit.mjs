...
const outputDir = process.env.RECALL_STAGE628_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE628_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE628_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE628_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE628_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE628_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage628-post-stage627-home-lower-card-title-wrap-and-source-chip-handoff-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage628-post-stage627-home-lower-card-title-wrap-and-source-chip-handoff-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeAudit(page, outputDir, 'stage628')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage628-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const readerEvidence = await openOriginalReaderFromHome(readerPage, outputDir, 'stage628')

  await writeFile(
    path.join(
      outputDir,
      'stage628-post-stage627-home-lower-card-title-wrap-and-source-chip-handoff-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 627 lower-card title-wrap and source/chip handoff refinement pass',
          'the Home surface should keep the Stage 563 structure while the lower title line wraps a little more calmly and the source/chip handoff reads slightly steadier than the Stage 625 balance',
          'Graph and original-only Reader remain regression surfaces only after the Home-only Stage 627 pass',
        ],
        baseUrl,
...
async function captureHomeAudit(page, directory, stagePrefix) {
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 628 card'),
    readMeasuredBox(preview, 'Stage 628 card preview'),
    readMeasuredBox(copy, 'Stage 628 card copy block'),
    readMeasuredBox(title, 'Stage 628 card title'),
    readMeasuredBox(sourceRow, 'Stage 628 card source row'),
    readMeasuredBox(chip, 'Stage 628 card chip'),
  ])
  ...
  const [titleState, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 628 Home canvas, day header, and grid.')
      }
  ...
  if (!(titleState.lineHeightPx > stage625TitleLineHeightPxBaseline && titleState.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 625 baseline, found ${titleState.lineHeightPx}.`)
  }
...
async function openOriginalReaderFromHome(page, directory, stagePrefix) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not keep Reader in Original mode for the Stage 628 audit.')
  }
...
async function readTextNodeState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible text content for the measured Stage 628 node.')
  }
  return {
...
async function readChipState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible chip text for the measured Stage 628 chip.')
  }
  return {
    alpha: parseColorAlpha(styles.color),
