...
const outputDir = process.env.RECALL_STAGE249_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE249_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE249_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE249_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE249_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE249_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage249-narrower-width-focused-notes-empty-detail-lane-collapse-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage249-focused-overview-narrow-top.png',
  )
  ...
  const focusedNotesEmptyNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage249-focused-notes-empty-narrow-top.png',
  )
  ...
  const focusedNotesDrawerOpenNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage249-focused-notes-drawer-open-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureTopScreenshot(page, outputDir, 'stage249-reader-narrow-top.png')

  await writeFile(
    path.join(outputDir, 'stage249-narrower-width-focused-notes-empty-detail-lane-collapse-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        captures: {
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage249-narrower-width-focused-notes-empty-detail-lane-collapse-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
