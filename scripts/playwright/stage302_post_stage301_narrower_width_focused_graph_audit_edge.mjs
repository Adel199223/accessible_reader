...
const outputDir = process.env.RECALL_STAGE302_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE302_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE302_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE302_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE302_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE302_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage302-post-stage301-narrower-width-focused-graph-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage302-focused-overview-narrow-top.png',
  )
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage302-focused-notes-drawer-open-empty-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const focusedGraphNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage302-focused-graph-narrow-top.png')
  const focusedGraphNarrowFull = await captureFullPageScreenshot(
    page,
    outputDir,
    'stage302-focused-graph-narrow-full.png',
  )

  const mentionsEntryLeadingCardSeam = page
    .locator('.recall-graph-focused-detail-section-leading-entry-softened')
    .first()
  await mentionsEntryLeadingCardSeam.waitFor({ state: 'visible', timeout: 20000 })
  await mentionsEntryLeadingCardSeam.scrollIntoViewIfNeeded()
  await page.waitForTimeout(350)
  const focusedGraphMentionsEntryLeadingCardSeamNarrowTop = await captureCurrentViewportScreenshot(
    page,
    outputDir,
    'stage302-focused-graph-mentions-entry-leading-card-seam-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage302-focused-study-narrow-top.png')
  ...
  const focusedStudyAnswerShownNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage302-focused-study-answer-shown-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage302-reader-narrow-top.png')

  await writeFile(
    path.join(outputDir, 'stage302-post-stage301-narrower-width-focused-graph-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'narrower-width focused Graph Mentions entry and leading-card seam deflation beside live reading',
          ...
          'neighboring focused Notes, Study, and Reader stability after the focused Graph Mentions entry seam follow-up',
        ],
        baseUrl,
        benchmarkMatrix: 'docs/ux/recall_benchmark_matrix.md',
        captures: {
          focusedGraphMentionsEntryLeadingCardSeamNarrowTop,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage302-post-stage301-narrower-width-focused-graph-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
