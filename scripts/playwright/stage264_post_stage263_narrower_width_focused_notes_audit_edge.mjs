...
const outputDir = process.env.RECALL_STAGE264_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE264_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE264_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE264_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE264_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE264_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage264-post-stage263-narrower-width-focused-notes-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage264-focused-overview-narrow-top.png',
  )
  ...
  const focusedNotesEmptyNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage264-focused-notes-empty-narrow-top.png',
  )
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage264-focused-notes-drawer-open-empty-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const focusedGraphNarrowTop = await captureTopScreenshot(page, outputDir, 'stage264-focused-graph-narrow-top.png')
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureTopScreenshot(page, outputDir, 'stage264-focused-study-narrow-top.png')
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureTopScreenshot(page, outputDir, 'stage264-reader-narrow-top.png')

  await writeFile(
    path.join(outputDir, 'stage264-post-stage263-narrower-width-focused-notes-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'narrower-width focused Notes drawer-open empty detail-panel deflation beside live reading',
          'retention of the Stage 223 shell correction and Stage 227 focused-source strip compression',
          'retention of the Stage 225 Reader gains, Stage 249 focused Notes gains, Stage 251 focused overview gains, Stage 253 focused Study gains, Stage 255 focused Graph gains, Stage 257 focused Study gains, Stage 259 focused Notes empty-rail gains, and Stage 261 focused Notes drawer-open browse-empty gains',
          'neighboring focused Graph and Study stability after the focused Notes empty detail-panel pass',
        ],
        baseUrl,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage264-post-stage263-narrower-width-focused-notes-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
