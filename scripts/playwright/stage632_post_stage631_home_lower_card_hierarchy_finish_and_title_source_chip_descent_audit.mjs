...
const outputDir = process.env.RECALL_STAGE632_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE632_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE632_BASE_URL ?? 'http://172.27.18.251:8002'
const headless = process.env.RECALL_STAGE632_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE632_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE632_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const desktopViewport = { width: 1600, height: 1080 }
const stage629PreviewRatioBaseline = 0.6091362763915547
const stage629CopyRatioBaseline = 0.27731285988483684
const stage629TitleLineHeightPxBaseline = 17.7445
const stage629TitleToSourceGapPxBaseline = 4.03125
const stage629SourceToChipGapPxBaseline = 3.3125
...
const previewRatioCeiling = 0.6088
const copyRatioFloor = 0.278
const titleLineHeightPxFloor = 17.86
const titleToSourceGapPxFloor = 4.1
const sourceToChipGapPxFloor = 3.4
const sourceFontSizePxFloor = 10.44
const chipFontSizePxFloor = 7.74
const sourceAlphaFloor = 0.7
const chipAlphaFloor = 0.53
...
await rm(
    path.join(
      outputDir,
      'stage632-post-stage631-home-lower-card-hierarchy-finish-and-title-source-chip-descent-audit-failure.png',
    ),
  { force: true },
)
await rm(
    path.join(
      outputDir,
      'stage632-post-stage631-home-lower-card-hierarchy-finish-and-title-source-chip-descent-audit-failure-reader.png',
    ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeAudit(page, outputDir, 'stage632')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage632-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const readerEvidence = await openOriginalReaderFromHome(readerPage, outputDir, 'stage632')

  await writeFile(
    path.join(
      outputDir,
      'stage632-post-stage631-home-lower-card-hierarchy-finish-and-title-source-chip-descent-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 631 lower-card hierarchy finish and title/source/chip descent pass',
          'the Home surface should keep the Stage 563 structure while the lower hierarchy reads slightly cleaner than the Stage 629 baseline across title, source row, and collection chip',
          'Graph and original-only Reader remain regression surfaces only after the Home-only Stage 631 pass',
        ],
        baseUrl,
...
async function captureHomeAudit(page, directory, stagePrefix) {
  ...
  const representativeCard = dayGroup.locator('.recall-home-parity-card-stage631').first()
  const preview = representativeCard.locator('.recall-home-parity-card-preview-stage631').first()
  const copy = representativeCard.locator('.recall-home-parity-card-copy-stage631').first()
  const title = representativeCard.locator('.recall-home-parity-card-title-stage631').first()
  const sourceRow = representativeCard.locator('.recall-home-parity-card-source-row-stage631').first()
  const source = representativeCard.locator('.recall-home-parity-card-source-stage631').first()
  const chip = representativeCard.locator('.recall-home-parity-card-chip-stage631').first()
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 632 card'),
    readMeasuredBox(preview, 'Stage 632 card preview'),
    readMeasuredBox(copy, 'Stage 632 card copy block'),
    readMeasuredBox(title, 'Stage 632 card title'),
    readMeasuredBox(sourceRow, 'Stage 632 card source row'),
    readMeasuredBox(chip, 'Stage 632 card chip'),
  ])
  ...
  const [titleState, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 632 Home canvas, day header, and grid.')
      }
  ...
  if (!(previewRatio < stage629PreviewRatioBaseline && previewRatio <= previewRatioCeiling)) {
    throw new Error(`Expected preview ratio below the Stage 629 baseline and within the target band, found ${previewRatio}.`)
  }
  if (!(copyRatio > stage629CopyRatioBaseline && copyRatio >= copyRatioFloor)) {
    throw new Error(`Expected copy ratio above the Stage 629 baseline and within the target band, found ${copyRatio}.`)
  }
  if (!(titleState.lineHeightPx > stage629TitleLineHeightPxBaseline && titleState.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 629 baseline, found ${titleState.lineHeightPx}.`)
  }
  if (!(titleToSourceGapPx > stage629TitleToSourceGapPxBaseline && titleToSourceGapPx >= titleToSourceGapPxFloor)) {
    throw new Error(`Expected title-to-source gap above the Stage 629 baseline, found ${titleToSourceGapPx}.`)
  }
  if (!(sourceToChipGapPx > stage629SourceToChipGapPxBaseline && sourceToChipGapPx >= sourceToChipGapPxFloor)) {
    throw new Error(`Expected source-to-chip gap above the Stage 629 baseline, found ${sourceToChipGapPx}.`)
  }
  if (!(sourceState.fontSizePx >= sourceFontSizePxFloor && sourceState.alpha >= sourceAlphaFloor)) {
    throw new Error(`Expected source-row legibility to stay at or above the Stage 629 band, found ${sourceState.fontSizePx}px and alpha ${sourceState.alpha}.`)
  }
  if (!(chipState.fontSizePx >= chipFontSizePxFloor && chipState.alpha >= chipAlphaFloor)) {
    throw new Error(`Expected chip legibility to stay at or above the Stage 629 band, found ${chipState.fontSizePx}px and alpha ${chipState.alpha}.`)
  }
  ...
  const webCard = canvas.locator('.recall-home-parity-card-stage631').first()
...
async function openOriginalReaderFromHome(page, directory, stagePrefix) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not keep Reader in Original mode for the Stage 632 audit.')
  }
...
async function readTextNodeState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible text content for the measured Stage 632 node.')
  }
  return {
...
async function readChipState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible chip text for the measured Stage 632 chip.')
  }
  return {
    alpha: parseColorAlpha(styles.color),
