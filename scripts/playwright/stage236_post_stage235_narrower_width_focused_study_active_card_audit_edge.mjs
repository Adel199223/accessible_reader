...
const outputDir = process.env.RECALL_STAGE236_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE236_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE236_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE236_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE236_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE236_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage236-post-stage235-narrower-width-focused-study-active-card-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage236-focused-overview-narrow-top.png',
  )
  ...
  const focusedNotesNarrowTop = await captureTopScreenshot(page, outputDir, 'stage236-focused-notes-narrow-top.png')
  ...
  await page.waitForTimeout(350)
  const focusedGraphNarrowTop = await captureTopScreenshot(page, outputDir, 'stage236-focused-graph-narrow-top.png')
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureTopScreenshot(page, outputDir, 'stage236-focused-study-narrow-top.png')
  const focusedStudyNarrowFull = await captureFullPageScreenshot(
    page,
    outputDir,
    'stage236-focused-study-narrow-full.png',
  )
  ...
  const focusedStudyQueueOpenNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage236-focused-study-queue-open-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureTopScreenshot(page, outputDir, 'stage236-reader-narrow-top.png')

  await writeFile(
    path.join(outputDir, 'stage236-post-stage235-narrower-width-focused-study-active-card-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'narrower-width focused Study active-card-column deflation beside live reading',
          'retention of the Stage 223 shell correction and Stage 227 focused-source strip compression',
          'retention of the Stage 225 Reader chrome gains, Stage 229 focused split balance, Stage 231 focused Study rail gains, and Stage 233 focused Graph gains',
          'neighboring focused Notes and Graph stability after the localized Study active-card pass',
        ],
        baseUrl,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage236-post-stage235-narrower-width-focused-study-active-card-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
