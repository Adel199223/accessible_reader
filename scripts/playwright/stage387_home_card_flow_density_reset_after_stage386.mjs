...
const outputDir = process.env.RECALL_STAGE387_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE387_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE387_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE387_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE387_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE387_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage387-home-card-flow-density-reset-after-stage386-failure.png'), {
  force: true,
})
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage387-home-wide-top.png')
  const homeControlSeamWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage387-home-control-seam-wide-top.png',
    320,
  )
  const homeBrowseStripWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage387-home-browse-strip-wide-top.png',
    980,
  )
  const homePrimaryFlowWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage387-home-primary-flow-wide-top.png',
    980,
  )
  const homeLowerSavedSourceContinuationWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage387-home-lower-saved-source-continuation-wide-top.png',
    980,
    'locator',
  )
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage387-focused-overview-narrow-top.png',
  )

  await writeFile(
    path.join(outputDir, 'stage387-home-card-flow-density-reset-after-stage386-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        ...
        sourceTitle,
        validationFocus: [
          'wide desktop Home density reset judged first after the Stage 386 hold-state review',
          'evidence of the slimmer top seam, attached browse strip, denser primary card flow, and reduced blank main lane',
          'focused overview regression stays stable while Reader generated-content workflows remain out of scope',
        ],
      },
      null,
      2,
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage387-home-card-flow-density-reset-after-stage386-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (focusedPage && focusedPage !== desktopPage) {
    await focusedPage
      .screenshot({
        path: path.join(outputDir, 'stage387-home-card-flow-density-reset-after-stage386-failure-focused.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
