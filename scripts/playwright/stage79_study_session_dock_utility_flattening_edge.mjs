...
const outputDir = process.env.RECALL_STAGE79_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE79_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE79_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE79_HEADLESS === '0' ? false : true

await mkdir(outputDir, { recursive: true })
await rm(path.join(outputDir, 'stage79-study-session-dock-utility-flattening-failure.png'), { force: true })

const resolvedHarnessDir = resolveHarnessDir(harnessDir)
const playwrightModuleUrl = pathToFileURL(
  path.join(resolvedHarnessDir, 'node_modules', 'playwright', 'index.mjs'),
).href
...
try {
  ...
  const homeDesktop = path.join(outputDir, 'stage79-home-landing-desktop.png')
  ...
  const graphDesktop = path.join(outputDir, 'stage79-graph-browse-desktop.png')
  ...
  await page.getByRole('button', { name: 'Show queue' }).waitFor({ state: 'visible', timeout: 20000 })
  await page.waitForTimeout(1000)

  const studyDesktop = path.join(outputDir, 'stage79-study-browse-desktop.png')
  ...
  const focusedStudyDesktop = path.join(outputDir, 'stage79-focused-study-desktop.png')
  await page.screenshot({ path: focusedStudyDesktop, fullPage: true })

  await writeFile(
    path.join(outputDir, 'stage79-study-session-dock-utility-flattening-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        focusedStudyDesktop,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage79-study-session-dock-utility-flattening-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
