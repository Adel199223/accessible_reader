...
const outputDir = process.env.RECALL_STAGE343_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE343_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE343_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE343_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE343_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE343_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(
  path.join(
    outputDir,
    'stage343-bundled-narrower-width-focused-study-active-card-flow-reset-after-stage342-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage343-focused-overview-narrow-top.png',
  )
  ...
  const focusedStudyNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage343-focused-study-narrow-top.png')
  ...
  const focusedStudyRightLaneNarrowPanelTop = await captureLocatorTopScreenshot(
    ...
    focusedStudyPanel,
    outputDir,
    'stage343-focused-study-right-lane-narrow-panel-top.png',
    760,
  )
  const focusedStudyBodyFlowNarrowPanelTop = await captureLocatorTopScreenshot(
    ...
    focusedStudyBody,
    outputDir,
    'stage343-focused-study-body-flow-narrow-panel-top.png',
    760,
  )
  ...
  const focusedStudyAnswerShownNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage343-focused-study-answer-shown-narrow-top.png',
  )
  const focusedStudyAnswerShownRightLaneNarrowPanelTop = await captureLocatorTopScreenshot(
    page,
    focusedStudyPanel,
    outputDir,
    'stage343-focused-study-answer-shown-right-lane-narrow-panel-top.png',
    760,
  )
  const focusedStudyAnswerShownBodyFlowNarrowPanelTop = await captureLocatorTopScreenshot(
    ...
    focusedStudyBody,
    outputDir,
    'stage343-focused-study-answer-shown-body-flow-narrow-panel-top.png',
    760,
  )
  ...
  await page.waitForTimeout(350)
  const focusedGraphNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage343-focused-graph-narrow-top.png')
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage343-focused-notes-drawer-open-empty-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage343-reader-narrow-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage343-bundled-narrower-width-focused-study-active-card-flow-reset-after-stage342-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
