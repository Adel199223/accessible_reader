...
const outputDir = process.env.RECALL_STAGE308_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE308_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE308_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE308_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE308_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE308_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage308-post-stage307-narrower-width-focused-graph-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage308-focused-overview-narrow-top.png',
  )
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage308-focused-notes-drawer-open-empty-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const focusedGraphNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage308-focused-graph-narrow-top.png')
  const focusedGraphNarrowFull = await captureFullPageScreenshot(
    page,
    outputDir,
    'stage308-focused-graph-narrow-full.png',
  )

  const sameSourceContinuationUtilitySeam = page
    .locator('.recall-graph-focused-evidence-actions-repeat-source-continuation')
    .first()
  await sameSourceContinuationUtilitySeam.waitFor({ state: 'visible', timeout: 20000 })
  await sameSourceContinuationUtilitySeam.scrollIntoViewIfNeeded()
  await page.waitForTimeout(350)
  const focusedGraphSameSourceContinuationUtilitySeamNarrowTop = await captureCurrentViewportScreenshot(
    page,
    outputDir,
    'stage308-focused-graph-same-source-continuation-utility-seam-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage308-focused-study-narrow-top.png')
  ...
  const focusedStudyAnswerShownNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage308-focused-study-answer-shown-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage308-reader-narrow-top.png')

  await writeFile(
    path.join(outputDir, 'stage308-post-stage307-narrower-width-focused-graph-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'narrower-width focused Graph same-source continuation utility-seam audit beside live reading',
          ...
          'neighboring focused Notes, Study, and Reader stability after the focused Graph same-source continuation utility-seam follow-up',
        ],
        baseUrl,
        ...
        captures: {
          ...
          focusedGraphSameSourceContinuationUtilitySeamNarrowTop,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage308-post-stage307-narrower-width-focused-graph-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
