...
const outputDir = process.env.RECALL_STAGE629_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE629_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE629_BASE_URL ?? 'http://172.27.18.251:8002'
const headless = process.env.RECALL_STAGE629_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE629_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE629_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
    path.join(
      outputDir,
      'stage629-home-lower-card-title-tail-balance-and-source-chip-cadence-refinement-reset-after-stage628-failure.png',
    ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeImplementation(page, outputDir, 'stage629')

  await writeFile(
    path.join(
      outputDir,
      'stage629-home-lower-card-title-tail-balance-and-source-chip-cadence-refinement-reset-after-stage628-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        implementationFocus: [
          ...
          'The next polish pass should reduce the remaining March 25, 2026 Recall homepage mismatch by balancing the lower title tail a little more evenly and making the source/chip cadence feel slightly steadier at page scale.',
          'Shell, rail, toolbar, column cadence, and source-aware poster variants should stay intact while the lower copy stack gains one subtler follow-through pass.',
        ],
...
async function captureHomeImplementation(page, directory, stagePrefix) {
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 629 card'),
    readMeasuredBox(preview, 'Stage 629 card preview'),
    readMeasuredBox(copy, 'Stage 629 card copy block'),
    readMeasuredBox(title, 'Stage 629 card title'),
    readMeasuredBox(sourceRow, 'Stage 629 card source row'),
    readMeasuredBox(chip, 'Stage 629 card chip'),
  ])
  ...
  const [titleStyles, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 629 Home canvas, day header, and grid.')
      }
  ...
  if (!(titleStyles.lineHeightPx > stage627TitleLineHeightPxBaseline && titleStyles.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 627 baseline, found ${titleStyles.lineHeightPx}.`)
  }
  ...
  if (!(representativeCardBox.height <= cardHeightPxCeiling)) {
    throw new Error(`Expected representative card height to stay compact after the Stage 629 pass, found ${representativeCardBox.height}.`)
  }
...
async function readTextNodeState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible text content for the measured Stage 629 node.')
  }
  return {
...
async function readChipState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible chip text for the measured Stage 629 chip.')
  }
  return {
    alpha: parseColorAlpha(styles.color),
