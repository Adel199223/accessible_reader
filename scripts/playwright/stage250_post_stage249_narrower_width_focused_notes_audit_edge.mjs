...
const outputDir = process.env.RECALL_STAGE250_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE250_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE250_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE250_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE250_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE250_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage250-post-stage249-narrower-width-focused-notes-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage250-focused-overview-narrow-top.png',
  )
  ...
  const focusedNotesEmptyNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage250-focused-notes-empty-narrow-top.png',
  )
  ...
  const focusedNotesDrawerOpenNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage250-focused-notes-drawer-open-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const focusedGraphNarrowTop = await captureTopScreenshot(page, outputDir, 'stage250-focused-graph-narrow-top.png')
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureTopScreenshot(page, outputDir, 'stage250-focused-study-narrow-top.png')
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureTopScreenshot(page, outputDir, 'stage250-reader-narrow-top.png')

  await writeFile(
    path.join(outputDir, 'stage250-post-stage249-narrower-width-focused-notes-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'narrower-width focused Notes empty-detail lane collapse beside live reading',
          'retention of the Stage 223 shell correction and Stage 227 focused-source strip compression',
          'retention of the Stage 225 Reader gains, Stage 245 focused Graph gains, and Stage 247 focused Study gains',
          'neighboring focused Graph and Study stability after the focused Notes empty-detail pass',
        ],
        baseUrl,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage250-post-stage249-narrower-width-focused-notes-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
