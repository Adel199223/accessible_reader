...
const outputDir = process.env.RECALL_STAGE626_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE626_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE626_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE626_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE626_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE626_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const desktopViewport = { width: 1600, height: 1080 }
const stage623PreviewRatioBaseline = 0.6271017274472169
const stage623CopyRatioBaseline = 0.26249520153550865
const stage623TitleLineHeightPxBaseline = 16.7796
const stage623TitleToSourceGapPxBaseline = 3.265625
const stage623SourceToChipGapPxBaseline = 2.46875
...
const previewRatioCeiling = 0.626
const copyRatioFloor = 0.264
const titleLineHeightPxFloor = 16.95
const titleToSourceGapPxFloor = 3.35
const sourceToChipGapPxFloor = 2.55
const sourceFontSizePxFloor = 10.3
const chipFontSizePxFloor = 7.58
const sourceAlphaFloor = 0.66
const chipAlphaFloor = 0.49
...
await rm(
  path.join(
    outputDir,
    'stage626-post-stage625-home-lower-card-title-source-chip-hierarchy-refinement-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage626-post-stage625-home-lower-card-title-source-chip-hierarchy-refinement-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeAudit(page, outputDir, 'stage626')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage626-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const readerEvidence = await openOriginalReaderFromHome(readerPage, outputDir, 'stage626')

  await writeFile(
    path.join(
      outputDir,
      'stage626-post-stage625-home-lower-card-title-source-chip-hierarchy-refinement-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 625 lower-card title/source/chip hierarchy refinement pass',
          'the Home surface should keep the Stage 563 structure while the lower title/source/chip seam reads slightly steadier than the Stage 623 balance',
          'Graph and original-only Reader remain regression surfaces only after the Home-only Stage 625 pass',
        ],
        baseUrl,
...
async function captureHomeAudit(page, directory, stagePrefix) {
  ...
  const representativeCard = dayGroup.locator('.recall-home-parity-card-stage625').first()
  const preview = representativeCard.locator('.recall-home-parity-card-preview-stage625').first()
  const copy = representativeCard.locator('.recall-home-parity-card-copy-stage625').first()
  const title = representativeCard.locator('.recall-home-parity-card-title-stage625').first()
  const sourceRow = representativeCard.locator('.recall-home-parity-card-source-row-stage625').first()
  const source = representativeCard.locator('.recall-home-parity-card-source-stage625').first()
  const chip = representativeCard.locator('.recall-home-parity-card-chip-stage625').first()
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 626 card'),
    readMeasuredBox(preview, 'Stage 626 card preview'),
    readMeasuredBox(copy, 'Stage 626 card copy block'),
    readMeasuredBox(title, 'Stage 626 card title'),
    readMeasuredBox(sourceRow, 'Stage 626 card source row'),
    readMeasuredBox(chip, 'Stage 626 card chip'),
  ])
  ...
  const [titleState, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 626 Home canvas, day header, and grid.')
      }
  ...
  if (!(previewRatio < stage623PreviewRatioBaseline && previewRatio <= previewRatioCeiling)) {
    throw new Error(`Expected preview ratio below the Stage 623 baseline and within the target band, found ${previewRatio}.`)
  }
  if (!(copyRatio > stage623CopyRatioBaseline && copyRatio >= copyRatioFloor)) {
    throw new Error(`Expected copy ratio above the Stage 623 baseline and within the target band, found ${copyRatio}.`)
  }
  if (!(titleState.lineHeightPx > stage623TitleLineHeightPxBaseline && titleState.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 623 baseline, found ${titleState.lineHeightPx}.`)
  }
  if (!(titleToSourceGapPx > stage623TitleToSourceGapPxBaseline && titleToSourceGapPx >= titleToSourceGapPxFloor)) {
    throw new Error(`Expected title-to-source gap above the Stage 623 baseline, found ${titleToSourceGapPx}.`)
  }
  if (!(sourceToChipGapPx > stage623SourceToChipGapPxBaseline && sourceToChipGapPx >= sourceToChipGapPxFloor)) {
    throw new Error(`Expected source-to-chip gap above the Stage 623 baseline, found ${sourceToChipGapPx}.`)
  }
  if (!(sourceState.fontSizePx >= sourceFontSizePxFloor && sourceState.alpha >= sourceAlphaFloor)) {
    throw new Error(`Expected source-row legibility to stay at or above the Stage 623 band, found ${sourceState.fontSizePx}px and alpha ${sourceState.alpha}.`)
  }
  if (!(chipState.fontSizePx >= chipFontSizePxFloor && chipState.alpha >= chipAlphaFloor)) {
    throw new Error(`Expected chip legibility to stay at or above the Stage 623 band, found ${chipState.fontSizePx}px and alpha ${chipState.alpha}.`)
  }
  ...
  const webCard = canvas.locator('.recall-home-parity-card-stage625').first()
...
async function openOriginalReaderFromHome(page, directory, stagePrefix) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not keep Reader in Original mode for the Stage 626 audit.')
  }
...
async function readTextNodeState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible text content for the measured Stage 626 node.')
  }
  return {
...
async function readChipState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible chip text for the measured Stage 626 chip.')
  }
  return {
    alpha: parseColorAlpha(styles.color),
