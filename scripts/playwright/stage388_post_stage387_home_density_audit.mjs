...
const outputDir = process.env.RECALL_STAGE388_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE388_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE388_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE388_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE388_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE388_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage388-post-stage387-home-density-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage388-home-wide-top.png')
  const homeControlSeamWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage388-home-control-seam-wide-top.png',
    320,
  )
  const homeBrowseStripWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage388-home-browse-strip-wide-top.png',
    980,
  )
  const homePrimaryFlowWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage388-home-primary-flow-wide-top.png',
    980,
  )
  const homeLowerSavedSourceContinuationWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage388-home-lower-saved-source-continuation-wide-top.png',
    980,
    'locator',
  )
  ...
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage388-graph-wide-top.png')
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopReaderPage,
    outputDir,
    'stage388-reader-original-wide-top.png',
  )
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage388-focused-overview-narrow-top.png',
  )

  await writeFile(
    path.join(outputDir, 'stage388-post-stage387-home-density-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is judged first after the Stage 387 density reset',
          'Home-specific crops cover the slimmer top seam, attached browse strip, denser primary flow, and lower continuation lane',
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage388-post-stage387-home-density-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (desktopReaderPage && desktopReaderPage !== desktopPage) {
    await desktopReaderPage
      .screenshot({
        path: path.join(outputDir, 'stage388-post-stage387-home-density-audit-failure-reader.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (focusedPage && focusedPage !== desktopPage) {
    await focusedPage
      .screenshot({
        path: path.join(outputDir, 'stage388-post-stage387-home-density-audit-failure-focused.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 388 audit.')
  }
