...
const outputDir = process.env.RECALL_STAGE654_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE654_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE654_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE654_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE654_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE654_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage654-post-stage653-home-poster-differentiation-and-parity-closeout-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage654-post-stage653-home-poster-differentiation-and-parity-closeout-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeAudit(page, outputDir, 'stage654')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage654-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const readerEvidence = await openOriginalReaderFromHome(readerPage, outputDir, 'stage654')

  await writeFile(
    path.join(
      outputDir,
      'stage654-post-stage653-home-poster-differentiation-and-parity-closeout-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 653 poster-differentiation and parity-closeout pass',
          'the Home surface should keep the Stage 563 structure while poster previews read less blank and less samey at page scale without giving back the settled lower title/source/chip seam',
          'Graph and original-only Reader remain regression surfaces only after the Home-only Stage 653 pass',
        ],
        baseUrl,
...
async function captureHomeAudit(page, directory, stagePrefix) {
  ...
  const preview = representativeCard.locator('.recall-home-parity-card-preview-stage653').first()
  const previewHero = representativeCard.locator('.recall-home-parity-card-preview-hero-stage653').first()
  ...
  await Promise.all([
    ...
    previewHero.waitFor({ state: 'visible', timeout: 20000 }),
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 654 card'),
    readMeasuredBox(preview, 'Stage 654 card preview'),
    readMeasuredBox(copy, 'Stage 654 card copy block'),
    readMeasuredBox(title, 'Stage 654 card title'),
    readMeasuredBox(sourceRow, 'Stage 654 card source row'),
    readMeasuredBox(chip, 'Stage 654 card chip'),
  ])

  const [titleState, sourceState, chipState, previewHeroState, canvasMetrics] = await Promise.all([
    readTitleState(title),
    readTextNodeState(source),
    readChipState(chip),
    readTextNodeState(previewHero),
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 654 Home canvas, day header, and grid.')
      }
  ...
  if (!(titleState.lineHeightPx > stage649TitleLineHeightPxBaseline && titleState.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 649 baseline, found ${titleState.lineHeightPx}.`)
  }
  ...
  if (!previewHeroState.text.trim().length) {
    throw new Error('Expected a visible Stage 654 preview hero text node in the representative Home card.')
  }
  ...
  const firstDayPreviewGlows = await dayGroup.locator('.recall-home-parity-card-preview-stage653').evaluateAll((nodes) =>
    nodes.slice(0, 3).map((node) => getComputedStyle(node).getPropertyValue('--recall-home-preview-glow-stage653').trim()),
  )
  const distinctPreviewGlowCount = new Set(firstDayPreviewGlows.filter(Boolean)).size
  if (distinctPreviewGlowCount < 2) {
    throw new Error(`Expected at least two distinct Stage 654 preview accent glows in the first day group, found ${distinctPreviewGlowCount}.`)
  }
  ...
  const homeFileCardCapture = await captureLocatorScreenshot(page, fileCard, directory, `${stagePrefix}-home-file-card.png`)

  return {
    captures: {
      homeFileCard: homeFileCardCapture,
...
async function openOriginalReaderFromHome(page, directory, stagePrefix) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not keep Reader in Original mode for the Stage 654 audit.')
  }
...
async function readTextNodeState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible text content for the measured Stage 654 node.')
  }
  return {
...
async function readChipState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible chip text for the measured Stage 654 chip.')
  }
  return {
    alpha: parseColorAlpha(styles.color),
