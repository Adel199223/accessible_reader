...
const outputDir = process.env.RECALL_STAGE353_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE353_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE353_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE353_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE353_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE353_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(
  path.join(
    outputDir,
    'stage353-bundled-narrower-width-focused-graph-follow-on-text-wall-reset-after-stage352-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage353-focused-overview-narrow-top.png',
  )
  ...
  const focusedGraphNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage353-focused-graph-narrow-top.png')
  ...
  const focusedGraphNodeDetailRailNarrowTop = await captureLocatorTopScreenshot(
    ...
    graphFocusedDetail,
    outputDir,
    'stage353-focused-graph-node-detail-rail-narrow-top.png',
    920,
  )
  const focusedGraphFollowOnTextWallResetNarrowPanelTop = await captureLocatorBottomScreenshot(
    ...
    followOnBundle,
    outputDir,
    'stage353-focused-graph-follow-on-text-wall-reset-narrow-panel-top.png',
    560,
  )
  const focusedGraphLeadingEvidenceMetaDensityNarrowPanelTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage353-focused-graph-leading-evidence-meta-density-narrow-panel-top.png',
    500,
  )
  const focusedGraphFollowOnBundleNarrowPanelTop = await captureLocatorTopScreenshot(
    ...
    followOnBundle,
    outputDir,
    'stage353-focused-graph-follow-on-bundle-narrow-panel-top.png',
    760,
  )
  const focusedGraphRelationsFollowOnNarrowPanelTop =
    (await relationsFollowOn.count()) > 0
      ? await captureLocatorTopScreenshot(
          ...
          relationsFollowOn,
          outputDir,
          'stage353-focused-graph-relations-follow-on-narrow-panel-top.png',
          760,
        )
      : null
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage353-focused-study-narrow-top.png')
  ...
  const focusedStudyAnswerShownNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage353-focused-study-answer-shown-narrow-top.png',
  )
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage353-focused-notes-drawer-open-empty-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage353-reader-narrow-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage353-bundled-narrower-width-focused-graph-follow-on-text-wall-reset-after-stage352-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
