...
const outputDir = process.env.RECALL_STAGE639_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE639_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE639_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE639_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE639_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE639_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
    path.join(
      outputDir,
      'stage639-home-lower-card-copy-seam-and-title-source-chip-descent-finish-reset-after-stage638-failure.png',
    ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeImplementation(page, outputDir, 'stage639')

  await writeFile(
    path.join(
      outputDir,
      'stage639-home-lower-card-copy-seam-and-title-source-chip-descent-finish-reset-after-stage638-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        implementationFocus: [
          ...
          'The next polish pass should reduce the remaining March 25, 2026 Recall homepage mismatch by finishing one subtler lower-card copy-seam and title/source/chip descent step beyond the Stage 637 follow-through state.',
          'Shell, rail, toolbar, column cadence, and source-aware poster variants should stay intact while the lower copy stack gains a slightly fuller share and an even calmer descending cadence.',
        ],
...
async function captureHomeImplementation(page, directory, stagePrefix) {
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 639 card'),
    readMeasuredBox(preview, 'Stage 639 card preview'),
    readMeasuredBox(copy, 'Stage 639 card copy block'),
    readMeasuredBox(title, 'Stage 639 card title'),
    readMeasuredBox(sourceRow, 'Stage 639 card source row'),
    readMeasuredBox(chip, 'Stage 639 card chip'),
  ])
  ...
  const [titleStyles, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 639 Home canvas, day header, and grid.')
      }
  ...
  if (!(titleStyles.lineHeightPx > stage637TitleLineHeightPxBaseline && titleStyles.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 637 baseline, found ${titleStyles.lineHeightPx}.`)
  }
  ...
  if (!(representativeCardBox.height <= cardHeightPxCeiling)) {
    throw new Error(`Expected representative card height to stay compact after the Stage 637 pass, found ${representativeCardBox.height}.`)
  }
