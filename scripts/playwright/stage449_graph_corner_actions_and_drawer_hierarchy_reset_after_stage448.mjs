...
const outputDir = process.env.RECALL_STAGE449_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE449_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE449_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE449_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE449_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE449_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage449-graph-corner-actions-and-drawer-hierarchy-reset-after-stage448-failure.png'), {
  force: true,
})
...
try {
  ...
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage449-graph-wide-top.png')
  const graphLauncherPodWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage449-graph-launcher-pod-wide-top.png',
  )
  const graphSearchActionCornerWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage449-graph-search-action-corner-wide-top.png',
  )
  ...
  const graphToolsDrawerWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage449-graph-tools-drawer-wide-top.png',
  )
  ...
  const graphWorkingTrailWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage449-graph-working-trail-wide-top.png',
    'locator',
  )
  ...
  const graphDetailDockPeekWideTop = await captureLocatorTopScreenshot(
    ...
    graphDetailDock,
    outputDir,
    'stage449-graph-detail-dock-peek-wide-top.png',
    'locator',
  )
  ...
  const graphDetailDockExpandedWideTop = await captureLocatorTopScreenshot(
    ...
    graphDetailDock,
    outputDir,
    'stage449-graph-detail-dock-expanded-wide-top.png',
    'locator',
  )

  await writeFile(
    path.join(outputDir, 'stage449-graph-corner-actions-and-drawer-hierarchy-reset-after-stage448-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        ...
        runtimeBrowser,
        validationFocus: [
          'wide desktop Graph judged first after the Stage 448 checkpoint',
          'evidence of a truer launcher-only pod, a tighter search-action corner, and a slimmer left tools drawer over the canvas-first graph',
          'the working trail plus peek and expanded detail drawer states confirm that selected-node context moved down and right instead of back into the top seam',
        ],
      },
      null,
      2,
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage449-graph-corner-actions-and-drawer-hierarchy-reset-after-stage448-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
...
async function selectFirstGraphNode(page) {
  ...
  if (bestIndex < 0) {
    throw new Error('Could not find a visible graph node to select during the Stage 449 run.')
  }
