...
const outputDir = process.env.RECALL_STAGE636_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE636_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE636_BASE_URL ?? 'http://172.27.18.251:8002'
const headless = process.env.RECALL_STAGE636_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE636_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE636_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const desktopViewport = { width: 1600, height: 1080 }
const stage633PreviewRatioBaseline = 0.5989251439539347
const stage633CopyRatioBaseline = 0.2852975047984645
const stage633TitleLineHeightPxBaseline = 18.2998
const stage633TitleToSourceGapPxBaseline = 4.4375
const stage633SourceToChipGapPxBaseline = 3.734375
...
const previewRatioCeiling = 0.5984
const copyRatioFloor = 0.286
const titleLineHeightPxFloor = 18.45
const titleToSourceGapPxFloor = 4.5
const sourceToChipGapPxFloor = 3.8
const sourceFontSizePxFloor = 10.47
const chipFontSizePxFloor = 7.79
const sourceAlphaFloor = 0.72
const chipAlphaFloor = 0.55
...
await rm(
    path.join(
      outputDir,
      'stage636-post-stage635-home-lower-card-title-source-chip-descent-finish-and-copy-seam-settling-audit-failure.png',
    ),
  { force: true },
)
await rm(
    path.join(
      outputDir,
      'stage636-post-stage635-home-lower-card-title-source-chip-descent-finish-and-copy-seam-settling-audit-failure-reader.png',
    ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeAudit(page, outputDir, 'stage636')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage636-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const readerEvidence = await openOriginalReaderFromHome(readerPage, outputDir, 'stage636')

  await writeFile(
    path.join(
      outputDir,
      'stage636-post-stage635-home-lower-card-title-source-chip-descent-finish-and-copy-seam-settling-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 635 lower-card title/source/chip descent finish and copy-seam-settling pass',
          'the Home surface should keep the Stage 563 structure while the lower hierarchy reads slightly cleaner than the Stage 633 baseline across title, source row, and collection chip',
          'Graph and original-only Reader remain regression surfaces only after the Home-only Stage 635 pass',
        ],
        baseUrl,
...
async function captureHomeAudit(page, directory, stagePrefix) {
  ...
  const representativeCard = dayGroup.locator('.recall-home-parity-card-stage635').first()
  const preview = representativeCard.locator('.recall-home-parity-card-preview-stage635').first()
  const copy = representativeCard.locator('.recall-home-parity-card-copy-stage635').first()
  const title = representativeCard.locator('.recall-home-parity-card-title-stage635').first()
  const sourceRow = representativeCard.locator('.recall-home-parity-card-source-row-stage635').first()
  const source = representativeCard.locator('.recall-home-parity-card-source-stage635').first()
  const chip = representativeCard.locator('.recall-home-parity-card-chip-stage635').first()
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 636 card'),
    readMeasuredBox(preview, 'Stage 636 card preview'),
    readMeasuredBox(copy, 'Stage 636 card copy block'),
    readMeasuredBox(title, 'Stage 636 card title'),
    readMeasuredBox(sourceRow, 'Stage 636 card source row'),
    readMeasuredBox(chip, 'Stage 636 card chip'),
  ])
  ...
  const [titleState, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 636 Home canvas, day header, and grid.')
      }
  ...
  if (!(previewRatio < stage633PreviewRatioBaseline && previewRatio <= previewRatioCeiling)) {
    throw new Error(`Expected preview ratio below the Stage 633 baseline and within the target band, found ${previewRatio}.`)
  }
  if (!(copyRatio > stage633CopyRatioBaseline && copyRatio >= copyRatioFloor)) {
    throw new Error(`Expected copy ratio above the Stage 633 baseline and within the target band, found ${copyRatio}.`)
  }
  if (!(titleState.lineHeightPx > stage633TitleLineHeightPxBaseline && titleState.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 633 baseline, found ${titleState.lineHeightPx}.`)
  }
  if (!(titleToSourceGapPx > stage633TitleToSourceGapPxBaseline && titleToSourceGapPx >= titleToSourceGapPxFloor)) {
    throw new Error(`Expected title-to-source gap above the Stage 633 baseline, found ${titleToSourceGapPx}.`)
  }
  if (!(sourceToChipGapPx > stage633SourceToChipGapPxBaseline && sourceToChipGapPx >= sourceToChipGapPxFloor)) {
    throw new Error(`Expected source-to-chip gap above the Stage 633 baseline, found ${sourceToChipGapPx}.`)
  }
  if (!(sourceState.fontSizePx >= sourceFontSizePxFloor && sourceState.alpha >= sourceAlphaFloor)) {
    throw new Error(`Expected source-row legibility to stay at or above the Stage 633 band, found ${sourceState.fontSizePx}px and alpha ${sourceState.alpha}.`)
  }
  if (!(chipState.fontSizePx >= chipFontSizePxFloor && chipState.alpha >= chipAlphaFloor)) {
    throw new Error(`Expected chip legibility to stay at or above the Stage 633 band, found ${chipState.fontSizePx}px and alpha ${chipState.alpha}.`)
  }
  ...
  const webCard = canvas.locator('.recall-home-parity-card-stage635').first()
...
async function openOriginalReaderFromHome(page, directory, stagePrefix) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not keep Reader in Original mode for the Stage 636 audit.')
  }
