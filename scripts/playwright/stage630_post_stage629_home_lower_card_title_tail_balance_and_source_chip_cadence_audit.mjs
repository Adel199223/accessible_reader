...
const outputDir = process.env.RECALL_STAGE630_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE630_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE630_BASE_URL ?? 'http://172.27.18.251:8002'
const headless = process.env.RECALL_STAGE630_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE630_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE630_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const desktopViewport = { width: 1600, height: 1080 }
const stage627PreviewRatioBaseline = 0.61489443378119
const stage627CopyRatioBaseline = 0.2726295585412668
const stage627TitleLineHeightPxBaseline = 17.4571
const stage627TitleToSourceGapPxBaseline = 3.796875
const stage627SourceToChipGapPxBaseline = 3.03125
...
const previewRatioCeiling = 0.6145
const copyRatioFloor = 0.273
const titleLineHeightPxFloor = 17.6
const titleToSourceGapPxFloor = 3.9
const sourceToChipGapPxFloor = 3.08
const sourceFontSizePxFloor = 10.42
const chipFontSizePxFloor = 7.7
const sourceAlphaFloor = 0.68
const chipAlphaFloor = 0.51
...
await rm(
    path.join(
      outputDir,
      'stage630-post-stage629-home-lower-card-title-tail-balance-and-source-chip-cadence-audit-failure.png',
    ),
  { force: true },
)
await rm(
    path.join(
      outputDir,
      'stage630-post-stage629-home-lower-card-title-tail-balance-and-source-chip-cadence-audit-failure-reader.png',
    ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeAudit(page, outputDir, 'stage630')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage630-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const readerEvidence = await openOriginalReaderFromHome(readerPage, outputDir, 'stage630')

  await writeFile(
    path.join(
      outputDir,
      'stage630-post-stage629-home-lower-card-title-tail-balance-and-source-chip-cadence-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 629 lower-card title-tail balance and source/chip cadence refinement pass',
          'the Home surface should keep the Stage 563 structure while the lower title tail reads a little more even and the source/chip cadence settles slightly more cleanly than the Stage 627 baseline',
          'Graph and original-only Reader remain regression surfaces only after the Home-only Stage 629 pass',
        ],
        baseUrl,
...
async function captureHomeAudit(page, directory, stagePrefix) {
  ...
  const representativeCard = dayGroup.locator('.recall-home-parity-card-stage629').first()
  const preview = representativeCard.locator('.recall-home-parity-card-preview-stage629').first()
  const copy = representativeCard.locator('.recall-home-parity-card-copy-stage629').first()
  const title = representativeCard.locator('.recall-home-parity-card-title-stage629').first()
  const sourceRow = representativeCard.locator('.recall-home-parity-card-source-row-stage629').first()
  const source = representativeCard.locator('.recall-home-parity-card-source-stage629').first()
  const chip = representativeCard.locator('.recall-home-parity-card-chip-stage629').first()
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 630 card'),
    readMeasuredBox(preview, 'Stage 630 card preview'),
    readMeasuredBox(copy, 'Stage 630 card copy block'),
    readMeasuredBox(title, 'Stage 630 card title'),
    readMeasuredBox(sourceRow, 'Stage 630 card source row'),
    readMeasuredBox(chip, 'Stage 630 card chip'),
  ])
  ...
  const [titleState, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 630 Home canvas, day header, and grid.')
      }
  ...
  if (!(previewRatio < stage627PreviewRatioBaseline && previewRatio <= previewRatioCeiling)) {
    throw new Error(`Expected preview ratio below the Stage 627 baseline and within the target band, found ${previewRatio}.`)
  }
  if (!(copyRatio > stage627CopyRatioBaseline && copyRatio >= copyRatioFloor)) {
    throw new Error(`Expected copy ratio above the Stage 627 baseline and within the target band, found ${copyRatio}.`)
  }
  if (!(titleState.lineHeightPx > stage627TitleLineHeightPxBaseline && titleState.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 627 baseline, found ${titleState.lineHeightPx}.`)
  }
  if (!(titleToSourceGapPx > stage627TitleToSourceGapPxBaseline && titleToSourceGapPx >= titleToSourceGapPxFloor)) {
    throw new Error(`Expected title-to-source gap above the Stage 627 baseline, found ${titleToSourceGapPx}.`)
  }
  if (!(sourceToChipGapPx > stage627SourceToChipGapPxBaseline && sourceToChipGapPx >= sourceToChipGapPxFloor)) {
    throw new Error(`Expected source-to-chip gap above the Stage 627 baseline, found ${sourceToChipGapPx}.`)
  }
  if (!(sourceState.fontSizePx >= sourceFontSizePxFloor && sourceState.alpha >= sourceAlphaFloor)) {
    throw new Error(`Expected source-row legibility to stay at or above the Stage 627 band, found ${sourceState.fontSizePx}px and alpha ${sourceState.alpha}.`)
  }
  if (!(chipState.fontSizePx >= chipFontSizePxFloor && chipState.alpha >= chipAlphaFloor)) {
    throw new Error(`Expected chip legibility to stay at or above the Stage 627 band, found ${chipState.fontSizePx}px and alpha ${chipState.alpha}.`)
  }
  ...
  const webCard = canvas.locator('.recall-home-parity-card-stage629').first()
...
async function openOriginalReaderFromHome(page, directory, stagePrefix) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not keep Reader in Original mode for the Stage 630 audit.')
  }
...
async function readTextNodeState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible text content for the measured Stage 630 node.')
  }
  return {
...
async function readChipState(locator) {
  ...
  if (!text) {
    throw new Error('Expected visible chip text for the measured Stage 630 chip.')
  }
  return {
    alpha: parseColorAlpha(styles.color),
