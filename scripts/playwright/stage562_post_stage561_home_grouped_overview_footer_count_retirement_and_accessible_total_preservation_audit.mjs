...
const outputDir = process.env.RECALL_STAGE562_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE562_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE562_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE562_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE562_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE562_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage562-post-stage561-home-grouped-overview-footer-count-retirement-and-accessible-total-preservation-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage562-post-stage561-home-grouped-overview-footer-count-retirement-and-accessible-total-preservation-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeOverviewEvidence = await captureHomeOverviewEvidence(page, outputDir, 'stage562')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage562-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const { sourceTitle } = await openOriginalReaderFromHome(readerPage)
  const readerOriginalWideTop = await captureViewportScreenshot(readerPage, outputDir, 'stage562-reader-original-wide-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage562-post-stage561-home-grouped-overview-footer-count-retirement-and-accessible-total-preservation-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 561 grouped-overview footer count reset',
          'the grouped overview should now retire the visible source total from the continuation footer while preserving that total in hidden accessible text',
...
async function captureHomeOverviewEvidence(page, directory, stagePrefix) {
  ...
  if (!primaryMetric || secondaryMetrics.length !== 2) {
    throw new Error('Could not measure the primary lane and secondary stack cards for the Stage 562 audit.')
  }
  ...
  for (const metric of cardMetrics) {
    ...
    if (metric.title === 'Captures') {
      if (
        ...
        throw new Error('The Captures grouped-overview card no longer exposed measurable Stage 562 footer evidence.')
      }
  ...
  if (!overviewBox || !headerBox || !gridBox || !titleRowBox || !titleHeadingBox || !metaBox) {
    throw new Error('Could not measure the grouped-overview shell geometry for the Stage 562 audit.')
  }
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 562 audit.')
  }
  return { sourceTitle }
}
