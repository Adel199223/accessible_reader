...
const outputDir = process.env.RECALL_STAGE499_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE499_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE499_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE499_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE499_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE499_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const desktopViewport = { width: 1600, height: 1080 }

await mkdir(outputDir, { recursive: true })
await rm(
  path.join(outputDir, 'stage499-post-stage498-graph-filter-query-and-visibility-audit-failure.png'),
  { force: true },
)
await rm(
  path.join(outputDir, 'stage499-post-stage498-graph-filter-query-and-visibility-audit-failure-reader.png'),
  { force: true },
)
...
try {
  ...
  const graphSettingsDefaultWideTop = await captureLocatorTopScreenshot(
    ...
    graphRail,
    outputDir,
    'stage499-graph-settings-default-wide-top.png',
  )
  ...
  const graphBoundedQueryWideTop = await captureLocatorTopScreenshot(
    ...
    graphRail,
    outputDir,
    'stage499-graph-bounded-query-wide-top.png',
  )
  ...
  const graphVisibilityControlsWideTop = await captureLocatorTopScreenshot(
    ...
    graphRail,
    outputDir,
    'stage499-graph-visibility-controls-wide-top.png',
  )

  const savedPresetName = `Audit view ${Date.now()}`
  ...
  const graphSavedViewReapplyWideTop = await captureLocatorTopScreenshot(
    ...
    graphRail,
    outputDir,
    'stage499-graph-saved-view-reapply-wide-top.png',
  )

  await desktopPage.goto(`${baseUrl}/recall`, { waitUntil: 'networkidle' })
  await desktopPage.locator('.recall-home-workspace').first().waitFor({ state: 'visible', timeout: 20000 })
  await desktopPage.waitForTimeout(350)
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage499-home-wide-top.png')
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopReaderPage,
    outputDir,
    'stage499-reader-original-wide-top.png',
  )

  await writeFile(
    path.join(outputDir, 'stage499-post-stage498-graph-filter-query-and-visibility-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Graph is audited first after the Stage 498 filter-query and visibility-controls reset',
          'the settings sidebar should now read like a real graph-management surface with discoverable bounded queries and explicit visibility rules',
          'Home and original-only Reader remain regression surfaces, and generated-content Reader views stay explicitly locked out of scope',
        ],
        baseUrl,
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage499-post-stage498-graph-filter-query-and-visibility-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (desktopReaderPage && desktopReaderPage !== desktopPage) {
    await desktopReaderPage
      .screenshot({
        path: path.join(outputDir, 'stage499-post-stage498-graph-filter-query-and-visibility-audit-failure-reader.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 499 audit.')
  }
  await page.waitForTimeout(250)
  return { sourceTitle }
}
...
async function getFirstVisibleGraphNodeLabel(page) {
  ...
  if (!nodeLabel) {
    throw new Error('Could not derive a graph node label for the Stage 499 audit.')
  }
  return nodeLabel
}
