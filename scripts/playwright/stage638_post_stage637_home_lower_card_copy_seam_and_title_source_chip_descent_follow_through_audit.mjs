...
const outputDir = process.env.RECALL_STAGE638_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE638_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE638_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE638_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE638_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE638_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const desktopViewport = { width: 1600, height: 1080 }
const stage635PreviewRatioBaseline = 0.593704414587332
const stage635CopyRatioBaseline = 0.2894433781190019
const stage635TitleLineHeightPxBaseline = 18.6007
const stage635TitleToSourceGapPxBaseline = 4.671875
const stage635SourceToChipGapPxBaseline = 3.953125
...
const previewRatioCeiling = 0.5933
const copyRatioFloor = 0.29
const titleLineHeightPxFloor = 18.75
const titleToSourceGapPxFloor = 4.8
const sourceToChipGapPxFloor = 4.1
const sourceFontSizePxFloor = 10.49
const chipFontSizePxFloor = 7.8
const sourceAlphaFloor = 0.73
const chipAlphaFloor = 0.56
...
await rm(
    path.join(
      outputDir,
      'stage638-post-stage637-home-lower-card-copy-seam-and-title-source-chip-descent-follow-through-audit-failure.png',
    ),
  { force: true },
)
await rm(
    path.join(
      outputDir,
      'stage638-post-stage637-home-lower-card-copy-seam-and-title-source-chip-descent-follow-through-audit-failure-reader.png',
    ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeAudit(page, outputDir, 'stage638')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage638-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const readerEvidence = await openOriginalReaderFromHome(readerPage, outputDir, 'stage638')

  await writeFile(
    path.join(
      outputDir,
      'stage638-post-stage637-home-lower-card-copy-seam-and-title-source-chip-descent-follow-through-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 637 lower-card copy-seam and title/source/chip descent follow-through pass',
          'the Home surface should keep the Stage 563 structure while the lower hierarchy reads slightly cleaner than the Stage 635 baseline across title, source row, and collection chip',
          'Graph and original-only Reader remain regression surfaces only after the Home-only Stage 637 pass',
        ],
        baseUrl,
...
async function captureHomeAudit(page, directory, stagePrefix) {
  ...
  const representativeCard = dayGroup.locator('.recall-home-parity-card-stage637').first()
  const preview = representativeCard.locator('.recall-home-parity-card-preview-stage637').first()
  const copy = representativeCard.locator('.recall-home-parity-card-copy-stage637').first()
  const title = representativeCard.locator('.recall-home-parity-card-title-stage637').first()
  const sourceRow = representativeCard.locator('.recall-home-parity-card-source-row-stage637').first()
  const source = representativeCard.locator('.recall-home-parity-card-source-stage637').first()
  const chip = representativeCard.locator('.recall-home-parity-card-chip-stage637').first()
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 638 card'),
    readMeasuredBox(preview, 'Stage 638 card preview'),
    readMeasuredBox(copy, 'Stage 638 card copy block'),
    readMeasuredBox(title, 'Stage 638 card title'),
    readMeasuredBox(sourceRow, 'Stage 638 card source row'),
    readMeasuredBox(chip, 'Stage 638 card chip'),
  ])
  ...
  const [titleState, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 638 Home canvas, day header, and grid.')
      }
  ...
  if (!(previewRatio < stage635PreviewRatioBaseline && previewRatio <= previewRatioCeiling)) {
    throw new Error(`Expected preview ratio below the Stage 635 baseline and within the target band, found ${previewRatio}.`)
  }
  if (!(copyRatio > stage635CopyRatioBaseline && copyRatio >= copyRatioFloor)) {
    throw new Error(`Expected copy ratio above the Stage 635 baseline and within the target band, found ${copyRatio}.`)
  }
  if (!(titleState.lineHeightPx > stage635TitleLineHeightPxBaseline && titleState.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 635 baseline, found ${titleState.lineHeightPx}.`)
  }
  if (!(titleToSourceGapPx > stage635TitleToSourceGapPxBaseline && titleToSourceGapPx >= titleToSourceGapPxFloor)) {
    throw new Error(`Expected title-to-source gap above the Stage 635 baseline, found ${titleToSourceGapPx}.`)
  }
  if (!(sourceToChipGapPx > stage635SourceToChipGapPxBaseline && sourceToChipGapPx >= sourceToChipGapPxFloor)) {
    throw new Error(`Expected source-to-chip gap above the Stage 635 baseline, found ${sourceToChipGapPx}.`)
  }
  if (!(sourceState.fontSizePx >= sourceFontSizePxFloor && sourceState.alpha >= sourceAlphaFloor)) {
    throw new Error(`Expected source-row legibility to stay at or above the Stage 635 band, found ${sourceState.fontSizePx}px and alpha ${sourceState.alpha}.`)
  }
  if (!(chipState.fontSizePx >= chipFontSizePxFloor && chipState.alpha >= chipAlphaFloor)) {
    throw new Error(`Expected chip legibility to stay at or above the Stage 635 band, found ${chipState.fontSizePx}px and alpha ${chipState.alpha}.`)
  }
  ...
  const webCard = canvas.locator('.recall-home-parity-card-stage637').first()
