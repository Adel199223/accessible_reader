...
const outputDir = process.env.RECALL_STAGE557_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE557_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE557_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE557_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE557_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE557_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage557-home-grouped-overview-visible-view-count-retirement-and-accessible-meta-preservation-reset-after-stage556-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeOverviewEvidence = await captureHomeOverviewEvidence(page, outputDir, 'stage557')

  await writeFile(
    path.join(
      outputDir,
      'stage557-home-grouped-overview-visible-view-count-retirement-and-accessible-meta-preservation-reset-after-stage556-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        runtimeBrowser,
        validationFocus: [
          'wide desktop Home keeps the grouped overview open while grouped-overview rows retire the visible views suffix from compact metadata',
          'Documents rows should keep format-specific context, accessible labels should preserve available-view-count context, and the compact metadata seam should read lighter without reopening row-height or footer regressions',
...
async function captureHomeOverviewEvidence(page, directory, stagePrefix) {
  ...
  if (!primaryMetric || secondaryMetrics.length !== 2) {
    throw new Error('Could not measure the primary lane and secondary stack cards for the Stage 557 validation.')
  }
  ...
  for (const metric of cardMetrics) {
    ...
    if (metric.title === 'Captures' && metric.footerButtonHeight == null) {
      throw new Error(
        'The Captures grouped-overview card no longer exposed a measurable Show all footer button for the Stage 557 validation.',
      )
    }
  }
  ...
  if (!overviewBox || !headerBox || !gridBox || !titleRowBox || !titleHeadingBox || !metaBox) {
    throw new Error('Could not measure the grouped-overview shell geometry for the Stage 557 validation.')
  }
