...
const outputDir = process.env.RECALL_STAGE363_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE363_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE363_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE363_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE363_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE363_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage363-priority-surface-baseline-consolidation-after-stage362-failure.png'), {
  force: true,
})
...
try {
  ...
  const desktopHomeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage363-home-wide-top.png')
  const homeGlanceWideTop = await captureLocatorTopScreenshot(
    desktopPage,
    desktopPage.getByLabel('Collection snapshot').first(),
    outputDir,
    'stage363-home-glance-wide-top.png',
    720,
  )
  ...
  await desktopPage.waitForTimeout(350)
  const desktopGraphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage363-graph-wide-top.png')
  const graphUtilityRailWideTop = await captureLocatorTopScreenshot(
    desktopPage,
    desktopPage.getByLabel('Graph selector strip').first(),
    outputDir,
    'stage363-graph-utility-rail-wide-top.png',
    920,
  )
  const graphDetailDockWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage363-graph-detail-dock-wide-top.png',
    760,
    'locator',
  )
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage363-focused-overview-narrow-top.png',
  )
  ...
  const focusedGraphNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage363-focused-graph-narrow-top.png',
  )
  ...
  } = await captureFocusedNotesRegressionArtifacts(focusedPage, outputDir, 'stage363')
  ...
  const focusedReaderNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage363-focused-reader-narrow-top.png',
  )

  await writeFile(
    path.join(outputDir, 'stage363-priority-surface-baseline-consolidation-after-stage362-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'bounded Stage 363 priority-surface consolidation review with wide desktop Home, Graph, Reader, and Notes captures first',
          'tight wide-desktop comparison crops for the Home support glance, Graph utility and detail rails, Reader dock, and Notes detail/context rail',
          'focused regressions only for the impacted priority surfaces: overview, Graph, Notes default, Notes drawer-open, and Reader',
          'Study stays frozen and is intentionally omitted from the active Stage 363 comparison harness',
        ],
        baseUrl,
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage363-priority-surface-baseline-consolidation-after-stage362-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (desktopReaderPage && desktopReaderPage !== desktopPage) {
    await desktopReaderPage
      .screenshot({
        path: path.join(outputDir, 'stage363-priority-surface-baseline-consolidation-after-stage362-failure-reader.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (focusedPage && focusedPage !== desktopPage) {
    await focusedPage
      .screenshot({
        path: path.join(outputDir, 'stage363-priority-surface-baseline-consolidation-after-stage362-failure-focused.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
