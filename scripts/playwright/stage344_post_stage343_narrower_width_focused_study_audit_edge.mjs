...
const outputDir = process.env.RECALL_STAGE344_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE344_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE344_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE344_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE344_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE344_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage344-post-stage343-narrower-width-focused-study-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage344-focused-overview-narrow-top.png',
  )
  ...
  const focusedStudyNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage344-focused-study-narrow-top.png')

  const focusedStudyPanel = page.locator('.recall-study-focused-panel-hierarchy-reset').first()
  const focusedStudyBody = page.locator('.recall-study-focused-body-flow-reset').first()
  ...
  const focusedStudyRightLaneNarrowPanelTop = await captureLocatorTopScreenshot(
    ...
    focusedStudyPanel,
    outputDir,
    'stage344-focused-study-right-lane-narrow-panel-top.png',
    760,
  )
  const focusedStudyBodyFlowNarrowPanelTop = await captureLocatorTopScreenshot(
    page,
    focusedStudyBody,
    outputDir,
    'stage344-focused-study-body-flow-narrow-panel-top.png',
    760,
  )
  ...
  const focusedStudyAnswerShownNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage344-focused-study-answer-shown-narrow-top.png',
  )
  const focusedStudyAnswerShownRightLaneNarrowPanelTop = await captureLocatorTopScreenshot(
    page,
    focusedStudyPanel,
    outputDir,
    'stage344-focused-study-answer-shown-right-lane-narrow-panel-top.png',
    760,
  )
  const focusedStudyAnswerShownBodyFlowNarrowPanelTop = await captureLocatorTopScreenshot(
    page,
    focusedStudyBody,
    outputDir,
    'stage344-focused-study-answer-shown-body-flow-narrow-panel-top.png',
    760,
  )
  ...
  await page.waitForTimeout(350)
  const focusedGraphNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage344-focused-graph-narrow-top.png')
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage344-focused-notes-drawer-open-empty-narrow-top.png',
  )
    ...
    JSON.stringify(
      {
        ...
        captures: {
          ...
          focusedStudyAnswerShownBodyFlowNarrowPanelTop,
          focusedStudyAnswerShownNarrowTop,
          focusedStudyAnswerShownRightLaneNarrowPanelTop,
          focusedStudyBodyFlowNarrowPanelTop,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage344-post-stage343-narrower-width-focused-study-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
