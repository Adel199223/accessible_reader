...
const outputDir = process.env.RECALL_STAGE1026_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE1026_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE1026_BASE_URL ?? readCliOption('base-url') ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE1026_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE1026_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE1026_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true

await mkdir(outputDir, { recursive: true })
await rm(path.join(outputDir, 'stage1026-graph-relation-practice-handoffs-failure.png'), { force: true })
...
let harness = null
try {
  harness = await createRelationPracticeReviewHarness({ baseUrl, stageLabel: 'Stage 1026' })
  ...
  const metrics = {
    ...
    cleanupUtilityDryRunMatchedAfterStage1026: null,
    collectionPracticeConnectionStartsSession: false,
    generatedReaderOutputsFrozen: false,
    graphNoPracticeCreateHandoff: false,
    graphNoPracticeStateVisible: false,
    graphPracticedScheduledStateVisible: false,
    graphPracticeConnectionStartsSession: false,
    graphPracticeConnectionVisible: false,
    homePracticedScheduledStateVisible: false,
    homePracticeConnectionStartsSession: false,
    homePracticeConnectionVisible: false,
    relationPracticeRecapReturnFocusesSourceConnection: false,
    relationPracticeRecapReturnOpensSourceConnection: false,
    relationPracticeSessionRecapVisible: false,
    sourcePracticedScheduledStateVisible: false,
  ...
  captures.homePracticeConnection = await captureLocatorScreenshot(
    ...
    homeQueueRow,
    outputDir,
    'stage1026-home-practice-connection.png',
  )
  ...
  captures.collectionPracticeConnection = await captureLocatorScreenshot(
    ...
    collectionQueueRow,
    outputDir,
    'stage1026-collection-practice-connection.png',
  )

  const graphReadyRelationRow = await openGraphRelationRow({
    page,
    selectedRelationEdge,
    selectedRelationLabel,
  })
  const graphPracticeButton = graphReadyRelationRow.getByRole('button', {
    name: `Practice connection: ${selectedRelationLabel}`,
  })
  await graphPracticeButton.waitFor({ state: 'visible', timeout: 20000 })
  metrics.graphPracticeConnectionVisible =
    (await graphReadyRelationRow.getByText(/practice question|practice questions/).isVisible()) &&
    (await graphReadyRelationRow.getByText(/new|due/).isVisible()) &&
    (await graphPracticeButton.isVisible())
  captures.graphPracticeConnection = await captureLocatorScreenshot(
    page,
    graphReadyRelationRow,
    outputDir,
    'stage1026-graph-practice-connection.png',
  )
  ...
  metrics.graphPracticeConnectionStartsSession =
    (await graphActivePrompt.innerText()).includes(homePracticeCard.prompt) &&
  ...
  captures.graphPracticeActiveSession = await captureViewportScreenshot(
    page,
    outputDir,
    'stage1026-graph-practice-active-session.png',
  )
  ...
  captures.homePracticeActiveSession = await captureViewportScreenshot(
    page,
    outputDir,
    'stage1026-home-practice-active-session.png',
  )
  ...
  captures.relationPracticeRecap = await captureViewportScreenshot(
    page,
    outputDir,
    'stage1026-relation-practice-recap.png',
  )
  ...
  metrics.relationPracticeRecapReturnFocusesSourceConnection =
    metrics.relationPracticeRecapReturnOpensSourceConnection &&
    (await returnedRelationRow.getAttribute('data-source-overview-related-graph-focused-stage1024')) === 'true'
  metrics.sourcePracticedScheduledStateVisible =
    (await returnedRelationRow.getByText('Practiced').isVisible()) &&
    (await returnedRelationRow.getByText('Scheduled').isVisible()) &&
    (await returnedRelationRow
      .getByRole('button', { name: `Study questions for relation: ${selectedRelationLabel}` })
      .isVisible())
  captures.relationPracticeReturnToSource = await captureLocatorScreenshot(
    page,
    returnedRelationRow,
    outputDir,
    'stage1026-relation-practice-return-to-source.png',
  )

  await page.goto(`${baseUrl}/recall?section=library`, { waitUntil: 'networkidle' })
  const practicedHomeQueueRow = page
    .getByRole('region', { name: 'Reading queue' })
    .locator('[data-reading-queue-learning-gaps-stage978]')
    .filter({ hasText: harness.primaryDocument.title })
    .first()
  await practicedHomeQueueRow.waitFor({ state: 'visible', timeout: 30000 })
  await practicedHomeQueueRow.getByText(/practiced relation|scheduled relation/).waitFor({
    state: 'visible',
    timeout: 30000,
  })
  const homeStudyConnectionButton = practicedHomeQueueRow.getByRole('button', {
    name: `Study connection for ${harness.primaryDocument.title}`,
  })
  metrics.homePracticedScheduledStateVisible =
    (await practicedHomeQueueRow.getByText(/practiced relation|scheduled relation/).isVisible()) &&
    (await homeStudyConnectionButton.isVisible()) &&
    !(await practicedHomeQueueRow
      .getByRole('button', { name: `Practice connection for ${harness.primaryDocument.title}` })
      .isVisible()
      .catch(() => false))
  captures.homePracticedScheduledState = await captureLocatorScreenshot(
    page,
    practicedHomeQueueRow,
    outputDir,
    'stage1026-home-practiced-scheduled-state.png',
  )
  await homeStudyConnectionButton.click()
  await page.waitForURL(/\/recall\?section=study/, { timeout: 20000 })
  await page.getByLabel('Study questions manager').waitFor({ state: 'visible', timeout: 30000 })

  const graphPracticedRelationRow = await openGraphRelationRow({
    page,
    selectedRelationEdge,
    selectedRelationLabel,
  })
  metrics.graphPracticedScheduledStateVisible =
    (await graphPracticedRelationRow.getByText('Practiced').isVisible()) &&
    (await graphPracticedRelationRow.getByText('Scheduled').isVisible()) &&
    (await graphPracticedRelationRow
      .getByRole('button', { name: `Study questions for connection: ${selectedRelationLabel}` })
      .isVisible()) &&
    !(await graphPracticedRelationRow
      .getByRole('button', { name: `Practice connection: ${selectedRelationLabel}` })
      .isVisible()
      .catch(() => false))
  captures.graphPracticedScheduledState = await captureLocatorScreenshot(
    page,
    graphPracticedRelationRow,
    outputDir,
    'stage1026-graph-practiced-scheduled-state.png',
  )
  ...
  captures.sourcePracticeRelation = await captureLocatorScreenshot(
    ...
    sourceRelationRow,
    outputDir,
    'stage1026-source-practice-relation.png',
  )
  ...
  captures.sourcePracticeActiveSession = await captureViewportScreenshot(
    page,
    outputDir,
    'stage1026-source-practice-active-session.png',
  )

  await deleteHarnessStudyCardsForSource({ baseUrl, sourceDocumentId: harness.primaryDocument.id })
  harness.practiceCards = []
  const graphNoPracticeRelationRow = await openGraphRelationRow({
    page,
    selectedRelationEdge,
    selectedRelationLabel,
  })
  const graphCreatePracticeButton = graphNoPracticeRelationRow.getByRole('button', {
    name: `Create practice card: ${selectedRelationLabel}`,
  })
  await graphCreatePracticeButton.waitFor({ state: 'visible', timeout: 20000 })
  metrics.graphNoPracticeStateVisible =
    (await graphNoPracticeRelationRow.getByText('No practice yet').isVisible()) &&
    (await graphCreatePracticeButton.isVisible()) &&
    !(await graphNoPracticeRelationRow
      .getByRole('button', { name: `Practice connection: ${selectedRelationLabel}` })
      .isVisible()
      .catch(() => false))
  captures.graphNoPracticeState = await captureLocatorScreenshot(
    page,
    graphNoPracticeRelationRow,
    outputDir,
    'stage1026-graph-no-practice-state.png',
  )
  await graphCreatePracticeButton.click()
  await page.waitForURL(/\/recall\?section=study/, { timeout: 20000 })
  const graphCreatedQuestionsManager = page.getByLabel('Study questions manager')
  await graphCreatedQuestionsManager.waitFor({ state: 'visible', timeout: 30000 })
  const graphCreatedCards = await fetchJson(
    `${baseUrl}/api/recall/study/cards?status=all&limit=100&source_document_id=${encodeURIComponent(
      harness.primaryDocument.id,
    )}`,
  )
  const graphCreatedPracticeCard =
    graphCreatedCards.find((card) => card.prompt === buildRelationPracticePrompt(selectedRelationEdge)) ??
    graphCreatedCards.find((card) =>
      (card.source_spans ?? []).some((sourceSpan) => sourceSpan.edge_id === selectedRelationEdge.id),
    ) ??
    graphCreatedCards[0]
  if (graphCreatedPracticeCard) {
    harness.practiceCards.push(graphCreatedPracticeCard)
  }
  metrics.graphNoPracticeCreateHandoff =
    graphCreatedCards.length > 0 &&
    (await page.getByText('Practice card created for this connection.').isVisible()) &&
    (await page.getByText('1 study card').isVisible())
  captures.graphCreatePracticeQuestions = await captureViewportScreenshot(
    page,
    outputDir,
    'stage1026-graph-create-practice-questions.png',
  )

  await cleanupRelationPracticeReviewHarness({ baseUrl, harness })
  harness = null
  const cleanupDryRun = await findRecallNoteAuditArtifacts({ baseUrl })
  metrics.cleanupUtilityDryRunMatchedAfterStage1026 = cleanupDryRun.matchedCount

  for (const [metricName, expected] of Object.entries({
    cleanupUtilityDryRunMatchedAfterStage1026: 0,
    collectionPracticeConnectionStartsSession: true,
    generatedReaderOutputsFrozen: true,
    graphNoPracticeCreateHandoff: true,
    graphNoPracticeStateVisible: true,
    graphPracticedScheduledStateVisible: true,
    graphPracticeConnectionStartsSession: true,
    graphPracticeConnectionVisible: true,
    homePracticedScheduledStateVisible: true,
    homePracticeConnectionStartsSession: true,
    homePracticeConnectionVisible: true,
    relationPracticeRecapReturnFocusesSourceConnection: true,
    relationPracticeRecapReturnOpensSourceConnection: true,
    relationPracticeSessionRecapVisible: true,
    sourcePracticedScheduledStateVisible: true,
    sourcePracticeRelationStartsSession: true,
    sourcePracticeRelationVisible: true,
  })) {
    if (metrics[metricName] !== expected) {
      throw new Error(
        `Stage 1026 expected ${metricName}: ${expected}, got ${metrics[metricName]}. Metrics: ${JSON.stringify(
          metrics,
        )}`,
      )
    }
  ...
  const validation = {
    ...
    metrics,
    runtimeBrowser,
    stage: 'stage1026-graph-relation-practice-handoffs-after-stage1025',
  }
  await writeFile(
    path.join(outputDir, 'stage1026-graph-relation-practice-handoffs-validation.json'),
  ...
  await captureViewportScreenshot(
    page,
    outputDir,
    'stage1026-graph-relation-practice-handoffs-failure.png',
  ).catch(() => null)
  throw error
} finally {
  await cleanupRelationPracticeReviewHarness({ baseUrl, harness }).catch((error) => {
    console.error(`Stage 1026 cleanup failed: ${error instanceof Error ? error.message : String(error)}`)
  })
...
async function openGraphRelationRow({ page, selectedRelationEdge, selectedRelationLabel }) {
  ...
  const sourceRelatedConnections = page.locator('[data-source-overview-related-graph-connections-stage1010="true"]')
  await sourceRelatedConnections.waitFor({ state: 'visible', timeout: 30000 })
  const sourceRelationRow = sourceRelatedConnections
    .locator(
      `[data-source-overview-related-graph-row-stage1010][data-source-overview-related-graph-edge-id-stage1014="${cssAttributeValue(
        selectedRelationEdge.id,
      )}"]`,
    ...
    .first()
  await sourceRelationRow.waitFor({ state: 'visible', timeout: 30000 })
...
async function createRelationPracticeReviewHarness({ baseUrl, stageLabel }) {
  ...
  const collectionId = `collection:stage1026-relation-review-${timestamp}`
...
async function waitForHarnessRelatedEdges({ baseUrl, harness, timeoutMs = 30000 }) {
  ...
  throw new Error(
    `Timed out waiting for Stage 1026 related edges ${harness.sourceLabel} -> ${harness.targetLabel}: ${JSON.stringify(
      lastSnapshot,
    )}`,
  )
}
...
async function insertHarnessRelationPracticeCard({ edge, harness, prompt, suffix }) {
  ...
  const card = {
    ...
    id: `card:stage1026-relation-review:${suffix}:${edge.id.replace(/[^a-zA-Z0-9:_-]/g, '-')}:${Date.now()}`,
    prompt,
    scheduling_state: {
