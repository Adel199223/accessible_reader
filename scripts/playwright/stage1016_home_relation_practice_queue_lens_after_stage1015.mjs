...
const outputDir = process.env.RECALL_STAGE1016_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE1016_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE1016_BASE_URL ?? readCliOption('base-url') ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE1016_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE1016_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE1016_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true

await mkdir(outputDir, { recursive: true })
await rm(path.join(outputDir, 'stage1016-home-relation-practice-queue-lens-failure.png'), { force: true })
...
let harness = null
try {
  harness = await createRelationPracticeQueueHarness({ baseUrl, stageLabel: 'Stage 1016' })
  ...
  const metrics = {
    cleanupUtilityDryRunMatchedAfterStage1016: null,
    customCollectionRelationPracticeSignalVisible: false,
    generatedReaderOutputsFrozen: false,
    homePracticeConnectionHandoffFiltersStudy: false,
    homePracticeConnectionSignalVisible: false,
    relationPracticeClearRestoresSourceQuestions: false,
    sourceOverviewRelationPracticePreserved: false,
  }
  const captures = {}
  ...
  await primaryQueueRow.getByRole('button', { name: `Practice connection for ${harness.primaryDocument.title}` }).waitFor({
    state: 'visible',
    timeout: 20000,
  })
  metrics.homePracticeConnectionSignalVisible =
    (await primaryQueueRow.getByText(/practice relation/).isVisible()) &&
    (await primaryQueueRow.getByText(/new/).isVisible()) &&
    (await primaryQueueRow.getByRole('button', { name: `Practice connection for ${harness.primaryDocument.title}` }).isVisible())
  captures.homeRelationPracticeQueueSignal = await captureLocatorScreenshot(
    page,
    primaryQueueRow,
    outputDir,
    'stage1016-home-relation-practice-queue-signal.png',
  )
  ...
  await collectionQueueRow.getByText(/practice relation/).waitFor({ state: 'visible', timeout: 30000 })
  metrics.customCollectionRelationPracticeSignalVisible =
    (await collectionQueueRow.getByText(/practice relation/).isVisible()) &&
    (await collectionQueueRow.getByRole('button', { name: `Practice connection for ${harness.primaryDocument.title}` }).isVisible())
  captures.customCollectionRelationPracticeQueueSignal = await captureLocatorScreenshot(
    page,
    collectionQueueRow,
    outputDir,
    'stage1016-custom-collection-relation-practice-queue-signal.png',
  )

  await collectionQueueRow.getByRole('button', { name: `Practice connection for ${harness.primaryDocument.title}` }).click()
  ...
  selectedRelationEdge =
    harness.edges.find((edge) => relationFilterText.includes(formatGraphRelationLabel(edge))) ?? null
  selectedRelationLabel = selectedRelationEdge ? formatGraphRelationLabel(selectedRelationEdge) : null
  if (!selectedRelationEdge || !selectedRelationLabel) {
    throw new Error(
      `Stage 1016 relation handoff opened an unexpected relation filter: ${JSON.stringify(relationFilterText)}`,
    )
  }
  ...
  metrics.homePracticeConnectionHandoffFiltersStudy =
    ...
    visibleRelationRows > 0 &&
    (await matchingQuestionRow.isVisible())
  captures.studyRelationPracticeFromHomeQueue = await captureViewportScreenshot(
    page,
    outputDir,
    'stage1016-study-relation-practice-from-home-queue.png',
  )
  ...
  metrics.sourceOverviewRelationPracticePreserved =
    (await sourceRelationRow.getByText(/practice question/).isVisible()) &&
    (await sourceRelationRow.getByRole('button', { name: `Practice relation: ${selectedRelationLabel}` }).isVisible())
  captures.sourceOverviewRelationPracticePreserved = await captureLocatorScreenshot(
    page,
    sourceRelationRow,
    outputDir,
    'stage1016-source-overview-relation-practice-preserved.png',
  )

  await cleanupRelationPracticeQueueHarness({ baseUrl, harness })
  harness = null
  const cleanupDryRun = await findRecallNoteAuditArtifacts({ baseUrl })
  metrics.cleanupUtilityDryRunMatchedAfterStage1016 = cleanupDryRun.matchedCount

  for (const [metricName, expected] of Object.entries({
    cleanupUtilityDryRunMatchedAfterStage1016: 0,
    customCollectionRelationPracticeSignalVisible: true,
    generatedReaderOutputsFrozen: true,
    homePracticeConnectionHandoffFiltersStudy: true,
    homePracticeConnectionSignalVisible: true,
    relationPracticeClearRestoresSourceQuestions: true,
    sourceOverviewRelationPracticePreserved: true,
  })) {
    if (metrics[metricName] !== expected) {
      throw new Error(
        `Stage 1016 expected ${metricName}: ${expected}, got ${metrics[metricName]}. Metrics: ${JSON.stringify(
          metrics,
        )}`,
      )
    }
  ...
  const validation = {
    ...
    metrics,
    runtimeBrowser,
    stage: 'stage1016-home-relation-practice-queue-lens-after-stage1015',
  }
  await writeFile(
    path.join(outputDir, 'stage1016-home-relation-practice-queue-lens-validation.json'),
    JSON.stringify(validation, null, 2),
  )
  console.log(JSON.stringify(validation, null, 2))
} catch (error) {
  await captureViewportScreenshot(page, outputDir, 'stage1016-home-relation-practice-queue-lens-failure.png').catch(
    () => null,
  )
  throw error
} finally {
  await cleanupRelationPracticeQueueHarness({ baseUrl, harness }).catch((error) => {
    console.error(`Stage 1016 cleanup failed: ${error instanceof Error ? error.message : String(error)}`)
  })
...
async function createRelationPracticeQueueHarness({ baseUrl, stageLabel }) {
  ...
  const collectionId = `collection:stage1016-relation-practice-${timestamp}`
  const collectionName = `${stageLabel} Relation Practice`
  const sourceLabel = `Relation Practice Alpha Mike${timestamp}`
  const targetLabel = `Relation Practice Beta Mike${timestamp}`
  const relationSentence = `${sourceLabel} uses ${targetLabel} to make related saved sources practice-ready from Home.`
...
async function waitForHarnessRelatedEdges({ baseUrl, harness, timeoutMs = 30000 }) {
  ...
  throw new Error(
    `Timed out waiting for Stage 1016 related edges ${harness.sourceLabel} -> ${harness.targetLabel}: ${JSON.stringify(
      lastSnapshot,
    )}`,
  )
}
