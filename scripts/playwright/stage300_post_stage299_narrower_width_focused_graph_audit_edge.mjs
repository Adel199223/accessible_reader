...
const outputDir = process.env.RECALL_STAGE300_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE300_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE300_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE300_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE300_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE300_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage300-post-stage299-narrower-width-focused-graph-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage300-focused-overview-narrow-top.png',
  )
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage300-focused-notes-drawer-open-empty-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const focusedGraphNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage300-focused-graph-narrow-top.png')
  const focusedGraphNarrowFull = await captureFullPageScreenshot(
    page,
    outputDir,
    'stage300-focused-graph-narrow-full.png',
  )

  const nodeDetailContinuationStack = page.locator('.recall-graph-focused-mention-source-run-leading-cluster').first()
  await nodeDetailContinuationStack.waitFor({ state: 'visible', timeout: 20000 })
  await nodeDetailContinuationStack.scrollIntoViewIfNeeded()
  await page.waitForTimeout(350)
  const focusedGraphNodeDetailContinuationStackNarrowTop = await captureCurrentViewportScreenshot(
    page,
    outputDir,
    'stage300-focused-graph-node-detail-continuation-stack-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage300-focused-study-narrow-top.png')
  ...
  const focusedStudyAnswerShownNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage300-focused-study-answer-shown-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage300-reader-narrow-top.png')

  await writeFile(
    path.join(outputDir, 'stage300-post-stage299-narrower-width-focused-graph-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'narrower-width focused Graph Node detail continuation-stack fusion beside live reading',
          ...
          'neighboring focused Notes, Study, and Reader stability after the focused Graph continuation-stack fusion follow-up',
        ],
        baseUrl,
        benchmarkMatrix: 'docs/ux/recall_benchmark_matrix.md',
        captures: {
          focusedGraphNodeDetailContinuationStackNarrowTop,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage300-post-stage299-narrower-width-focused-graph-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
