...
const outputDir = process.env.RECALL_STAGE69_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE69_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE69_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE69_HEADLESS === '0' ? false : true

await mkdir(outputDir, { recursive: true })
await rm(path.join(outputDir, 'stage69-study-dock-summary-reduction-failure.png'), { force: true })
...
try {
  ...
  const homeDesktop = path.join(outputDir, 'stage69-home-landing-desktop.png')
  ...
  const graphDesktop = path.join(outputDir, 'stage69-graph-browse-desktop.png')
  ...
  const studyDesktop = path.join(outputDir, 'stage69-study-browse-desktop.png')
  ...
  const focusedStudyDesktop = path.join(outputDir, 'stage69-focused-study-desktop.png')
  await page.screenshot({ path: focusedStudyDesktop, fullPage: true })

  await writeFile(
    path.join(outputDir, 'stage69-study-dock-summary-reduction-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        focusedStudyDesktop,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage69-study-dock-summary-reduction-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
