...
const outputDir = process.env.RECALL_STAGE242_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE242_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE242_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE242_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE242_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE242_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage242-post-stage241-narrower-width-focused-study-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage242-focused-overview-narrow-top.png',
  )
  ...
  const focusedNotesNarrowTop = await captureTopScreenshot(page, outputDir, 'stage242-focused-notes-narrow-top.png')
  ...
  await page.waitForTimeout(350)
  const focusedGraphNarrowTop = await captureTopScreenshot(page, outputDir, 'stage242-focused-graph-narrow-top.png')
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureTopScreenshot(page, outputDir, 'stage242-focused-study-narrow-top.png')
  ...
  const focusedStudyAnswerShownNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage242-focused-study-answer-shown-narrow-top.png',
  )
  const focusedStudyAnswerShownNarrowFull = await captureFullPageScreenshot(
    page,
    outputDir,
    'stage242-focused-study-answer-shown-narrow-full.png',
  )
  ...
  const focusedStudyQueueOpenNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage242-focused-study-queue-open-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureTopScreenshot(page, outputDir, 'stage242-reader-narrow-top.png')

  await writeFile(
    path.join(outputDir, 'stage242-post-stage241-narrower-width-focused-study-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'narrower-width focused Study active-card stack flattening beside live reading',
          'retention of the Stage 223 shell correction and Stage 227 focused-source strip compression',
          'retention of the Stage 225 Reader gains, Stage 229 focused split balance, Stage 237 focused Notes gains, and Stage 239 focused Graph gains',
          'neighboring focused Notes and Graph stability after the localized Study active-card stack pass',
        ],
        baseUrl,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage242-post-stage241-narrower-width-focused-study-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
