...
const outputDir = process.env.RECALL_STAGE524_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE524_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE524_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE524_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE524_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE524_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const desktopViewport = { width: 1600, height: 1080 }

await mkdir(outputDir, { recursive: true })
await rm(path.join(outputDir, 'stage524-post-stage523-reader-top-seam-and-dock-audit-failure.png'), { force: true })
await rm(path.join(outputDir, 'stage524-post-stage523-reader-top-seam-and-dock-audit-failure-reader.png'), {
  force: true,
})
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage524-home-wide-top.png')

  await openGraph(desktopPage)
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage524-graph-wide-top.png')
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopReaderPage,
    outputDir,
    'stage524-reader-original-wide-top.png',
  )
  const readerTopSeamWideTop = await captureLocatorTopScreenshot(
    desktopReaderPage,
    await getVisibleLocator(desktopReaderPage, ['.reader-reading-stage-original-parity', '.reader-reading-stage']),
    outputDir,
    'stage524-reader-top-seam-wide-top.png',
    408,
  )
  const readerArticleLaneWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage524-reader-article-lane-wide-top.png',
    980,
  )
  const readerSupportDockWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage524-reader-support-dock-wide-top.png',
    980,
  )
  ...
  const readerDockNotesWideTop = await captureLocatorTopScreenshot(
    ...
    await getVisibleLocator(desktopReaderPage, ['.reader-support-dock-original-parity', '.reader-support-dock']),
    outputDir,
    'stage524-reader-dock-notes-wide-top.png',
    980,
  )

  const readerMetrics = await getReaderMetrics(desktopReaderPage)

  await writeFile(
    path.join(outputDir, 'stage524-post-stage523-reader-top-seam-and-dock-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop original-only Reader is audited first after the Stage 523 seam-and-dock deflation reset',
          'Reader crops should show a shorter top seam, a clearer article lead, and a lighter attached dock with source and notes continuity intact',
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage524-post-stage523-reader-top-seam-and-dock-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (desktopReaderPage && desktopReaderPage !== desktopPage) {
    await desktopReaderPage
      .screenshot({
        path: path.join(outputDir, 'stage524-post-stage523-reader-top-seam-and-dock-audit-failure-reader.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 524 audit.')
  }
