...
const outputDir = process.env.RECALL_STAGE337_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE337_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE337_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE337_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE337_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE337_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(
  path.join(
    outputDir,
    'stage337-bundled-narrower-width-focused-study-right-lane-hierarchy-rebalance-after-stage336-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage337-focused-overview-narrow-top.png',
  )
  ...
  const focusedStudyNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage337-focused-study-narrow-top.png')
  ...
  const focusedStudyRightLaneNarrowPanelTop = await captureLocatorTopScreenshot(
    ...
    focusedStudyPanel,
    outputDir,
    'stage337-focused-study-right-lane-narrow-panel-top.png',
    760,
  )
  ...
  const focusedStudyAnswerShownNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage337-focused-study-answer-shown-narrow-top.png',
  )
  const focusedStudyAnswerShownRightLaneNarrowPanelTop = await captureLocatorTopScreenshot(
    page,
    focusedStudyPanel,
    outputDir,
    'stage337-focused-study-answer-shown-right-lane-narrow-panel-top.png',
    760,
  )
  ...
  await page.waitForTimeout(350)
  const focusedGraphNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage337-focused-graph-narrow-top.png')
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage337-focused-notes-drawer-open-empty-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage337-reader-narrow-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage337-bundled-narrower-width-focused-study-right-lane-hierarchy-rebalance-after-stage336-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        sourceTitle,
        validationFocus: [
          'narrower-width focused Study right-lane hierarchy rebalance with dedicated pre-answer and answer-shown right-lane crops beside live reading',
          ...
          'neighboring focused Graph, Notes, overview, and Reader stability after the focused Study right-lane hierarchy reset',
        ],
        viewport,
      },
      null,
