...
const outputDir = process.env.RECALL_STAGE642_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE642_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE642_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE642_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE642_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE642_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const desktopViewport = { width: 1600, height: 1080 }
const stage639PreviewRatioBaseline = 0.5838003838771593
const stage639CopyRatioBaseline = 0.29742802303262955
const stage639TitleLineHeightPxBaseline = 19.1479
const stage639TitleToSourceGapPxBaseline = 5.078125
const stage639SourceToChipGapPxBaseline = 4.40625
...
const previewRatioCeiling = 0.5832
const copyRatioFloor = 0.298
const titleLineHeightPxFloor = 19.3
const titleToSourceGapPxFloor = 5.2
const sourceToChipGapPxFloor = 4.52
const sourceFontSizePxFloor = 10.52
const chipFontSizePxFloor = 7.83
const sourceAlphaFloor = 0.75
const chipAlphaFloor = 0.58
...
await rm(
    path.join(
      outputDir,
      'stage642-post-stage641-home-lower-card-copy-seam-and-title-source-chip-descent-settle-audit-failure.png',
    ),
  { force: true },
)
await rm(
    path.join(
      outputDir,
      'stage642-post-stage641-home-lower-card-copy-seam-and-title-source-chip-descent-settle-audit-failure-reader.png',
    ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeAudit(page, outputDir, 'stage642')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage642-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const readerEvidence = await openOriginalReaderFromHome(readerPage, outputDir, 'stage642')

  await writeFile(
    path.join(
      outputDir,
      'stage642-post-stage641-home-lower-card-copy-seam-and-title-source-chip-descent-settle-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 641 lower-card copy-seam and title/source/chip descent settle pass',
          'the Home surface should keep the Stage 563 structure while the lower hierarchy reads slightly cleaner than the Stage 639 baseline across title, source row, and collection chip',
          'Graph and original-only Reader remain regression surfaces only after the Home-only Stage 641 pass',
        ],
        baseUrl,
...
async function captureHomeAudit(page, directory, stagePrefix) {
  ...
  const representativeCard = dayGroup.locator('.recall-home-parity-card-stage641').first()
  const preview = representativeCard.locator('.recall-home-parity-card-preview-stage641').first()
  const copy = representativeCard.locator('.recall-home-parity-card-copy-stage641').first()
  const title = representativeCard.locator('.recall-home-parity-card-title-stage641').first()
  const sourceRow = representativeCard.locator('.recall-home-parity-card-source-row-stage641').first()
  const source = representativeCard.locator('.recall-home-parity-card-source-stage641').first()
  const chip = representativeCard.locator('.recall-home-parity-card-chip-stage641').first()
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 642 card'),
    readMeasuredBox(preview, 'Stage 642 card preview'),
    readMeasuredBox(copy, 'Stage 642 card copy block'),
    readMeasuredBox(title, 'Stage 642 card title'),
    readMeasuredBox(sourceRow, 'Stage 642 card source row'),
    readMeasuredBox(chip, 'Stage 642 card chip'),
  ])
  ...
  const [titleState, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 642 Home canvas, day header, and grid.')
      }
  ...
  if (!(previewRatio < stage639PreviewRatioBaseline && previewRatio <= previewRatioCeiling)) {
    throw new Error(`Expected preview ratio below the Stage 639 baseline and within the target band, found ${previewRatio}.`)
  }
  if (!(copyRatio > stage639CopyRatioBaseline && copyRatio >= copyRatioFloor)) {
    throw new Error(`Expected copy ratio above the Stage 639 baseline and within the target band, found ${copyRatio}.`)
  }
  if (!(titleState.lineHeightPx > stage639TitleLineHeightPxBaseline && titleState.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 639 baseline, found ${titleState.lineHeightPx}.`)
  }
  if (!(titleToSourceGapPx > stage639TitleToSourceGapPxBaseline && titleToSourceGapPx >= titleToSourceGapPxFloor)) {
    throw new Error(`Expected title-to-source gap above the Stage 639 baseline, found ${titleToSourceGapPx}.`)
  }
  if (!(sourceToChipGapPx > stage639SourceToChipGapPxBaseline && sourceToChipGapPx >= sourceToChipGapPxFloor)) {
    throw new Error(`Expected source-to-chip gap above the Stage 639 baseline, found ${sourceToChipGapPx}.`)
  }
  if (!(sourceState.fontSizePx >= sourceFontSizePxFloor && sourceState.alpha >= sourceAlphaFloor)) {
    throw new Error(`Expected source-row legibility to stay at or above the Stage 639 band, found ${sourceState.fontSizePx}px and alpha ${sourceState.alpha}.`)
  }
  if (!(chipState.fontSizePx >= chipFontSizePxFloor && chipState.alpha >= chipAlphaFloor)) {
    throw new Error(`Expected chip legibility to stay at or above the Stage 639 band, found ${chipState.fontSizePx}px and alpha ${chipState.alpha}.`)
  }
  ...
  const webCard = canvas.locator('.recall-home-parity-card-stage641').first()
...
async function openOriginalReaderFromHome(page, directory, stagePrefix) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not keep Reader in Original mode for the Stage 642 audit.')
  }
