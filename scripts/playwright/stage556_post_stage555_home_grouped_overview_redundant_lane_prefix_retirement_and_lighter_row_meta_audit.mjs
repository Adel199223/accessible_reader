...
const outputDir = process.env.RECALL_STAGE556_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE556_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE556_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE556_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE556_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE556_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage556-post-stage555-home-grouped-overview-redundant-lane-prefix-retirement-and-lighter-row-meta-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage556-post-stage555-home-grouped-overview-redundant-lane-prefix-retirement-and-lighter-row-meta-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeOverviewEvidence = await captureHomeOverviewEvidence(page, outputDir, 'stage556')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage556-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const { sourceTitle } = await openOriginalReaderFromHome(readerPage)
  const readerOriginalWideTop = await captureViewportScreenshot(readerPage, outputDir, 'stage556-reader-original-wide-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage556-post-stage555-home-grouped-overview-redundant-lane-prefix-retirement-and-lighter-row-meta-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 555 grouped-overview metadata-seam reset',
          'the grouped overview should now retire redundant visible lane prefixes in Captures and Web while keeping useful file-format context in Documents',
...
async function captureHomeOverviewEvidence(page, directory, stagePrefix) {
  ...
  if (!primaryMetric || secondaryMetrics.length !== 2) {
    throw new Error('Could not measure the primary lane and secondary stack cards for the Stage 556 audit.')
  }
  ...
  for (const metric of cardMetrics) {
    ...
    if (metric.title === 'Captures' && metric.footerButtonHeight == null) {
      throw new Error(
        'The Captures grouped-overview card no longer exposed a measurable Show all footer button for the Stage 556 audit.',
      )
    }
  }
  ...
  if (!overviewBox || !headerBox || !gridBox || !titleRowBox || !titleHeadingBox || !metaBox) {
    throw new Error('Could not measure the grouped-overview shell geometry for the Stage 556 audit.')
  }
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 556 audit.')
  }
  return { sourceTitle }
}
