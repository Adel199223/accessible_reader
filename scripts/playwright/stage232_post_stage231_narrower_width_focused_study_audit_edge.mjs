...
const outputDir = process.env.RECALL_STAGE232_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE232_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE232_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE232_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE232_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE232_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage232-post-stage231-narrower-width-focused-study-audit-failure.png'), { force: true })
...
try {
  ...
  const focusedOverviewNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage232-focused-overview-narrow-top.png',
  )
  ...
  const focusedNotesNarrowTop = await captureTopScreenshot(page, outputDir, 'stage232-focused-notes-narrow-top.png')
  ...
  await page.waitForTimeout(350)
  const focusedGraphNarrowTop = await captureTopScreenshot(page, outputDir, 'stage232-focused-graph-narrow-top.png')
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureTopScreenshot(page, outputDir, 'stage232-focused-study-narrow-top.png')
  ...
  const focusedStudyQueueOpenNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage232-focused-study-queue-open-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureTopScreenshot(page, outputDir, 'stage232-reader-narrow-top.png')

  await writeFile(
    path.join(outputDir, 'stage232-post-stage231-narrower-width-focused-study-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'narrower-width focused Study queue-rail deflation beside live reading',
          'retention of the Stage 223 shell correction and Stage 227 focused-source strip compression',
          'retention of the Stage 225 Reader chrome gains and Stage 229 focused split balance',
          'neighboring focused Notes and Graph stability after the localized Study rail pass',
        ],
        baseUrl,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage232-post-stage231-narrower-width-focused-study-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
