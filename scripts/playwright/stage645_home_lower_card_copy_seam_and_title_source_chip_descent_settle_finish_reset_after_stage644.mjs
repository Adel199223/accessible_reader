...
const outputDir = process.env.RECALL_STAGE645_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE645_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE645_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE645_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE645_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE645_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
    path.join(
      outputDir,
      'stage645-home-lower-card-copy-seam-and-title-source-chip-descent-settle-finish-reset-after-stage644-failure.png',
    ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeImplementation(page, outputDir, 'stage645')

  await writeFile(
    path.join(
      outputDir,
      'stage645-home-lower-card-copy-seam-and-title-source-chip-descent-settle-finish-reset-after-stage644-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        implementationFocus: [
          ...
          'The next polish pass should reduce the remaining March 25, 2026 Recall homepage mismatch by carrying one final subtler lower-card copy-seam and title/source/chip settle finish beyond the Stage 643 follow-through state.',
          'Shell, rail, toolbar, column cadence, and source-aware poster variants should stay intact while the lower copy stack gains a final small step of ownership and a slightly steadier descending cadence.',
        ],
...
async function captureHomeImplementation(page, directory, stagePrefix) {
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 645 card'),
    readMeasuredBox(preview, 'Stage 645 card preview'),
    readMeasuredBox(copy, 'Stage 645 card copy block'),
    readMeasuredBox(title, 'Stage 645 card title'),
    readMeasuredBox(sourceRow, 'Stage 645 card source row'),
    readMeasuredBox(chip, 'Stage 645 card chip'),
  ])
  ...
  const [titleStyles, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 645 Home canvas, day header, and grid.')
      }
  ...
  if (!(titleStyles.lineHeightPx > stage643TitleLineHeightPxBaseline && titleStyles.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 643 baseline, found ${titleStyles.lineHeightPx}.`)
  }
  ...
  if (!(representativeCardBox.height <= cardHeightPxCeiling)) {
    throw new Error(`Expected representative card height to stay compact after the Stage 643 pass, found ${representativeCardBox.height}.`)
  }
