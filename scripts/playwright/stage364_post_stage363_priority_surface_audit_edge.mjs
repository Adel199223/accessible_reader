...
const outputDir = process.env.RECALL_STAGE364_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE364_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE364_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE364_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE364_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE364_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage364-post-stage363-priority-surface-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const desktopHomeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage364-home-wide-top.png')
  const homeGlanceWideTop = await captureLocatorTopScreenshot(
    desktopPage,
    desktopPage.getByLabel('Collection snapshot').first(),
    outputDir,
    'stage364-home-glance-wide-top.png',
    720,
  )
  ...
  await desktopPage.waitForTimeout(350)
  const desktopGraphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage364-graph-wide-top.png')
  const graphDetailDockWideTop = await captureLocatorTopScreenshot(
    desktopPage,
    desktopPage.getByLabel('Node detail dock').first(),
    outputDir,
    'stage364-graph-detail-dock-wide-top.png',
    760,
    'locator',
  )
  ...
  await desktopPage.waitForTimeout(350)
  const desktopStudyWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage364-study-wide-top.png')
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage364-focused-overview-narrow-top.png',
  )
  ...
  const focusedGraphNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage364-focused-graph-narrow-top.png',
  )
  ...
  } = await captureFocusedNotesRegressionArtifacts(focusedPage, outputDir, 'stage364')
  ...
  const focusedStudyNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage364-focused-study-narrow-top.png',
  )
  ...
  const focusedReaderNarrowTop = await captureViewportScreenshot(
    focusedPage,
    outputDir,
    'stage364-focused-reader-narrow-top.png',
  )

  await writeFile(
    path.join(outputDir, 'stage364-post-stage363-priority-surface-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'full Stage 364 audit with wide desktop Home, Graph, Reader, Notes, and frozen Study captures in that order',
          'shared baseline evidence crops for the Home support glance, Graph detail dock, Reader dock, and Notes detail/context rail',
          'focused regressions second: overview, Graph, Notes default, Notes drawer-open Search notes, frozen Study, and Reader',
          'Graph, Home, Reader, and Notes are locked priority-surface baselines while Study remains regression-only',
        ],
        baseUrl,
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage364-post-stage363-priority-surface-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (desktopReaderPage && desktopReaderPage !== desktopPage) {
    await desktopReaderPage
      .screenshot({
        path: path.join(outputDir, 'stage364-post-stage363-priority-surface-audit-failure-reader.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (focusedPage && focusedPage !== desktopPage) {
    await focusedPage
      .screenshot({
        path: path.join(outputDir, 'stage364-post-stage363-priority-surface-audit-failure-focused.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
