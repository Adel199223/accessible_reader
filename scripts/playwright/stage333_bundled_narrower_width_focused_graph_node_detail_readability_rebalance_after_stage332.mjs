...
const outputDir = process.env.RECALL_STAGE333_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE333_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE333_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE333_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE333_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE333_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(
  path.join(
    outputDir,
    'stage333-bundled-narrower-width-focused-graph-node-detail-readability-rebalance-after-stage332-failure.png',
  ),
  {
    force: true,
  },
...
try {
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage333-focused-overview-narrow-top.png',
  )
  ...
  const focusedGraphNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage333-focused-graph-narrow-top.png')
  const focusedGraphNarrowFull = await captureFullPageScreenshot(
    page,
    outputDir,
    'stage333-focused-graph-narrow-full.png',
  )
  ...
  const focusedGraphNodeDetailRailNarrowTop = await captureLocatorTopScreenshot(
    ...
    graphFocusedDetail,
    outputDir,
    'stage333-focused-graph-node-detail-rail-narrow-top.png',
    920,
  )
  const focusedGraphLeadingEvidenceReadabilityBundleNarrowPanelTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage333-focused-graph-leading-evidence-readability-bundle-narrow-panel-top.png',
    460,
  )
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage333-focused-notes-drawer-open-empty-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage333-focused-study-narrow-top.png')
  ...
  const focusedStudyAnswerShownNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage333-focused-study-answer-shown-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage333-reader-narrow-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage333-bundled-narrower-width-focused-graph-node-detail-readability-rebalance-after-stage332-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
