...
const outputDir = process.env.RECALL_STAGE546_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE546_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE546_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE546_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE546_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE546_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage546-post-stage545-home-grouped-overview-inline-title-status-join-and-left-cluster-anchor-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage546-post-stage545-home-grouped-overview-inline-title-status-join-and-left-cluster-anchor-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeOverviewEvidence = await captureHomeOverviewEvidence(page, outputDir, 'stage546')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage546-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const { sourceTitle } = await openOriginalReaderFromHome(readerPage)
  const readerOriginalWideTop = await captureViewportScreenshot(readerPage, outputDir, 'stage546-reader-original-wide-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage546-post-stage545-home-grouped-overview-inline-title-status-join-and-left-cluster-anchor-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 545 grouped-overview inline title-status join and left-cluster anchor reset',
          'the grouped overview should now keep the condensed status note joined to the All collections title row while the Stage 537 primary-lane and secondary-stack composition stays intact',
...
async function captureHomeOverviewEvidence(page, directory, stagePrefix) {
  ...
  const overviewHeader = savedLibraryOverview.locator('.recall-home-library-stream-header-stage545-reset').first()
  const overviewGrid = savedLibraryOverview.locator('.recall-home-library-stream-grid-stage541-reset').first()
  const overviewTitleRow = savedLibraryOverview.locator('.recall-home-library-stream-title-row-stage545-reset').first()
  const overviewTitleHeading = overviewTitleRow.locator('h3').first()
  const overviewMeta = savedLibraryOverview.locator('.recall-home-library-stream-meta-stage545-reset').first()
  ...
  if (!primaryMetric || secondaryMetrics.length !== 2) {
    throw new Error('Could not measure the primary lane and secondary stack cards for the Stage 546 audit.')
  }
  ...
  if (!overviewBox || !headerBox || !gridBox || !titleRowBox || !titleHeadingBox || !metaBox) {
    throw new Error('Could not measure the grouped-overview shell geometry for the Stage 546 audit.')
  }
  ...
  const homeOverviewTitleStatusWideTop = await captureLocatorTopScreenshot(
    page,
    savedLibraryOverview,
    directory,
    `${stagePrefix}-home-overview-title-status-wide-top.png`,
    320,
  )
  ...
  return {
    captures: {
      ...
      homeOverviewTitleStatusWideTop,
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 546 audit.')
  }
  return { sourceTitle }
}
