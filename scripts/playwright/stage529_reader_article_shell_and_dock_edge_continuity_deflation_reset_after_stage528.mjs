...
const outputDir = process.env.RECALL_STAGE529_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE529_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE529_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE529_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE529_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE529_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage529-reader-article-shell-and-dock-edge-continuity-deflation-reset-after-stage528-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopPage,
    outputDir,
    'stage529-reader-original-wide-top.png',
  )
  const readerDeckJoinWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage529-reader-deck-join-wide-top.png',
    430,
  )
  const readerArticleLeadWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage529-reader-article-lead-wide-top.png',
    430,
  )
  const readerDockEdgeWideTop = await captureDockEdgeScreenshot(
    desktopPage,
    outputDir,
    'stage529-reader-dock-edge-wide-top.png',
    430,
  )
  const readerMetrics = await getReaderDeckMetrics(desktopPage)

  await writeFile(
    path.join(
      outputDir,
      'stage529-reader-article-shell-and-dock-edge-continuity-deflation-reset-after-stage528-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        sourceTitle,
        validationFocus: [
          'original-only Reader is reset again immediately after the Stage 528 audit instead of hopping to a new surface',
          'the reading deck should behave more like one attached article-and-dock surface instead of two bordered cards separated by a visible gutter',
          'the deck-join, article-lead, and dock-edge crops should show a flatter article shell with the companion dock attached at the edge',
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 529 validation.')
  }
