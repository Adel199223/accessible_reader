...
const outputDir = process.env.RECALL_STAGE123_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE123_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE123_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE123_HEADLESS === '0' ? false : true

await mkdir(outputDir, { recursive: true })
await rm(path.join(outputDir, 'stage123-study-support-strip-collapse-failure.png'), { force: true })
...
try {
  ...
  const homeDesktop = path.join(outputDir, 'stage123-home-landing-desktop.png')
  ...
  const studyDesktop = path.join(outputDir, 'stage123-study-browse-desktop.png')
  ...
  const graphDesktop = path.join(outputDir, 'stage123-graph-browse-desktop.png')
  ...
  const focusedStudyDesktop = path.join(outputDir, 'stage123-focused-study-desktop.png')
  await page.screenshot({ path: focusedStudyDesktop, fullPage: true })

  await writeFile(
    path.join(outputDir, 'stage123-study-support-strip-collapse-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        focusedStudyDesktop,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage123-study-support-strip-collapse-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
