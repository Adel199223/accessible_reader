...
const outputDir = process.env.RECALL_STAGE350_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE350_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE350_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE350_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE350_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE350_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage350-post-stage349-narrower-width-focused-study-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage350-focused-overview-narrow-top.png',
  )
  ...
  const focusedStudyNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage350-focused-study-narrow-top.png')

  const focusedStudyPanel = page.locator('.recall-study-focused-panel-body-fused').first()
  const focusedStudyBody = page.locator('.recall-study-focused-body-flow-fused').first()
  await focusedStudyPanel.waitFor({ state: 'visible', timeout: 20000 })
  await focusedStudyBody.waitFor({ state: 'visible', timeout: 20000 })
  const focusedStudyRightLaneNarrowPanelTop = await captureLocatorTopScreenshot(
    page,
    focusedStudyPanel,
    outputDir,
    'stage350-focused-study-right-lane-narrow-panel-top.png',
    760,
  )
  const focusedStudyBodyFlowFusionNarrowPanelTop = await captureLocatorTopScreenshot(
    page,
    focusedStudyBody,
    outputDir,
    'stage350-focused-study-body-flow-fusion-narrow-panel-top.png',
    760,
  )
  ...
  const focusedStudyAnswerShownNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage350-focused-study-answer-shown-narrow-top.png',
  )
  const focusedStudyAnswerShownRightLaneNarrowPanelTop = await captureLocatorTopScreenshot(
    page,
    focusedStudyPanel,
    outputDir,
    'stage350-focused-study-answer-shown-right-lane-narrow-panel-top.png',
    760,
  )
  const focusedStudyAnswerShownBodyFlowFusionNarrowPanelTop = await captureLocatorTopScreenshot(
    page,
    focusedStudyBody,
    outputDir,
    'stage350-focused-study-answer-shown-body-flow-fusion-narrow-panel-top.png',
    760,
  )
  ...
  await page.waitForTimeout(350)
  const focusedGraphNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage350-focused-graph-narrow-top.png')
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage350-focused-notes-drawer-open-empty-narrow-top.png',
  )
    ...
    JSON.stringify(
      {
        ...
        captures: {
          ...
          focusedStudyAnswerShownBodyFlowFusionNarrowPanelTop,
          focusedStudyAnswerShownNarrowTop,
          focusedStudyAnswerShownRightLaneNarrowPanelTop,
          focusedStudyBodyFlowFusionNarrowPanelTop,
          focusedStudyNarrowTop,
          focusedStudyRightLaneNarrowPanelTop,
          readerNarrowTop,
        },
        headless,
        runtimeBrowser,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage350-post-stage349-narrower-width-focused-study-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
