...
const outputDir = process.env.RECALL_STAGE547_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE547_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE547_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE547_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE547_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE547_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage547-home-grouped-overview-helper-note-retirement-and-leaner-title-cluster-reset-after-stage546-failure.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeOverviewEvidence = await captureHomeOverviewEvidence(page, outputDir, 'stage547')

  await writeFile(
    path.join(
      outputDir,
      'stage547-home-grouped-overview-helper-note-retirement-and-leaner-title-cluster-reset-after-stage546-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        runtimeBrowser,
        validationFocus: [
          'wide desktop Home keeps the grouped overview open while the All collections title cluster drops the remaining default-state helper sentence band',
          'the Stage 545 inline title-status join and the Stage 537 primary-lane plus secondary-stack composition should stay intact while the header gets leaner',
          'the helper-note retirement should improve the working-board read without widening into selected-group or generated-content Reader work',
        ],
      },
      null,
      2,
...
async function captureHomeOverviewEvidence(page, directory, stagePrefix) {
  ...
  if (
    ...
    throw new Error('Grouped overview still rendered the old helper sentence; the lean title-cluster reset did not hold.')
  }
  ...
  if (!primaryMetric || secondaryMetrics.length !== 2) {
    throw new Error('Could not measure the primary lane and secondary stack cards for the Stage 547 validation.')
  }
  ...
  if (!overviewBox || !headerBox || !gridBox || !titleRowBox || !titleHeadingBox || !metaBox) {
    throw new Error('Could not measure the grouped-overview shell geometry for the Stage 547 validation.')
  }
