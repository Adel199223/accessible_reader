...
const outputDir = process.env.RECALL_STAGE635_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE635_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE635_BASE_URL ?? 'http://172.27.18.251:8002'
const headless = process.env.RECALL_STAGE635_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE635_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE635_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
    path.join(
      outputDir,
      'stage635-home-lower-card-title-source-chip-descent-finish-and-copy-seam-settling-reset-after-stage634-failure.png',
    ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeImplementation(page, outputDir, 'stage635')

  await writeFile(
    path.join(
      outputDir,
      'stage635-home-lower-card-title-source-chip-descent-finish-and-copy-seam-settling-reset-after-stage634-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
        ...
        implementationFocus: [
          ...
          'The next polish pass should reduce the remaining March 25, 2026 Recall homepage mismatch by finishing one subtler lower-card title/source/chip descent and settling the copy seam a little further.',
          'Shell, rail, toolbar, column cadence, and source-aware poster variants should stay intact while the lower copy stack gains one calmer descending settle.',
        ],
...
async function captureHomeImplementation(page, directory, stagePrefix) {
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 635 card'),
    readMeasuredBox(preview, 'Stage 635 card preview'),
    readMeasuredBox(copy, 'Stage 635 card copy block'),
    readMeasuredBox(title, 'Stage 635 card title'),
    readMeasuredBox(sourceRow, 'Stage 635 card source row'),
    readMeasuredBox(chip, 'Stage 635 card chip'),
  ])
  ...
  const [titleStyles, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 635 Home canvas, day header, and grid.')
      }
  ...
  if (!(titleStyles.lineHeightPx > stage633TitleLineHeightPxBaseline && titleStyles.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 633 baseline, found ${titleStyles.lineHeightPx}.`)
  }
  ...
  if (!(representativeCardBox.height <= cardHeightPxCeiling)) {
    throw new Error(`Expected representative card height to stay compact after the Stage 633 pass, found ${representativeCardBox.height}.`)
  }
