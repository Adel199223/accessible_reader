...
const outputDir = process.env.RECALL_STAGE332_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE332_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE332_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE332_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE332_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE332_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(path.join(outputDir, 'stage332-post-stage331-narrower-width-focused-graph-audit-failure.png'), {
  force: true,
})
...
try {
  ...
  const focusedOverviewNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage332-focused-overview-narrow-top.png',
  )
  ...
  const focusedGraphNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage332-focused-graph-narrow-top.png')
  const focusedGraphNarrowFull = await captureFullPageScreenshot(
    page,
    outputDir,
    'stage332-focused-graph-narrow-full.png',
  )
  ...
  const focusedGraphNodeDetailRailNarrowTop = await captureLocatorTopScreenshot(
    ...
    graphFocusedDetail,
    outputDir,
    'stage332-focused-graph-node-detail-rail-narrow-top.png',
    920,
  )
  const focusedGraphSelectedNodeGlanceLeadingGroundedEvidencePreviewMetaNarrowPanelTop = await captureLocatorTopScreenshot(
    page,
    graphFocusedDetail,
    outputDir,
    'stage332-focused-graph-selected-node-glance-leading-grounded-evidence-preview-meta-narrow-panel-top.png',
    720,
  )
  ...
  const focusedNotesDrawerOpenEmptyNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage332-focused-notes-drawer-open-empty-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const focusedStudyNarrowTop = await captureViewportScreenshot(page, outputDir, 'stage332-focused-study-narrow-top.png')
  ...
  const focusedStudyAnswerShownNarrowTop = await captureViewportScreenshot(
    page,
    outputDir,
    'stage332-focused-study-answer-shown-narrow-top.png',
  )
    ...
    JSON.stringify(
      {
        ...
        captures: {
          ...
          focusedGraphSelectedNodeGlanceLeadingGroundedEvidencePreviewMetaNarrowPanelTop,
  ...
  if (page) {
    await page
      .screenshot({
        path: path.join(outputDir, 'stage332-post-stage331-narrower-width-focused-graph-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
