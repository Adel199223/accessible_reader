...
const outputDir = process.env.RECALL_STAGE544_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE544_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE544_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE544_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE544_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE544_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const desktopViewport = { width: 1600, height: 1080 }
const minimumPrimaryWidthDelta = 80
const maximumSecondaryColumnXSpread = 12
const minimumSecondaryRowTopDelta = 60
const maximumOverviewHeaderHeight = 72
const maximumGridOffsetFromOverviewTop = 84
const maximumTitleRowTopOffsetFromOverviewTop = 18
const maximumStatusGapBelowTitleRow = 8
const minimumStatusTopOffsetFromOverviewTop = 4.5
...
await rm(
  path.join(
    outputDir,
    'stage544-post-stage543-home-grouped-overview-status-seam-narrowing-and-heading-attachment-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage544-post-stage543-home-grouped-overview-status-seam-narrowing-and-heading-attachment-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeOverviewEvidence = await captureHomeOverviewEvidence(page, outputDir, 'stage544')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage544-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const { sourceTitle } = await openOriginalReaderFromHome(readerPage)
  const readerOriginalWideTop = await captureViewportScreenshot(readerPage, outputDir, 'stage544-reader-original-wide-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage544-post-stage543-home-grouped-overview-status-seam-narrowing-and-heading-attachment-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 543 grouped-overview status-seam narrowing and heading attachment',
          'the grouped overview should now keep a smaller attached heading note instead of a broad top-cap status seam while the Stage 537 primary-lane and secondary-stack composition stays intact',
        ...
        minimumStatusTopOffsetFromOverviewTop,
        ...
        overviewStatusSeamMetrics: homeOverviewEvidence.statusSeamMetrics,
        runtimeBrowser,
        sourceTitle,
      },
      null,
...
async function captureHomeOverviewEvidence(page, directory, stagePrefix) {
  ...
  const overviewHeader = savedLibraryOverview.locator('.recall-home-library-stream-header-stage543-reset').first()
  const overviewGrid = savedLibraryOverview.locator('.recall-home-library-stream-grid-stage541-reset').first()
  const overviewTitleRow = savedLibraryOverview.locator('.recall-home-library-stream-title-row-stage543-reset').first()
  const overviewMeta = savedLibraryOverview.locator('.recall-home-library-stream-meta-stage543-reset').first()
  ...
  if (/groups/i.test(overviewMetaText) || /\bboard\b/i.test(overviewMetaText)) {
    throw new Error(
      `Overview status summary stayed too verbose (${overviewMetaText}); the narrowed heading note still exposed redundant groups or Board labels.`,
    )
  }
  ...
  if (!primaryMetric || secondaryMetrics.length !== 2) {
    throw new Error('Could not measure the primary lane and secondary stack cards for the Stage 544 audit.')
  }

  const [overviewBox, headerBox, gridBox, titleRowBox, metaBox] = await Promise.all([
  ...
  if (!overviewBox || !headerBox || !gridBox || !titleRowBox || !metaBox) {
    throw new Error('Could not measure the grouped-overview shell geometry for the Stage 544 audit.')
  }
  ...
  const titleStatusTopDelta = roundMetric(Math.abs(metaBox.y - titleRowBox.y))
  const statusGapBelowTitleRow = roundMetric(metaBox.y - (titleRowBox.y + titleRowBox.height))
  const statusTopOffsetFromOverviewTop = roundMetric(metaBox.y - overviewBox.y)
  ...
  if (overviewHeaderHeight > maximumOverviewHeaderHeight) {
    throw new Error(
      `Overview header height (${overviewHeaderHeight}px) stayed too tall; the grouped board still carries too much top-cap banding.`,
    )
  }
  if (gridOffsetFromOverviewTop > maximumGridOffsetFromOverviewTop) {
    throw new Error(
      `Overview grid offset (${gridOffsetFromOverviewTop}px) stayed too large; the grouped board did not start early enough.`,
    )
  }
  ...
  if (statusGapBelowTitleRow > maximumStatusGapBelowTitleRow) {
    throw new Error(
      `Status gap below the title row (${statusGapBelowTitleRow}px) stayed too large; the status note drifted too far from the heading block.`,
    )
  }
  if (statusTopOffsetFromOverviewTop < minimumStatusTopOffsetFromOverviewTop) {
    throw new Error(
      `Status top offset (${statusTopOffsetFromOverviewTop}px) stayed too shallow; the status note still reads like a flush top cap.`,
    )
  }
  if (statusBlockHeight > maximumStatusBlockHeight) {
    throw new Error(
      `Status block height (${statusBlockHeight}px) stayed too tall; the overview status note did not compress enough.`,
    )
  }
  if (statusBlockWidth > maximumStatusBlockWidth) {
    throw new Error(
      `Status block width (${statusBlockWidth}px) stayed too wide; the overview status still reads like a broad top seam.`,
    )
  }
  ...
  const homeOverviewStatusSeamWideTop = await captureLocatorTopScreenshot(
    page,
    savedLibraryOverview,
    directory,
    `${stagePrefix}-home-overview-status-seam-wide-top.png`,
    320,
  )
  ...
  return {
    ...
    statusSeamMetrics: {
      statusBlockHeight,
      statusBlockWidth,
      statusTopOffsetFromOverviewTop,
      statusGapBelowTitleRow,
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 540 audit.')
  }
  return { sourceTitle }
}
