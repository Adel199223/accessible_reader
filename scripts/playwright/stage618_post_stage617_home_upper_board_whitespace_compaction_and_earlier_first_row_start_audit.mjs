...
const outputDir = process.env.RECALL_STAGE618_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE618_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE618_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE618_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE618_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE618_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage618-post-stage617-home-upper-board-whitespace-compaction-and-earlier-first-row-start-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage618-post-stage617-home-upper-board-whitespace-compaction-and-earlier-first-row-start-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeAudit(page, outputDir, 'stage618')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage618-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const readerEvidence = await openOriginalReaderFromHome(readerPage, outputDir, 'stage618')

  await writeFile(
    path.join(
      outputDir,
      'stage618-post-stage617-home-upper-board-whitespace-compaction-and-earlier-first-row-start-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 617 upper-board whitespace pass',
          'the Home surface should keep the Stage 563 structure while starting the first day label and first row earlier inside the canvas without reopening the card-width cadence pass',
          'Graph and original-only Reader remain regression surfaces only after the Home-only Stage 617 pass',
        ],
        baseUrl,
...
async function captureHomeAudit(page, directory, stagePrefix) {
  ...
  const [representativeCardBox, addTileBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 618 card'),
    readMeasuredBox(addTile, 'Stage 618 add tile'),
  ])
  ...
  const spacingMetrics = await page.evaluate(() => {
    ...
    if (
      ...
      throw new Error('Expected Stage 618 Home layout elements.')
    }
...
async function openOriginalReaderFromHome(page, directory, stagePrefix) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not keep Reader in Original mode for the Stage 618 audit.')
  }
