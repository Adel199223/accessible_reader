...
const outputDir = process.env.RECALL_STAGE548_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE548_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE548_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE548_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE548_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE548_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage548-post-stage547-home-grouped-overview-helper-note-retirement-and-leaner-title-cluster-audit-failure.png',
  ),
  { force: true },
)
await rm(
  path.join(
    outputDir,
    'stage548-post-stage547-home-grouped-overview-helper-note-retirement-and-leaner-title-cluster-audit-failure-reader.png',
  ),
  { force: true },
)
...
try {
  ...
  const homeOverviewEvidence = await captureHomeOverviewEvidence(page, outputDir, 'stage548')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage548-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const { sourceTitle } = await openOriginalReaderFromHome(readerPage)
  const readerOriginalWideTop = await captureViewportScreenshot(readerPage, outputDir, 'stage548-reader-original-wide-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage548-post-stage547-home-grouped-overview-helper-note-retirement-and-leaner-title-cluster-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 547 grouped-overview helper-note retirement and leaner title-cluster reset',
          'the grouped overview should now keep a lean title cluster without the default-state helper sentence while the Stage 545 title-status join and the Stage 537 primary-lane plus secondary-stack composition stay intact',
...
async function captureHomeOverviewEvidence(page, directory, stagePrefix) {
  ...
  if (
    ...
    throw new Error('Grouped overview still rendered the old helper sentence; the lean title-cluster audit failed.')
  }
  ...
  if (/groups/i.test(overviewMetaText) || /\bboard\b/i.test(overviewMetaText)) {
    throw new Error(
      `Overview status summary stayed too verbose (${overviewMetaText}); the lean title cluster still exposed redundant groups or Board labels.`,
    )
  }
  ...
  if (!primaryMetric || secondaryMetrics.length !== 2) {
    throw new Error('Could not measure the primary lane and secondary stack cards for the Stage 548 audit.')
  }
  ...
  if (!overviewBox || !headerBox || !gridBox || !titleRowBox || !titleHeadingBox || !metaBox) {
    throw new Error('Could not measure the grouped-overview shell geometry for the Stage 548 audit.')
  }
  ...
  if (overviewHeaderHeight > maximumOverviewHeaderHeight) {
    throw new Error(
      `Overview header height (${overviewHeaderHeight}px) stayed too tall; the helper-note retirement did not create a meaningfully leaner title cluster.`,
    )
  }
  if (gridOffsetFromOverviewTop > maximumGridOffsetFromOverviewTop) {
    throw new Error(
      `Overview grid offset (${gridOffsetFromOverviewTop}px) stayed too large; the grouped board did not pull up after the helper-note retirement.`,
    )
  }
  ...
  const homeOverviewLeanTitleWideTop = await captureLocatorTopScreenshot(
    page,
    savedLibraryOverview,
    directory,
    `${stagePrefix}-home-overview-lean-title-wide-top.png`,
    240,
  )
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 548 audit.')
  }
  return { sourceTitle }
}
