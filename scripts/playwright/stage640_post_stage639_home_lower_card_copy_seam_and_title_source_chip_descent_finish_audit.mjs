...
const outputDir = process.env.RECALL_STAGE640_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE640_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE640_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE640_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE640_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE640_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const desktopViewport = { width: 1600, height: 1080 }
const stage637PreviewRatioBaseline = 0.5887140115163148
const stage637CopyRatioBaseline = 0.29351247600767755
const stage637TitleLineHeightPxBaseline = 18.8738
const stage637TitleToSourceGapPxBaseline = 4.859375
const stage637SourceToChipGapPxBaseline = 4.1875
...
const previewRatioCeiling = 0.5882
const copyRatioFloor = 0.294
const titleLineHeightPxFloor = 19.0
const titleToSourceGapPxFloor = 5.0
const sourceToChipGapPxFloor = 4.32
const sourceFontSizePxFloor = 10.5
const chipFontSizePxFloor = 7.82
const sourceAlphaFloor = 0.74
const chipAlphaFloor = 0.57
...
await rm(
    path.join(
      outputDir,
      'stage640-post-stage639-home-lower-card-copy-seam-and-title-source-chip-descent-finish-audit-failure.png',
    ),
  { force: true },
)
await rm(
    path.join(
      outputDir,
      'stage640-post-stage639-home-lower-card-copy-seam-and-title-source-chip-descent-finish-audit-failure-reader.png',
    ),
  { force: true },
)
...
try {
  ...
  const homeEvidence = await captureHomeAudit(page, outputDir, 'stage640')

  await openGraph(page)
  const graphWideTop = await captureViewportScreenshot(page, outputDir, 'stage640-graph-wide-top.png')

  readerPage = await browser.newPage({ viewport: desktopViewport })
  const readerEvidence = await openOriginalReaderFromHome(readerPage, outputDir, 'stage640')

  await writeFile(
    path.join(
      outputDir,
      'stage640-post-stage639-home-lower-card-copy-seam-and-title-source-chip-descent-finish-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 639 lower-card copy-seam and title/source/chip descent finish pass',
          'the Home surface should keep the Stage 563 structure while the lower hierarchy reads slightly cleaner than the Stage 637 baseline across title, source row, and collection chip',
          'Graph and original-only Reader remain regression surfaces only after the Home-only Stage 639 pass',
        ],
        baseUrl,
...
async function captureHomeAudit(page, directory, stagePrefix) {
  ...
  const representativeCard = dayGroup.locator('.recall-home-parity-card-stage639').first()
  const preview = representativeCard.locator('.recall-home-parity-card-preview-stage639').first()
  const copy = representativeCard.locator('.recall-home-parity-card-copy-stage639').first()
  const title = representativeCard.locator('.recall-home-parity-card-title-stage639').first()
  const sourceRow = representativeCard.locator('.recall-home-parity-card-source-row-stage639').first()
  const source = representativeCard.locator('.recall-home-parity-card-source-stage639').first()
  const chip = representativeCard.locator('.recall-home-parity-card-chip-stage639').first()
  ...
  const [representativeCardBox, previewBox, copyBox, titleBox, sourceRowBox, chipBox] = await Promise.all([
    readMeasuredBox(representativeCard, 'representative Stage 640 card'),
    readMeasuredBox(preview, 'Stage 640 card preview'),
    readMeasuredBox(copy, 'Stage 640 card copy block'),
    readMeasuredBox(title, 'Stage 640 card title'),
    readMeasuredBox(sourceRow, 'Stage 640 card source row'),
    readMeasuredBox(chip, 'Stage 640 card chip'),
  ])
  ...
  const [titleState, sourceState, chipState, canvasMetrics] = await Promise.all([
    ...
    page.evaluate(() => {
      ...
      if (!(canvas instanceof HTMLElement) || !(dayHeader instanceof HTMLElement) || !(grid instanceof HTMLElement)) {
        throw new Error('Expected Stage 640 Home canvas, day header, and grid.')
      }
  ...
  if (!(previewRatio < stage637PreviewRatioBaseline && previewRatio <= previewRatioCeiling)) {
    throw new Error(`Expected preview ratio below the Stage 637 baseline and within the target band, found ${previewRatio}.`)
  }
  if (!(copyRatio > stage637CopyRatioBaseline && copyRatio >= copyRatioFloor)) {
    throw new Error(`Expected copy ratio above the Stage 637 baseline and within the target band, found ${copyRatio}.`)
  }
  if (!(titleState.lineHeightPx > stage637TitleLineHeightPxBaseline && titleState.lineHeightPx >= titleLineHeightPxFloor)) {
    throw new Error(`Expected title line-height above the Stage 637 baseline, found ${titleState.lineHeightPx}.`)
  }
  if (!(titleToSourceGapPx > stage637TitleToSourceGapPxBaseline && titleToSourceGapPx >= titleToSourceGapPxFloor)) {
    throw new Error(`Expected title-to-source gap above the Stage 637 baseline, found ${titleToSourceGapPx}.`)
  }
  if (!(sourceToChipGapPx > stage637SourceToChipGapPxBaseline && sourceToChipGapPx >= sourceToChipGapPxFloor)) {
    throw new Error(`Expected source-to-chip gap above the Stage 637 baseline, found ${sourceToChipGapPx}.`)
  }
  if (!(sourceState.fontSizePx >= sourceFontSizePxFloor && sourceState.alpha >= sourceAlphaFloor)) {
    throw new Error(`Expected source-row legibility to stay at or above the Stage 637 band, found ${sourceState.fontSizePx}px and alpha ${sourceState.alpha}.`)
  }
  if (!(chipState.fontSizePx >= chipFontSizePxFloor && chipState.alpha >= chipAlphaFloor)) {
    throw new Error(`Expected chip legibility to stay at or above the Stage 637 band, found ${chipState.fontSizePx}px and alpha ${chipState.alpha}.`)
  }
  ...
  const webCard = canvas.locator('.recall-home-parity-card-stage639').first()
