...
const outputDir = process.env.RECALL_STAGE378_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE378_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE378_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE378_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE378_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE378_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(path.join(outputDir, 'stage378-desktop-first-study-finish-milestone-after-stage377-failure.png'), {
  force: true,
})
...
try {
  ...
  const {
    ...
    studyWideTop,
  } = await captureWideStudyArtifacts(desktopPage, outputDir, 'stage378')
  ...
  } = await captureFocusedStudyArtifacts(focusedPage, outputDir, 'stage378')

  await writeFile(
    path.join(outputDir, 'stage378-desktop-first-study-finish-milestone-after-stage377-validation.json'),
    JSON.stringify(
      {
        baseUrl,
        ...
        validationFocus: [
          ...
          'focused and narrow Study regression captures for pre-answer and answer-shown states after the desktop finish settles first',
        ],
      },
      null,
      2,
  ...
  if (desktopPage) {
    await desktopPage
      .screenshot({
        path: path.join(outputDir, 'stage378-desktop-first-study-finish-milestone-after-stage377-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (focusedPage && focusedPage !== desktopPage) {
    await focusedPage
      .screenshot({
        path: path.join(
          outputDir,
          'stage378-desktop-first-study-finish-milestone-after-stage377-failure-focused.png',
        ),
        fullPage: true,
      })
      .catch(() => undefined)
