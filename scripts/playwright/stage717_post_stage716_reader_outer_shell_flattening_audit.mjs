...
const outputDir = process.env.RECALL_STAGE717_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE717_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE717_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE717_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE717_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE717_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true

await mkdir(outputDir, { recursive: true })
await rm(path.join(outputDir, 'stage717-reader-outer-shell-audit-failure.png'), { force: true })
await rm(path.join(outputDir, 'stage717-reader-outer-shell-audit-regression-failure.png'), { force: true })
...
try {
  ...
  if (!(defaultReaderStageChromeHeight <= 104)) {
    throw new Error(`Expected the Reader pre-article chrome to compress further, found ${defaultReaderStageChromeHeight}.`)
  }
  if (defaultStageHasCardClass || defaultStageUsesPriorityShellClass) {
    throw new Error('Expected the Reader reading stage to stop using generic card / priority shell framing classes.')
  }
  if (defaultStageFramedAtRest) {
    throw new Error('Expected the outer Reader stage shell to stay visually quiet at rest.')
  }
  ...
  await writeFile(
    path.join(outputDir, 'stage717-post-stage716-reader-outer-shell-flattening-audit-validation.json'),
    JSON.stringify(
      {
        auditFocus: [
          'Reader should stop reading like a large outer card and instead present attached controls plus a dedicated article field.',
  ...
  if (readerPage) {
    await readerPage
      .screenshot({
        path: path.join(outputDir, 'stage717-reader-outer-shell-audit-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
  if (regressionPage && regressionPage !== readerPage) {
    await regressionPage
      .screenshot({
        path: path.join(outputDir, 'stage717-reader-outer-shell-audit-regression-failure.png'),
        fullPage: true,
      })
      .catch(() => undefined)
  }
