...
const outputDir = process.env.RECALL_STAGE277_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE277_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE277_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE277_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE277_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE277_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
const viewport = {
  width: 820,
  height: 980,
}
...
await rm(
  path.join(
    outputDir,
    'stage277-bundled-narrower-width-focused-study-answer-shown-support-continuation-deflation-after-stage276-failure.png',
  ),
  {
    force: true,
  },
...
try {
  ...
  const focusedOverviewNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage277-focused-overview-narrow-top.png',
  )
  ...
  const focusedStudyNarrowTop = await captureTopScreenshot(page, outputDir, 'stage277-focused-study-narrow-top.png')
  ...
  const focusedStudyAnswerShownNarrowTop = await captureTopScreenshot(
    page,
    outputDir,
    'stage277-focused-study-answer-shown-narrow-top.png',
  )
  ...
  await page.waitForTimeout(350)
  const readerNarrowTop = await captureTopScreenshot(page, outputDir, 'stage277-reader-narrow-top.png')

  await writeFile(
    path.join(
      outputDir,
      'stage277-bundled-narrower-width-focused-study-answer-shown-support-continuation-deflation-validation.json',
    ),
    JSON.stringify(
      {
        baseUrl,
