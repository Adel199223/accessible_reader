...
const outputDir = process.env.RECALL_STAGE476_OUTPUT_DIR ?? path.join(repoRoot, 'output', 'playwright')
const harnessDir =
  process.env.RECALL_STAGE476_PLAYWRIGHT_HARNESS ??
  'C:\\Users\\FA507\\AppData\\Local\\Temp\\accessible-reader-playwright'
const baseUrl = process.env.RECALL_STAGE476_BASE_URL ?? 'http://127.0.0.1:8000'
const headless = process.env.RECALL_STAGE476_HEADLESS === '0' ? false : true
const preferredChannel = process.env.RECALL_STAGE476_BROWSER_CHANNEL ?? 'msedge'
const allowChromiumFallback = process.env.RECALL_STAGE476_ALLOW_CHROMIUM_FALLBACK === '0' ? false : true
...
await rm(
  path.join(
    outputDir,
    'stage476-post-stage475-home-collection-lens-and-organizer-model-audit-failure.png',
  ),
  {
    force: true,
  },
...
await rm(
  path.join(
    outputDir,
    'stage476-post-stage475-home-collection-lens-and-organizer-model-audit-failure-reader.png',
  ),
  {
    force: true,
  },
...
try {
  ...
  const homeWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage476-home-wide-top.png')
  ...
  const homeCollectionsOrganizerDeckWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage476-home-collections-organizer-deck-wide-top.png',
    460,
    'locator',
  )
  ...
  const homeRecentLensBranchWideTop = await captureLocatorTopScreenshot(
    ...
    group,
    outputDir,
    'stage476-home-recent-lens-branch-wide-top.png',
    820,
    'locator',
  )
  const homeRecentLensBoardWideTop = await captureLocatorTopScreenshot(
    desktopPage,
    await getVisibleLocator(desktopPage, ['[aria-label="Saved library"]']),
    outputDir,
    'stage476-home-recent-lens-board-wide-top.png',
    880,
    'locator',
  )
  ...
  const homeCompactControlsWideTop = await captureLocatorTopScreenshot(
    ...
    outputDir,
    'stage476-home-compact-controls-wide-top.png',
    320,
    'locator',
  )
  ...
  const graphWideTop = await captureViewportScreenshot(desktopPage, outputDir, 'stage476-graph-wide-top.png')
  ...
  const readerOriginalWideTop = await captureViewportScreenshot(
    desktopReaderPage,
    outputDir,
    'stage476-reader-original-wide-top.png',
  )

  await writeFile(
    path.join(
      outputDir,
      'stage476-post-stage475-home-collection-lens-and-organizer-model-audit-validation.json',
    ),
    JSON.stringify(
      {
        auditFocus: [
          'wide desktop Home is audited first after the Stage 475 collection-lens and organizer-model reset',
          'Home evidence should show a true collection-led default organizer lens, a deliberate recent lens for branch navigation, and a compact organizer-hidden fallback that keeps the same controls accessible',
...
async function openOriginalReaderFromHome(page) {
  ...
  if ((await originalTab.getAttribute('aria-selected')) !== 'true') {
    throw new Error('Could not switch Reader into Original view for the Stage 476 audit.')
  }
