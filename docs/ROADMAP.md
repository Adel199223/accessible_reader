...
- The March 25, 2026 Stage 498-564 local continuity ladder now stands above clean `main`:
  - the Stage 498/499 plus Stage 533/534 Graph checkpoint set, the Stage 500-522 Home checkpoint set, the Stage 523-532 Reader checkpoint set, the Stage 535-562 legacy grouped-overview Home checkpoint set, the Stage 563/564 Home structural reset pair, and the Stage 506 launcher correction now define the latest local continuity checkpoint above the published clean-`main` baseline
  - the latest local evidence checkpoint is now the completed Stage 564 live Home/Graph/Reader audit, and `Home`, `Graph`, plus original-only `Reader` return to refreshed-baseline hold from this point
- The March 25, 2026 Stage 537-562 grouped-overview ladder is now a validated legacy Home baseline:
  - keep it as historical continuity evidence, but do not treat it as the active Home benchmark target after the Stage 563/564 reset unless the user explicitly asks to revisit that branch
- Current active queue: hold `Home`, `Graph`, and original-only `Reader` as refreshed parity baselines after the completed Stage 564 audit while `Notes` plus `Study` remain regression baselines; if the user explicitly reopens another bounded slice, `Home` remains the likeliest next target.
- Current mode: post-Stage-564 audit hold. Keep Reader generated-content work explicitly out of scope unless the user explicitly reprioritizes it.
