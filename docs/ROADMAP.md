...
- Current active queue: hold `Home`, `Graph`, and original-only `Reader` as refreshed parity baselines after the completed Stage 663 correction while `Notes` plus `Study` remain regression baselines; if the user explicitly reopens another bounded slice, `Home` remains the likeliest next target, now around richer screenshot-like or otherwise higher-fidelity preview generation for the sources that still fall back, especially paste/text or sources without usable saved image assets, rather than more shell, rail, toolbar, lower-card, metadata-only poster, or HTML-backed preview-framing micro-trims.
- Current mode: post-Stage-663 correction hold. Keep Reader generated-content work explicitly out of scope unless the user explicitly reprioritizes it.
