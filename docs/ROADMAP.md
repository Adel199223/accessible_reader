...
- The March 27, 2026 Stage 498-662 local continuity ladder now stands above clean `main`:
  ...
  - the latest local evidence checkpoint is now the completed Stage 662 live Home/Graph/Reader audit, and `Home`, `Graph`, plus original-only `Reader` return to refreshed-baseline hold from this point
...
- Current active queue: hold `Home`, `Graph`, and original-only `Reader` as refreshed parity baselines after the completed Stage 662 audit while `Notes` plus `Study` remain regression baselines; if the user explicitly reopens another bounded slice, `Home` remains the likeliest next target, now around richer screenshot-like or otherwise higher-fidelity preview generation for the sources that still fall back, especially paste/text or sources without usable saved image assets, rather than more shell, rail, toolbar, lower-card, metadata-only poster, or HTML-backed preview-framing micro-trims.
- Current mode: post-Stage-662 audit hold. Keep Reader generated-content work explicitly out of scope unless the user explicitly reprioritizes it.
