# ExecPlan: Stage 197 Recall Home Tail Carry Extension And Reveal Footer Delay

## Summary
- Implement the bounded Home follow-up selected by the Stage 196 benchmark audit.
...
## Problem Statement
- Stage 196 confirmed that Stage 195 materially calmed Home again by carrying slightly more visible continuation through the landing tail and by pushing the reveal footer lower.
...
- Preserve the calmer Stage 195 shell and keep Graph, Study, and focused Study stable.
...
- `frontend/src/components/RecallWorkspace.tsx`
  - carry the merged Home continuation tail farther and delay the reveal footer without undoing the calmer Stage 195 landing rhythm
...
- `scripts/playwright/stage197_home_tail_carry_extension_edge.mjs`
  - capture fresh Stage 197 Home, Graph, Study, and focused-Study artifacts in Windows Edge
...
- `node scripts/playwright/stage197_home_tail_carry_extension_edge.mjs`
- `node --check scripts/playwright/stage198_post_stage197_benchmark_audit_edge.mjs`
...
## Outcome
- Stage 197 carried the visible Home continuation farther through the landing tail so one more reopen item stays on the page before the reveal footer row.
- The reveal footer now lands lower in the fresh Home artifact, while Graph, Study, and focused Study matched the Stage 196 artifacts byte-for-byte.
