# ExecPlan: Stage 193 Recall Home Tail Carry Extension And Reveal Footer Delay

## Summary
- Implement the bounded Home follow-up selected by the Stage 192 benchmark audit.
...
## Problem Statement
- Stage 192 confirmed that Stage 191 materially calmed Home again by carrying slightly more visible Home continuation through the landing tail and by pushing the reveal footer lower.
...
- Preserve the calmer Stage 191 shell and keep Graph, Study, and focused Study stable.
...
- `frontend/src/components/RecallWorkspace.tsx`
  - carry the merged Home continuation tail farther and delay the reveal footer without undoing the calmer Stage 191 landing rhythm
...
- `scripts/playwright/stage193_home_tail_carry_extension_edge.mjs`
  - capture fresh Stage 193 Home, Graph, Study, and focused-Study artifacts in Windows Edge
...
- `node scripts/playwright/stage193_home_tail_carry_extension_edge.mjs`
- `node --check scripts/playwright/stage194_post_stage193_benchmark_audit_edge.mjs`
...
## Outcome
- Stage 193 carried the visible Home continuation farther through the landing tail so one more reopen item stays on the page before the reveal footer row.
- The reveal footer now lands lower in the fresh Home artifact, while Graph, Study, and focused Study matched the Stage 192 artifacts byte-for-byte.
