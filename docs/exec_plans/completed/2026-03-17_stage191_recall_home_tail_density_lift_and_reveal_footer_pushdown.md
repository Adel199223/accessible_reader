# ExecPlan: Stage 191 Recall Home Tail Density Lift And Reveal Footer Pushdown

## Summary
- Implement the bounded Home follow-up selected by the Stage 190 benchmark audit.
...
## Problem Statement
- Stage 190 confirmed that Stage 189 materially calmed Home again by carrying the visible continuation farther through the landing tail and by delaying the reveal footer.
...
- Preserve the calmer Stage 189 shell and keep Graph, Study, and focused Study stable.
...
- `frontend/src/components/RecallWorkspace.tsx`
  - carry slightly more visible Home continuation through the landing tail and push the reveal footer lower without undoing the calmer Stage 189 landing rhythm
...
- `scripts/playwright/stage191_home_tail_density_lift_edge.mjs`
  - capture fresh Stage 191 Home, Graph, Study, and focused-Study artifacts in Windows Edge
...
- `node scripts/playwright/stage191_home_tail_density_lift_edge.mjs`
- `node --check scripts/playwright/stage192_post_stage191_benchmark_audit_edge.mjs`
...
## Outcome
- Stage 191 carried slightly more visible Home continuation through the landing tail so one more reopen item stays on the page before the reveal footer row.
- The reveal footer now lands lower in the fresh Home artifact, while Graph, Study, and focused Study matched the Stage 190 artifacts byte-for-byte.
