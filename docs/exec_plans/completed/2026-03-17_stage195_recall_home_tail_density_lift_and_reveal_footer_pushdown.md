# ExecPlan: Stage 195 Recall Home Tail Density Lift And Reveal Footer Pushdown

## Summary
- Implement the bounded Home follow-up selected by the Stage 194 benchmark audit.
...
## Problem Statement
- Stage 194 confirmed that Stage 193 materially calmed Home again by carrying the visible continuation farther through the landing tail and by delaying the reveal footer.
...
- Preserve the calmer Stage 193 shell and keep Graph, Study, and focused Study stable.
...
- `frontend/src/components/RecallWorkspace.tsx`
  - carry slightly more visible Home continuation through the landing tail and push the reveal footer lower without undoing the calmer Stage 193 landing rhythm
...
- `scripts/playwright/stage195_home_tail_density_lift_edge.mjs`
  - capture fresh Stage 195 Home, Graph, Study, and focused-Study artifacts in Windows Edge
...
- `node scripts/playwright/stage195_home_tail_density_lift_edge.mjs`
- `node --check scripts/playwright/stage196_post_stage195_benchmark_audit_edge.mjs`
...
## Outcome
- Stage 195 carried slightly more visible Home continuation through the landing tail so one more reopen item stays on the page before the reveal footer row.
- The reveal footer now lands lower in the fresh Home artifact, while Graph, Study, and focused Study matched the Stage 194 artifacts byte-for-byte.
- The next step is now a benchmark audit so the denser Home tail and lower reveal footer can be judged against the Recall benchmark before another bounded follow-up or a surface shift is chosen.
