# ExecPlan: Stage 183 Recall Home Continuation Tail Extension And Reveal Footer Lowering

## Summary
- Implement the bounded Home follow-up selected by the Stage 182 benchmark audit.
...
## Problem Statement
- Stage 182 confirmed that Stage 181 materially calmed Home again by carrying the visible continuation farther through the landing tail and by delaying the reveal footer.
...
- The next pass should carry the Home landing farther through the continuation tail and lower the reveal footer while preserving the calmer shell, the restrained opening, and the current Graph, Study, and focused-study baselines.
...
- Preserve the calmer Stage 181 shell and keep Graph, Study, and focused Study stable.
...
- `frontend/src/components/RecallWorkspace.tsx`
  - carry the merged Home continuation tail farther and lower the reveal footer without undoing the calmer Stage 181 landing rhythm
...
- `scripts/playwright/stage183_home_continuation_tail_extension_edge.mjs`
  - capture fresh Stage 183 Home, Graph, Study, and focused-Study artifacts in Windows Edge
...
- `node scripts/playwright/stage183_home_continuation_tail_extension_edge.mjs`
- `node --check scripts/playwright/stage184_post_stage183_benchmark_audit_edge.mjs`
...
## Outcome
- Stage 183 carried the visible Home continuation farther through the landing tail so one more reopen item stays on the page before the reveal footer row.
- The reveal footer now lands lower in the fresh Home artifact, while Graph, Study, and focused Study matched the Stage 182 artifacts byte-for-byte.
