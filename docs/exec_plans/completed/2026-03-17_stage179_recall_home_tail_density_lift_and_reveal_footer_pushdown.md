# ExecPlan: Stage 179 Recall Home Tail Density Lift And Reveal Footer Pushdown

## Summary
- Implement the bounded Home follow-up selected by the Stage 178 benchmark audit.
...
## Problem Statement
- Stage 178 confirmed that Stage 177 materially calmed Home again by carrying the visible continuation tail farther and lowering the reveal footer.
...
- Preserve the calmer Stage 177 shell and keep Graph, Study, and focused Study stable.
...
- `frontend/src/components/RecallWorkspace.tsx`
  - carry the merged Home continuation tail farther and push the reveal footer lower without undoing the calmer Stage 177 landing rhythm
...
- `scripts/playwright/stage179_home_tail_density_lift_edge.mjs`
  - capture fresh Stage 179 Home, Graph, Study, and focused-Study artifacts in Windows Edge
...
- `node scripts/playwright/stage179_home_tail_density_lift_edge.mjs`
- `node --check scripts/playwright/stage180_post_stage179_benchmark_audit_edge.mjs`
...
## Outcome
- Stage 179 carried slightly more visible Home continuation through the landing tail so one more reopen item stays on the page before the reveal footer row.
- The reveal footer now lands lower in the fresh Home artifact, while Graph, Study, and focused Study matched the Stage 178 artifacts byte-for-byte.
