# ExecPlan: Stage 185 Recall Home Tail Carry Extension And Reveal Footer Lowering

## Summary
- Implement the bounded Home follow-up selected by the Stage 184 benchmark audit.
...
## Problem Statement
- Stage 184 confirmed that Stage 183 materially calmed Home again by carrying the visible continuation farther through the landing tail and by lowering the reveal footer.
...
- Preserve the calmer Stage 183 shell and keep Graph, Study, and focused Study stable.
...
- `frontend/src/components/RecallWorkspace.tsx`
  - carry the merged Home continuation tail farther and lower the reveal footer without undoing the calmer Stage 183 landing rhythm
...
- `scripts/playwright/stage185_home_tail_carry_extension_edge.mjs`
  - capture fresh Stage 185 Home, Graph, Study, and focused-Study artifacts in Windows Edge
...
- `node scripts/playwright/stage185_home_tail_carry_extension_edge.mjs`
- `node --check scripts/playwright/stage186_post_stage185_benchmark_audit_edge.mjs`
...
## Outcome
- Stage 185 carried the visible Home continuation farther through the landing tail so one more reopen item stays on the page before the reveal footer row.
- The reveal footer now lands lower in the fresh Home artifact, while Graph, Study, and focused Study matched the Stage 184 artifacts byte-for-byte.
