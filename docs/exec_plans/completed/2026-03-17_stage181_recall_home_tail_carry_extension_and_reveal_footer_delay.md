# ExecPlan: Stage 181 Recall Home Tail Carry Extension And Reveal Footer Delay

## Summary
- Implement the bounded Home follow-up selected by the Stage 180 benchmark audit.
...
## Problem Statement
- Stage 180 confirmed that Stage 179 materially calmed Home again by carrying slightly more visible continuation through the landing tail and by pushing the reveal footer lower.
...
- Preserve the calmer Stage 179 shell and keep Graph, Study, and focused Study stable.
...
- `frontend/src/components/RecallWorkspace.tsx`
  - carry the merged Home continuation tail farther and delay the reveal footer without undoing the calmer Stage 179 landing rhythm
...
- `scripts/playwright/stage181_home_tail_carry_extension_edge.mjs`
  - capture fresh Stage 181 Home, Graph, Study, and focused-Study artifacts in Windows Edge
...
- `node scripts/playwright/stage181_home_tail_carry_extension_edge.mjs`
- `node --check scripts/playwright/stage182_post_stage181_benchmark_audit_edge.mjs`
...
## Outcome
- Stage 181 carried the visible Home continuation farther through the landing tail so one more reopen item stays on the page before the reveal footer row.
- The reveal footer now lands lower in the fresh Home artifact, while Graph, Study, and focused Study matched the Stage 180 artifacts byte-for-byte.
