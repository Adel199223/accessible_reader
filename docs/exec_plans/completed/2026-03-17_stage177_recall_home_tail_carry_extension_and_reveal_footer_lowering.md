# ExecPlan: Stage 177 Recall Home Tail Carry Extension And Reveal Footer Lowering

## Summary
- Implement the bounded Home follow-up selected by the Stage 176 benchmark audit.
...
## Problem Statement
- Stage 176 confirmed that Stage 175 materially calmed Home again by lifting the visible continuation tail and delaying the reveal footer.
...
- Preserve the calmer Stage 175 shell and keep Graph, Study, and focused Study stable.
...
- `frontend/src/components/RecallWorkspace.tsx`
  - carry the merged Home continuation tail farther and lower the reveal footer without undoing the calmer Stage 175 landing rhythm
...
- `scripts/playwright/stage177_home_tail_carry_extension_edge.mjs`
  - capture fresh Stage 177 Home, Graph, Study, and focused-Study artifacts in Windows Edge
...
- `node scripts/playwright/stage177_home_tail_carry_extension_edge.mjs`
- `node --check scripts/playwright/stage178_post_stage177_benchmark_audit_edge.mjs`
...
## Outcome
- Stage 177 carried the visible Home continuation farther through the landing tail so one more reopen item stays on the page before the reveal footer row.
- The reveal footer now lands lower in the fresh Home artifact, while Graph, Study, and focused Study matched the Stage 176 artifacts byte-for-byte.
