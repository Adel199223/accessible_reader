# ExecPlan: Stage 404 Post-Stage-403 Home Library Sheet Flattening Audit

## Summary
- Audit the Stage 403 `Home` library-sheet flattening reset against the current Recall library direction.
...
- Supporting Home crops:
  ...
  - pinned reopen shelf
  - primary-flow crop showing the flatter selected-group sheet and later, softer footer stop
- Focused regressions second:
  - only if needed to confirm the shared shell stayed coherent

## Acceptance
- The audit states clearly whether Stage 403 moved `Home` closer to Recall's current continuous library-sheet direction.
