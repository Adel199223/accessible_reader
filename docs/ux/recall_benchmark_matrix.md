...
| Add Content modal | User-provided Add Content screenshot in this thread, supported by [Add Content tutorial](https://docs.getrecall.ai/docs/tutorials/add-content) | `output/playwright/stage709-home-add-dialog-wide-top.png` | Present one deliberate import modal with grouped source modes and a stronger primary entry region while keeping the same local-first text, URL, and file import capabilities. | Cleaner title and launcher hierarchy, clearer mode selection, more deliberate primary input panel, calmer support framing, a stronger file drop target, and clear separation from Notebook creation. | Keep only the import modes this product actually supports; unsupported Recall tabs such as Wiki or Extension stay out of scope. | Refreshed parity baseline |
| Knowledge graph | User-provided Recall Graph screenshot set in this thread on 2026-03-26, supported by [Knowledge Graph overview](https://docs.getrecall.ai/docs/features/knowledge-graph/overview), [Graph navigation](https://docs.getrecall.ai/deep-dives/graph/navigation), [Graph filtering and customization](https://docs.getrecall.ai/deep-dives/graph/filtering-and-customization), [Graph selection and exploration](https://docs.getrecall.ai/deep-dives/graph/selection-and-exploration), and [Recall Release Notes: Jan 12, 2026 - Graph View 2.0 and much more](https://feedback.getrecall.ai/changelog/recall-release-notes-jan-12-2026-graph-view-20-and-much-more) | `output/playwright/stage783-graph-wide-top.png` | Keep the graph canvas dominant with the left `Graph Settings` panel docked open by default, move title search plus match navigation and `Fit to view` / `Lock graph` into a compact top-right utility corner, keep presets/filter query/timeline/visibility/groups inside the settings panel, surface a compact bottom-left count pill plus bottom-right help controls, and reserve the contextual focus tray plus detail dock for selected-node or path workflows only. | Darker calmer canvas, circular node cores with external labels, thinner quieter edges, less floating dashboard chrome, an at-rest state that feels browse-first rather than inspect-first, a Replay-tour/help flow that mirrors the Recall tutorial cards, selected/path states that feel grounded and contextual instead of permanently occupying the whole lower workbench, flatter panel/control chrome that keeps `Presets`, `Filters`, and `Groups` prominent while advanced rows stay collapsed by default, shorter idle rail copy with the preset draft field hidden at rest, icon-first fit/lock controls, and a count chip that fully clears the docked settings rail. | Keep evidence grounding, local saved-view semantics, source reopen actions, and local provenance in the flow; this remains Recall-close rather than a literal clone. | Refreshed parity baseline |
| Study / review | User-provided spaced-repetition screenshot in this thread on 2026-03-18, supported by [Quiz & Spaced Repetition](https://docs.getrecall.ai/docs/features/quiz-and-spaced-repetition), [Release: 29 November 2024 // Recall Review Reminders](https://feedback.getrecall.ai/changelog/release-29-november-2024-recall-review-reminders), and [Recall Release Notes: Feb 19, 2026 - Quiz 2.0 with Shared Challenges](https://feedback.getrecall.ai/changelog/recall-release-notes-feb-19-2026-quiz-20-with-shared-challenges) | `output/playwright/stage783-study-wide-top.png` | Recenter the page on the review task with a guided flow and simpler queue support, rather than treating review as one panel inside a dashboard. | Cleaner step hierarchy, clearer main action, reduced sidebar weight and card framing. | Keep local FSRS state, source evidence, and Reader reopen actions. | Refreshed regression baseline |
| Notebook placement + note workspace | User-provided Recall `/items` screenshot in this thread on 2026-03-28, supported by [Our Biggest Update Yet: March 18, 2025 // Augmented Browsing, Chat & More](https://feedback.getrecall.ai/changelog/our-biggest-update-yet-march-18-2025-augmented-browsing-chat-more), the [Recall changelog](https://feedback.getrecall.ai/changelog) October 3, 2025 entry, and [Release Notes: 9 December 2025 – One place for all your knowledge including your personal notes](https://feedback.getrecall.ai/changelog/release-notes-9-december-2025-one-place-for-all-your-knowledge-including-your-pe) | `output/playwright/stage783-notebook-wide-top.png` | Remove the visible top-level `Notes` rail destination, reach note-taking from `Home` / Library plus saved-source workspaces, keep note items first-class inside the Library canvas, and preserve the stronger browse/detail notebook workbench plus source-focused split view rather than treating notes as a separate primary section. | Dark neutral shell with no visible `Notes` rail entry, a pen-style `New note` action beside `Add`, Library-native note rows that read like first-class saved items rather than a transplanted standalone Notes list, a calmer embedded notebook workbench inside `Home` / Library, a source-focused workbench that stacks cleanly inside the focused split instead of feeling like a squeezed standalone notes dashboard, `Notebook` wording in saved-source tabs and Reader-adjacent note flows, and search / Reader / source handoffs that reopen the notebook workspace instead of reviving a standalone Notes destination. | Keep local-first note CRUD, anchored reopen, note-to-source links, graph/study participation, and the hidden `/recall?section=notes` compatibility alias while the visible placement matches Recall more closely. | Refreshed parity baseline |
| Reader workspace | User-provided Reader screenshot in this thread on 2026-03-18 plus the user-provided March 28, 2026 Reader screenshots in this thread, supported by [Recall docs](https://docs.getrecall.ai/) for shell direction | `output/playwright/stage783-reader-original-wide-top.png` | Make original document text more dominant, compress the header/control stack, and demote side support into a calmer dock or tray instead of a co-equal right column while keeping Notebook as the editable companion surface. | More reading-first space, calmer side chrome, tighter transport/header rhythm, a thinner attached source-support rail, a flatter embedded library seam, a slimmer source-workspace lead-in, a quieter Reader-specific topbar, a calmer pre-article seam before the document starts, a joined article-shell-to-dock deck instead of two separate cards, a calmer attached tray that no longer behaves like a co-equal always-open dock, a more compact generated-mode context band with inline `Summary` detail controls, no visible `Reading deck`, `Reader dock`, or other over-explained resting copy above the article, an icon-first compact support seam at rest with no explanatory copy, only two compact meta chips, hidden visible Source / Notebook tab labels until the support rail is explicitly opened, a centered dedicated article field that carries the visible reading chrome instead of leaving the whole lane framed as one broad slab, a visually quiet outer Reader stage so the page no longer reads like one large framed card around the reading deck, a flatter attached source strip that reads like a seam instead of another boxed panel, one attached source title seam that carries the visible document heading so the duplicate Reader-stage title is retired, a calmer control band with no separate utility row, no visible at-rest `View` / `Read aloud` labels, no repeated current-view chip, a shorter stage-chrome stack above the article, an expanded `Source` support rail that now behaves like a lighter attached context strip instead of a second mini workspace, a centered compact resting deck whose metadata row, control band, article field, and support seam now behave like one deliberate reading band rather than stretching across the full stage, a fused at-rest support cluster that lives inside the main attached control band instead of leaving a separate right-side mini-dock visible by default, a compact-width source-strip shell that now matches that centered reading band at rest, a quieter compact source-destination trigger in the source strip instead of a full stacked `Overview / Reader / Notebook / Graph / Study` row, generated-mode empty states that now stay inside the Reader deck as calm inline placeholders instead of surfacing as a full-width alert slab, a visible mode strip that now follows the active document's real available modes instead of keeping unavailable AI modes as equal-weight resting pills, one labeled primary `Read aloud` action at rest instead of exposing the full idle transport cluster plus sentence progress before playback starts, a leaner source-strip meta row that now keeps only source-type and note-count cues instead of repeating available-view or current-view chips, an inline `Source` trigger attached to the source identity seam instead of a detached right-edge `Open` control, an active-reading overflow that now exposes inline `Light` / `Dark` theme choices instead of preserving either a bulky Reader settings drawer or a separate active-reading Theme dialog, a compact source seam that now drops the generic `Local source` fallback while the overflow holds only inline theme plus read-aloud voice/rate controls, a nearby Notebook handoff that now lives on the source-strip note chip instead of duplicating Notebook inside the overflow, a tighter active-reading overflow footprint that now presents `Theme` and `Voice` as inline utility rows while keeping `Rate` visible without reopening a large floating card, a short-document article field that now compresses the dead lower slab without shrinking longer-form reading lanes, a derived lead-in that now uses the mode title plus provenance without a redundant descriptor badge or repeated context paragraph above missing generated-state messaging, derived modes whose metadata row now disappears entirely when it would only repeat provenance, summary-detail, or readiness cues already visible elsewhere in the same block, a `Summary` lead-in whose detail selector now sits inside the same seam without an extra visible `Detail` label, expanded Source support that now exposes only one clear `Hide` affordance instead of duplicating the close control inside the embedded library, expanded Reader support that now keeps the source metadata only in the compact source strip instead of repeating it again inside the support dock header, an expanded Source panel that now starts straight with the search field and saved-source list instead of another nested heading stack, and a Reader Notebook workbench that now reaches the anchor preview plus note editor without a redundant selected-note heading stack while the saved-note switcher stays reserved for the other nearby notes instead of repeating the active one. | Preserve `/reader` compatibility, read-aloud, highlighting, note adjacency, and source-library access. Generated `Original`, `Reflowed`, `Simplified`, and `Summary` outputs remain frozen unless the user explicitly reprioritizes generated-content work. | Refreshed parity baseline |
...
- Current refreshed audit set from April 2, 2026 after the Stage 787 audit:
  - `Home`: `output/playwright/stage787-home-wide-top.png`
  - Add Content modal: `output/playwright/stage709-home-add-dialog-wide-top.png`
  - Add Content paste crop: `output/playwright/stage709-add-dialog-paste-mode.png`
  - Add Content web crop: `output/playwright/stage709-add-dialog-web-mode.png`
  - Add Content file crop: `output/playwright/stage709-add-dialog-file-mode.png`
  - Add Content on `Reader`: `output/playwright/stage709-reader-add-dialog-wide-top.png`
  - `Graph`: `output/playwright/stage787-graph-wide-top.png`
  - embedded `Notebook`: `output/playwright/stage787-notebook-wide-top.png`
  - original-only `Reader`: `output/playwright/stage787-reader-original-wide-top.png`
  - Reader default state: `output/playwright/stage787-reader-default-wide-top.png`
  - Reader control-ribbon crop: `output/playwright/stage787-reader-control-ribbon.png`
  - Reader control-overflow crop: `output/playwright/stage787-reader-control-overflow.png`
  - Reader source-strip crop: `output/playwright/stage787-reader-source-strip.png`
  - Reader Source support opened: `output/playwright/stage787-reader-source-support-open.png`
  - Reader Notebook workbench: `output/playwright/stage787-reader-notebook-workbench.png`
  - Reader reflowed: `output/playwright/stage787-reader-reflowed-wide-top.png`
  - Reader summary: `output/playwright/stage787-reader-summary-wide-top.png`
  ...
  - `Study`: `output/playwright/stage787-study-wide-top.png`
  - `Study` review dashboard crop: `output/playwright/stage707-study-dashboard-wide-top.png`
  - `Study` review lane crop: `output/playwright/stage707-study-review-lane-wide-top.png`
  - `Study` questions/support crop: `output/playwright/stage707-study-questions-support-wide-top.png`
  - `Study` evidence/support crop: `output/playwright/stage707-study-evidence-support-wide-top.png`
  - focused reader-led `Study` split: `output/playwright/stage707-focused-study-narrow-top.png`
  - Reader `Simplified`: not captured in the Stage 787 live audit because the audit recorded `simplifiedViewAvailable: false` on the current dataset.
