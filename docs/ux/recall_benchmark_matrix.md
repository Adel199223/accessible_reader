...
| --- | --- | --- | --- | --- | --- | --- |
| Shared shell + Home | User-provided Home screenshot in this thread on 2026-03-18, supported by [Recall docs](https://docs.getrecall.ai/), the [Tagging deep dive](https://docs.getrecall.ai/deep-dives/tagging), the [Feb 6, 2026 release notes](https://feedback.getrecall.ai/changelog/recall-release-notes-feb-6-2026-ocr-improved-organization-track-your-reading-pos), the [Release: 15 November 2024 // Exciting Quality of Life Improvements!](https://feedback.getrecall.ai/changelog/release-15-november-2024-exciting-quality-of-life-improvements), and the [Recall changelog](https://feedback.getrecall.ai/changelog) | `output/playwright/stage521-home-wide-top.png` | Let the organizer rail own the current lens, support true manual ordering plus desktop batch work, keep the right side beginning as a grouped overview workspace before explicit branch drill-in, and expose a real resizable organizer rail for wider browse work. | Dark neutral shell, restrained borders, fewer repeated labels, lighter metadata, a clearer organizer overview/reset row, calmer grouped overview cards, a tighter organizer utility header and control stack, a flatter integrated organizer utility cluster, a denser selected-branch list, lighter top-level group rows and count badges, visible drag handles in manual mode, a bottom organizer selection bar that reads like a real batch-action rail, a lighter selected-group card shell, titles that read like calmer row content instead of boxed mini headings, softer guttering between adjacent selected-group cards, a calmer selected-group board grid that reads more like one continuous results sheet than tiled mini-panels, and calmer title wrapping with steadier row-height behavior through longer selected-group titles. | Keep local-first wording, current core sections, and the shell-level `Search` / `New` actions. | Refreshed parity baseline |
...
- Current refreshed audit set from March 23, 2026 after the Stage 520 audit and Stage 521 implementation refresh:
  - `Home`: `output/playwright/stage521-home-wide-top.png`
  - `Graph`: `output/playwright/stage520-graph-wide-top.png`
  - `Notes`: `output/playwright/stage379-notes-wide-top.png`
  - `Reader` original-only: `output/playwright/stage520-reader-original-wide-top.png`
...
- The latest completed audit is now Stage 520, the latest green implementation validation is now Stage 521, and the next product move is the pre-staged Stage 522 live audit from the Stage 521 baseline.
- Stay on `Home` for the Stage 522 selected-group audit while `Graph` and original-only `Reader` stay regression surfaces.
